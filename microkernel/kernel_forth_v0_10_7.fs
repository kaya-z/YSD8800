...
\ Version: 0.10.7   v0.10.6 + Step 5-6a FILE-WRITE-IMPL（単一セクタ・ロールバックなし）実装
...
: FILE-WRITE-IMPL  ( arg2 arg1 arg0 tid -- )   \ arg0=name arg1=src arg2=size
    ...
    FW-SIZE @ 0< IF                 \ 案1: 最上位bit=1（$8000以上）→不正
        E-INVAL R> REPLY-OK EXIT
    THEN
    FW-SIZE @ FS-SECSIZE > IF       \ ★5-6a 限定: size>512 は単一セクタ制約で弾く
        E-INVAL R> REPLY-OK EXIT    \  （5-6b で複数セクタ化し解除）
    THEN
    ...
    \ [G] 手順6: 空き容量チェック（5-6a は n=1 固定 → 最終使用 LBA = next_free）
    FS-NEXT-FREE @ FS-TOTAL-SEC @ >= IF   \ next_free >= total なら範囲外
    ...
    \ [I] 手順8: データ書き込み（単一セクタ・書込順序の先頭）
    FW-SIZE @ FS-SECSIZE = IF        \ ちょうど512B：src を直接書く
        FW-START @ FW-SRC @ STOR-WRITE-1   \ ( r0 )
    ELSE                             \ size<512（size=0 含む）：端数0パディング
        FS-SECBUF 0 FS-SECSIZE MEMSET-B    \ SECBUF 全512B を 0 クリア
        FS-SECBUF FW-SRC @ FW-SIZE @ MEMCPY-B  \ src を size バイトコピー(len=0安全)
        FW-START @ FS-SECBUF STOR-WRITE-1  \ ( r0 )  SECBUF を書く
    THEN
    ...
    SB-LOAD 0<> IF
        E-IOERR R> REPLY-OK EXIT     \ SB-LOAD 失敗（5-6a は巻戻しなし）
    THEN
    FS-NEXT-FREE @   FS-SECBUF 24 +  !   \ SECBUF+24 = next_free_sec
    FS-FILE-COUNT @  FS-SECBUF 26 +  !   \ SECBUF+26 = file_count
    SB-STORE 0<> IF
        E-IOERR R> REPLY-OK EXIT     \ SB-STORE 失敗（5-6a は巻戻しなし・5-6c で対応）
    THEN

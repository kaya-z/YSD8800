...
\ Version: 0.10.2   Ph.4 Step 5-1: FILE-STAT-IMPL 実装版
...
\
\ ★★★ v0.10.2 追加点 (2026-05-24) ★★★
\ Ph.4 Step 5-1 FILE-STAT-IMPL 実装:
\   設計書 §4.3.5 / §6.2 に基づき FILE_STAT (0x0206) ハンドラを実装。
\   入力 stack: ( arg2 arg1 arg0 tid -- )  arg0=name_addr, arg1=stat_buf, arg2=未使用
\   FS-MOUNTED チェック → DIR-FIND → de_addr 計算 → 16B (size+start_sec+
\   sec_count+flags+予約) を stat_buf に書き込み → REPLY-OK
\   FS未マウント時は E-IOERR、name 不在時は E-NOENT を REPLY。
\   stat_buf の保護は DIR-FIND 前に R スタックへ退避することで実現
\   (FM-WK 系破壊から守る、机上検証 CK1-CK5 済み)。
...
\ --------------------------------------------------------------------
\ FILE-STAT-IMPL  ( arg2 arg1 arg0 tid -- )                   v0.10.2 実装
...
\
\ 机上検証メモ (CK1-CK5):
\   CK1: ROT DROP で arg2 を捨て ( stat_buf name_addr ) になる
\   CK2: DIR-FIND 前に stat_buf を R へ退避、FIND 後に R> で戻す
\        (DIR-FIND が NAME-EQ? 経由で FM-WK-* を破壊するため)
\   CK3: SWAP DUP -1 = IF の判定で stack ( stat_buf index ) に整形
\   CK4: MEMCPY-B 前に OVER で stat_buf を複製、MEMSET-B 用に保持
\   CK5: 最終 REPLY-OK は ( r0 tid ) を消費
\   R スタック: tid 1段のみ常駐、stat_buf は一時退避2段。最大2段 (安全範囲)
\ --------------------------------------------------------------------
: FILE-STAT-IMPL  ( arg2 arg1 arg0 tid -- )
    >R                              \ R: tid
    \ stack: ( arg2 arg1 arg0 ) = ( 0 stat_buf name_addr )

    \ FS-MOUNTED チェック (最初に: 引数を捨てる必要があるため)
    FS-MOUNTED @ 0= IF
        DROP DROP DROP              \ arg2 arg1 arg0 を全部捨てる
    ...
    16 +                            \ ( stat_buf src )
    OVER                            \ ( stat_buf src stat_buf )  ← stat_buf 複製
...
\ --------------------------------------------------------------------
\ FILE-DISPATCH-STAT  ( arg2 arg1 arg0 op client_tid -- )      v0.10.2
\ Step 5-1 暫定版: FILE-STAT-OP のみ処理する簡易ディスパッチ。
\ 他の op は E-INVAL を REPLY する。
\ Step 5-2 以降で FILE-LIST-IMPL 追加時に拡張、最終的に §6.1 の
\ 完全な FILE-DISPATCH へ移行する。
\
\ 入力 stack (底→TOS): arg2 arg1 arg0 op client_tid
\ --------------------------------------------------------------------
: FILE-DISPATCH-STAT  ( arg2 arg1 arg0 op tid -- )
    >R                              \ R: tid
    \ stack: ( arg2 arg1 arg0 op )
    DUP FILE-STAT-OP = IF
        DROP R> FILE-STAT-IMPL EXIT  \ ( arg2 arg1 arg0 tid -- )
    THEN
    \ 未対応 op
    DROP DROP DROP DROP             \ arg2 arg1 arg0 op を全部捨てる
    E-INVAL R> REPLY-OK ;

\ --------------------------------------------------------------------
\ FILEMGR-TASK  ( -- )                                       v0.10.2 更新
\ FileMgr メインタスク (tid=4)。STOR-DRV-TASK と同形のサーバループ。
\ v0.10.0: FILEMGR-INIT-DBG → 'M'/'m' 出力 → IPC4-RECV AGAIN ループ
\ v0.10.2: IPC4-RECV → FILE-DISPATCH-STAT を挿入 (Step 5-1 STAT 単体)
\ --------------------------------------------------------------------
: FILEMGR-TASK  ( -- )
    FILEMGR-INIT-DBG                    \ デバッグ版 (段階ごとに文字出力, 末尾で 'M' 出力)
    BEGIN
        IPC4-RECV                       \ ( -- msg3 msg2 msg1 msg0 )
        DI-OP
        IPC4-SENDER-DIRECT >R           \ R: client_tid
        ...
        FILE-DISPATCH-STAT              \ v0.10.2: STAT 単体 dispatch
    AGAIN ;
...
\ ====================================================================
\ Step 5-1 (v0.10.2): FILEMGR-TEST-TASK 簡易版 — STAT 単体動作確認
\ ====================================================================
\ 目的: FILE-STAT-IMPL が動作することを最小限のテストで確認する
\
\ テスト内容 (本版):
\   T1: 存在しないファイル "no.txt" を FILE_STAT → r0=E-NOENT を期待
\       成功なら 'Q' (=$51) を出力、失敗なら 'q' (=$71) を出力
\
\ 期待出力: ABCXD P Q   (UART-TEST + STOR-TEST + STAT-TEST=成功)
\
\ 注: 完全な FILEMGR-TEST-TASK は Step 6 で実装。本版は STAT 動作確認用
\ ====================================================================

\ FILEMGR-TEST 用バッファアドレス (設計書 §8.3 と一致)
$EC00 CONSTANT FT-NAME-BUF       \ 16 B
$EF00 CONSTANT FT-STAT-BUF       \ 32 B

: FILEMGR-TEST-TASK  ( -- )
    \ FT-NAME-BUF に "no.txt\0" を書き込む
    $6E FT-NAME-BUF       C!     \ 'n'
    $6F FT-NAME-BUF 1 +   C!     \ 'o'
    $2E FT-NAME-BUF 2 +   C!     \ '.'
    $74 FT-NAME-BUF 3 +   C!     \ 't'
    $78 FT-NAME-BUF 4 +   C!     \ 'x'
    $74 FT-NAME-BUF 5 +   C!     \ 't'
    $00 FT-NAME-BUF 6 +   C!     \ '\0'

    \ FILE_STAT 送信
    \ IPC4-CALL ( msg3 msg2 msg1 msg0 tid -- r3 r2 r1 r0 )
    \   msg3 = 0 (arg2 未使用)
    \   msg2 = arg1 = FT-STAT-BUF
    \   msg1 = arg0 = FT-NAME-BUF
    \   msg0 = op  = FILE-STAT-OP
    0
    FT-STAT-BUF
    FT-NAME-BUF
    FILE-STAT-OP
    ...
    \ stack: ( r3 r2 r1 r0 )
    \ r3, r2, r1 捨て、r0 残す
    >R DROP DROP DROP R>         \ ( -- r0 )

    \ E-NOENT 判定
    E-NOENT = IF
        $51 FILEMGR-PUTC         \ 'Q' = 成功
    ELSE
        $71 FILEMGR-PUTC         \ 'q' = 失敗 (期待外の r0)
    THEN

    BEGIN AGAIN ;                \ タスク終了暴走防止
...
: OS-START  ( -- )
    ...
    FILEMGR-TEST-START              \ v0.10.2: FILEMGR-TEST 起動 (tid=7) — STAT 試験

...
\ Version: 0.10.2b   Ph.4 Step 5-1B-β: STOR-TEST 末尾に STAT試験を組み込み (仮説α/β切り分け)
\ YSD8800 YUI OS Microkernel
\
\ ★★★ v0.10.2b 追加点 (2026-05-24) ★★★
\ Ph.4 Step 5-1B-β: 仮説α(タスク数増)と仮説β(FILEMGR-TASKループ拡張)の切り分け。
\   - v0.10.2a の FILE-STAT-IMPL / FILE-DISPATCH-STAT 定義 (動作確認済) を保持
\   - FILEMGR-TASK を v0.10.2 と同形に拡張 (DI/SENDER/EI/REORDER/DISPATCH-STAT)
\   - STOR-TEST-TASK の 'P' 出力後に STAT試験を追加 (新規タスクは作らない)
\   - 期待出力: 0A BC123MPQ (成功なら仮説α原因、Pで停止なら仮説β/δ原因)
\
...
: STOR-TEST-TASK  ( -- )
    ...
    \ ----------------------------------------------------------------
    \ v0.10.2b: Step 5-1B-β STAT試験 (存在しないファイル "no.txt" を stat)
    \ TEST-SRC-BUF=$EC00, TEST-DST-BUF=$EE00 は S5 後はもう使わないため
    \ FT-NAME-BUF=TEST-SRC-BUF, FT-STAT-BUF=TEST-DST-BUF として再利用 OK
    \ 期待: r0=E-NOENT($FE01) → 'Q' 出力、不一致なら 'q'
    \ ----------------------------------------------------------------
    \ "no.txt\0" を TEST-SRC-BUF へ
    $6E TEST-SRC-BUF       C!       \ 'n'
    $6F TEST-SRC-BUF 1 +   C!       \ 'o'
    $2E TEST-SRC-BUF 2 +   C!       \ '.'
    $74 TEST-SRC-BUF 3 +   C!       \ 't'
    $78 TEST-SRC-BUF 4 +   C!       \ 'x'
    $74 TEST-SRC-BUF 5 +   C!       \ 't'
    $00 TEST-SRC-BUF 6 +   C!       \ '\0'

    \ FILE_STAT 送信: ( msg3=0 msg2=stat_buf msg1=name msg0=op tid )
    0 TEST-DST-BUF TEST-SRC-BUF FILE-STAT-OP FILEMGR-TID-ADDR @ IPC4-CALL
    \ ( r3 r2 r1 r0 ) ← TOS が r0
    >R DROP DROP DROP R>            \ r0 のみ残す

    E-NOENT = IF
        0 0 $51 UART-PUTC-OP UART-DRV-TID @ IPC4-CALL  \ 'Q' = 成功
    ELSE
        0 0 $71 UART-PUTC-OP UART-DRV-TID @ IPC4-CALL  \ 'q' = 期待外
    THEN
    DROP DROP DROP DROP
...
: FILEMGR-TASK  ( -- )
    ...
    BEGIN
        ...
        FILE-DISPATCH-STAT              \ v0.10.2b: STAT 単体 dispatch
    AGAIN ;

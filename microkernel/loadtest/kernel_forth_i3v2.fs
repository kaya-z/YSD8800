...
\ Version: 0.10.11  v0.10.10 + Step 5-DELETE FILE-DELETE-IMPL 実装＋FILE-DISPATCH $0208 結線
...
\ ============================================================
\ Ph.3.5-I-3 負荷試験ハーネス (試験版 kernel_forth_i3_load.fs 限定・KY38)
\ 設計: yuios_ph3_5_i3_load_design_v1_2.md
\   ECHO-SERVER(tid=1・即REPLY方式) + LOAD-TASK×14(tid2..15)
\   UART出力は UART-PUTC-IMPL 直接MMIO限定(設計§3.3・No.7=(A))
\   POOL-HWM=$50A2 をカーネルASMフックが更新(設計§2.3)
\   V1判定: HWM ≤ $20(32) で枯渇耐性PASS
\ ============================================================
$50A2 CONSTANT I3-POOL-HWM        \ プール占有 high-water mark (ASMフックが更新)
14    CONSTANT I3-NCLIENT         \ LOAD-TASK数(tid 2..15)=14

\ --- I3 直接MMIO PUTC(設計§3.3: IPC経由PUTC不使用・デッドロック回避) ---
: I3-PUTC  ( char -- )  UART-PUTC-IMPL ;

\ --- POOL-HWM を Hxx 形式でUART出力(設計§2.3・V1判定) ---
: I3-HWM-PUTC  ( -- )
    $48 I3-PUTC                    \ 'H'
    I3-POOL-HWM @                  \ ( hwm )
    DUP $0F AND >R                 \ R:下位4bit
    4 RSHIFT $0F AND               \ 上位4bit
    DUP 10 < IF $30 + ELSE $37 + THEN I3-PUTC
    R> DUP 10 < IF $30 + ELSE $37 + THEN I3-PUTC ;

\ --- ECHO-SERVER(tid=1): 即REPLY方式。受信値+1をsenderへ返す ---
\   ( -- ) ループ。スタック収支は1ターンで閉じる(机上トレース済)。
: I3-ECHO-SERVER-TASK  ( -- )
    BEGIN
        IPC4-RECV                  \ ( m3 m2 m1 m0 )
        IPC4-SENDER                \ ( m3 m2 m1 m0 sender )
        >R                         \ ( m3 m2 m1 m0 )  R:sender
        1+                         \ ( m3 m2 m1 r0 )  r0=m0+1
        R>                         \ ( m3 m2 m1 r0 sender )=( r3 r2 r1 r0 tid )
        IPC4-REPLY                 \ ( ) 全消費
    AGAIN ;

\ --- LOAD-TASK(tid 2..15): 自tidをmsg0でtid=1へCALL→reply検証→記号 ---
\   成功(r0==自tid+1)なら 'a'+(tid-2) を直接MMIO出力。
: I3-LOAD-TASK  ( -- )
    TASK-ID                        \ ( mytid )
    DUP 1+ >R                      \ ( mytid )  R:(mytid+1)期待値
    >R                             \ ( )        R:(mytid+1 mytid)
    0 0 0                          \ ( 0 0 0 )=msg3 msg2 msg1
    R>                             \ ( 0 0 0 mytid )=msg0   R:(mytid+1)
    1                              \ ( 0 0 0 mytid 1 )=tid
    IPC4-CALL                      \ ( r3 r2 r1 r0 )  R:(mytid+1)
    NIP NIP NIP                    \ ( r0 )
    R>                             \ ( r0 mytid+1 )
    = IF
        TASK-ID 2 - $61 + I3-PUTC  \ 'a'+(tid-2)
    THEN
    BEGIN TASK-SLEEP AGAIN ;

CODE I3-ECHO-SERVER-ADDR  ( -- addr )
    SUBI X, #2
    LDW  A, #WORD_I3_ECHO_SERVER_TASK
    STW  A, [X]
END-CODE

CODE I3-LOAD-TASK-ADDR  ( -- addr )
    SUBI X, #2
    LDW  A, #WORD_I3_LOAD_TASK
    STW  A, [X]
END-CODE

\ ============================================================
\ V2: プール枯渇耐性試験 (枯渇時パニックしないこと)
\   I3-SINK-TASK(tid=1): RECVせず寝るだけ → 送信分がプールに滞留
\   I3-DRAIN-TASK(tid=2): SINKへ IPC4-SEND を40回連打 → 32で枯渇
\     33個目以降は ERR_IPC_NOSLOT(枯渇無視で返る)。連打後 'Z' 出力=継続(パニックなし)
\   V2合格: 'Z' が出る(枯渇後も実行継続) かつ HWM=32(POOL_SIZE到達)
\ ============================================================
: I3-SINK-TASK  ( -- )
    BEGIN TASK-SLEEP AGAIN ;        \ RECVせず寝続ける(送信分を溜める)

: I3-DRAIN-TASK  ( -- )
    $28                             \ ( count=40 ) 40回SEND(POOL_SIZE=32超)
    BEGIN
        DUP 0 > WHILE
        0 0 0 0 1 IPC4-SEND      \ ( count ) SINK(tid=1)へ固定msg。SENDは5個消費0個返す
        1 -
    REPEAT
    DROP
    $5A I3-PUTC                     \ 'Z' = 40回SEND完了(枯渇後も継続=パニックなし)
    BEGIN TASK-SLEEP AGAIN ;

CODE I3-SINK-TASK-ADDR  ( -- addr )
    SUBI X, #2
    LDW  A, #WORD_I3_SINK_TASK
    STW  A, [X]
END-CODE

CODE I3-DRAIN-TASK-ADDR  ( -- addr )
    SUBI X, #2
    LDW  A, #WORD_I3_DRAIN_TASK
    STW  A, [X]
END-CODE

: OS-START  ( -- )
    \ Ph.3.5-I-3 V2: プール枯渇耐性試験。SINK(tid=1)+DRAIN(tid=2)のみ。
    I3-SINK-TASK-ADDR  TASK-CREATE DROP      \ tid=1 SINK(RECVせず溜める)
    I3-DRAIN-TASK-ADDR TASK-CREATE DROP      \ tid=2 DRAIN(SEND連打)
    BEGIN TASK-SLEEP AGAIN ;

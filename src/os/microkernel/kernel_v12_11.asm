; ============================================================
; YSD8800 マイクロカーネル kernel.asm  v0.12.11
; 純粋アセンブラ版 Track A
;
; v0.12.11: コンテキストスイッチ A/B レジスタ復元欠落の修正【TKT-04】
;   （コメント追記のみ・md5 不変）: 対実装台帳 A-01 対応。_sc_chk に
;   1周検出が状態チェックの「前」でよい理由を追記（_sched_chk との
;   意図的な非対称。詳細は yuios_paired_impl_ledger_v1_0.md L-04）
;   設計書: yuios_ctxsw_abreg_restore_design_v0_2.md
;   欠陥: _sc_found が saved_a/saved_b を復元せず、SYSCALL 保存経路も
;         これらを書かないため、TASK_CREATE の EI 窓で中断されたタスクが
;         _sc_found 経由で再開すると、戻り値が saved_x（呼出元 Forth DSP）
;         に化ける。TKT-01 の T-B で観測された $F87E がこれである。
;   修正: M-1〜M-4 SYSCALL 保存経路4箇所（L653/L1021/L1167/L1764 付近）で
;           saved_a/saved_b に定義済み値(0)を書く
;         M-5 _sc_found（L737-747）で saved_a/saved_b を復元する
;   ABI: システムコールを跨いで A/B は保持されない（caller-saved）。
;        復元経路は中断理由を判別できないため、フィールドは必ず定義済みにする。
;   md5: 599a7f9d1ebf103f81f58450ea1b6491（56,416 B・yuios_road2.bin）
;   検証: W-1/W-2/W-2b/W-3/W-4/W-5/W-7/W-9/W-10 全 PASS
;         W-5 は陰性対照（M-5 抜きビルド）で症状値を再現させたうえで判定
;         ※ _irq_noready の透過性は TKT-00 の裏取りとして W-11 で別途確認済
;            （復元経路は _sched_found / _sc_found / _irq_noready の3つある）
;   ★日付は引継文書に記録が無いため未記載（推測で補わない）★
;
; v0.12.10: UART 受信リングバッファ破損の修正【TKT-02】
;   設計書: yuios_uart_rxring_fix_design_v0_4.md
;   修正: I-03(a) UART_RX_COUNT > 0 を必須条件とする
;         I-03(b) COUNT==0 のとき A = $FFFF（異常センチネル）を返す防御ガード
;   旧 md5: d5c18dd3aa1d4d4a4bc042fb5d6af893
;   ★日付は引継文書に記録が無いため未記載（推測で補わない）★
;
...
_sc_sched:
    ...
    LDW  A, #0
_sc_chk:
    ; --- v0.12.11(対実装台帳L-04): 1周検出は状態チェックの「前」でよい ---
    ; IRQ0側の_sched_chk（1周検出=状態チェックの「後」）とは意図的に非対称。
    ; SYSCALL経路では呼出タスクが既にSLEEPING/WAITINGへ落ちてから走査に
    ; 入るため、自分自身(CUR_TASK)を再度拾ってはならない。よってここで
    ; 先に「1周した=自分自身に戻ってきた」を検出してよい。
    ; IRQ0側と同じ「後」に揃えてはならない（詳細: 対実装台帳L-04参照）。

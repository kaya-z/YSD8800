; ============================================================
; YSD8800 マイクロカーネル kernel.asm  v0.12.7
; 純粋アセンブラ版 Track A
;
...
_msg_copy_loop:
    ...
    ; === [I3-HOOK] プール占有 -1 (試験版限定・KY38) ===
    ; v1.1: RECVスロット(.org $07E0, 224B)超過回避のためJSR化(8B→3B)
    JSR  _i3_dec_inuse
    ; === [/I3-HOOK] ===
...
_init_pool_done:
    ...
    ; === [I3-HOOK] プール占有カウンタ初期化 (試験版限定・KY38) ===
    LDW  A, #0
    STW  A, [$50A0]            ; POOL-INUSE = 0
    STW  A, [$50A2]            ; POOL-HWM   = 0
    ; === [/I3-HOOK] ===
...
_enq_got_slot:
    ...
    ; === [I3-HOOK] プール占有 +1 / HWM 更新 (試験版 kernel_v12_7_i3.asm 限定・KY38) ===
    ;   POOL-INUSE=$50A0, POOL-HWM=$50A2。alloc 確定直後に占有++、HWM=max(HWM,INUSE)
    ;   レジスタ B を破壊するが、この直後 B は #4(shift) で再設定されるため安全
    LDW  B, [$50A0]            ; B = POOL-INUSE
    ADDI B, #1                 ; INUSE++
    STW  B, [$50A0]
    LDW  A, [$50A2]            ; A = POOL-HWM
    CMP  A, B                  ; HWM - INUSE
    BGE  _i3hook_enq_nohwm     ; HWM >= INUSE ならHWM更新不要(ISA2.3はBLE非対応のためBGEで表現)
    STW  B, [$50A2]            ; HWM = INUSE (INUSE > HWM のときのみ)
_i3hook_enq_nohwm:
    LDW  A, [IPC4_WK_IDX]      ; A を idx に復元（後続の SHL A 用）
    ; === [/I3-HOOK] ===
...
; ================================================================
; [I3-HOOK] _i3_dec_inuse : POOL-INUSE($50A0) を -1 する
;   RECV free点からJSRで呼ぶ(RECVスロット$07E0-$08BF 224B超過回避のため外出し)
;   破壊: A。呼び出し元(RECV free点)はこの先 A/B 未使用なので安全
;   試験版 kernel_v12_7_i3.asm 限定・KY38
; ================================================================
_i3_dec_inuse:
    LDW  A, [$50A0]            ; A = POOL-INUSE
    SUBI A, #1                 ; INUSE--
    STW  A, [$50A0]
    RET

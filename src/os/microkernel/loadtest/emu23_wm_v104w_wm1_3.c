// emu23_wm_v104w.c [wm v1.3]: slot watermark + I3 pool HWM + warmup (本番無改変・実験ビルド)
...
/* ===== [wm] Ph.3.5-I-3-T stack watermark 拡張 ===== */
/* 設計: emu23_watermark_design_v1_0.md  本番(emu23_v104.c)無改変・本ファイルは実験ビルド */
...
#define WM_GUARD_PAT  0xA55A      /* OS が _kstart で設置するパターン */
static int      wm_enable   = 0;
static uint64_t wm_max_steps = 2000000ULL;  /* v1.1: 常駐OS打切り上限 (--wm-steps で変更) */
static uint64_t wm_warmup_cycles = 2000ULL; /* v1.3: 起動初期の一過性除外(ウォームアップ) */
/* v1.2: スロット別最深追跡。全体最深では16タスクの使用が混ざり深さ計算不能のため、
 *       128Bスロット(=tid)ごとに最深 SP/X を別々に記録する。*/
...
/* --- [wm] watermark 報告（-q HALT後 / repl q後 に呼ぶ。stderr へ出力し stdout 非汚染）--- */
static void wm_report(void) {
    if (!wm_enable) return;
    fprintf(stderr, "[WATERMARK] ---- Ph.3.5-I-3-T stack watermark (slot-wise) ----\n");
    ...
    for (int t = 0; t < 16; t++) {
        ...
        int depth = (int)(top - wm_callsp_slot[t]) + 2;             /* 使用バイト */
        if (depth < 0) depth = 0;
        if (depth > call_peak) { call_peak = depth; call_peak_tid = t; }
        fprintf(stderr,
            "[WATERMARK]   call tid=%2d: min_sp=$%04X  used=%3d/128 B  %s\n",
    ...
    for (int t = 0; t < 16; t++) {
        ...
        fprintf(stderr,
            "[WATERMARK]   data tid=%2d: min_x =$%04X  used=%3d/128 B  %s\n",
    ...
    /* [I3] プール占有 high-water mark ($50A2) / INUSE ($50A0) を読み V1 判定 */
    {
        uint16_t inuse = (uint16_t)(mem[0x50A0] | (mem[0x50A1] << 8));
        uint16_t hwm   = (uint16_t)(mem[0x50A2] | (mem[0x50A3] << 8));
        fprintf(stderr, "[I3-POOL]   INUSE($50A0)=%u  HWM($50A2)=%u  POOL_SIZE=32\n",
                inuse, hwm);
        if (hwm == 0)
            fprintf(stderr, "[I3-POOL]   V1: HWM=0 (フック未動作か未初期化の疑い)\n");
        else if (hwm <= 32)
            fprintf(stderr, "[I3-POOL]   V1 PASS: HWM=%u <= 32 (プール枯渇せず)\n", hwm);
        else
            fprintf(stderr, "[I3-POOL]   V1 FAIL: HWM=%u > 32 (枯渇の危険)\n", hwm);
    }
}
...
void exec_one(void) {
    ...
    /* --- [wm] stack watermark 追跡（スロット別・範囲ガードで誤検出排除） --- */
    /* v1.3: 起動初期(_kstart の X=$F800 暫定値など)の一過性を除外するため、
       ウォームアップ期間(既定 50000 cycle)経過後から追跡開始。
       カーネル初期化は数千cycleで完了し、実タスク稼働はその後に始まる。 */
    if (wm_enable && cpu.cycle >= wm_warmup_cycles) {
        if (cpu.sp >= WM_CALLSTK_LO && cpu.sp <= WM_CALLSTK_HI) {
            int t = (cpu.sp - WM_CALLSTK_LO) / 0x80;   /* 所属スロット tid */
            if (t >= 0 && t < 16 &&
...
int main(int argc, char **argv) {
    ...
    /* [wm] スロット別 watermark 配列を未観測(0xFFFF)で初期化 */
    ...
    if (argc < 2) {
        ...
        "  - v1.04: DBG printf removed\n"
            ...
            "  -w,--watermark  measure stack high-water mark (stderr)\n"
    ...
    if (quiet_mode) {
        ...
        while (!cpu.halted) {
            exec_one();
            if (wm_enable && ++steps >= wm_max_steps) break;  /* v1.1: 常駐OS打切り */
        }
        fflush(stdout);
        wm_report();   /* [wm] stderr へ watermark 出力（stdout 非汚染）*/
        return 0;
    }
    ...
    printf("YSD8800 ISA2.3 Emulator (emu23 v1.04+wm [I-3-T]) — reset vector = %04x\n", cpu.pc);
    printf("Type 'help' for commands.\n");

    repl();
    wm_report();   /* [wm] 対話モード終了後にも watermark 出力 */
    return 0;
}

...
: OS-START  ( -- )
    \ Ph.3.5-I-3: 既存サーバ群は非起動(設計§2.4)。I3ハーネスのみ。
    \ HWMはASMフックがenqueue毎に$50A2へ記録。V1判定はemu23メモリ直読(C案)。
    I3-ECHO-SERVER-ADDR TASK-CREATE DROP     \ tid=1 ECHO-SERVER
    I3-NCLIENT                                \ ( count=14 )
    BEGIN
        DUP 0 > WHILE                         \ count>0 の間
        I3-LOAD-TASK-ADDR TASK-CREATE DROP    \ tid 2..15
        1 -                                   \ count--
    REPEAT
    DROP
    \ 全クライアント起動完了。MAINは待機(クライアントがCALL→ECHO即REPLY進行)。
    BEGIN TASK-SLEEP AGAIN ;

...
\ Version: 0.10.2e   v0.10.2 オリジナルにさらに近づける (STOR-TEST末尾削除+FILEMGR-PUTC使用)
\ YSD8800 YUI OS Microkernel
\
\ ★★★ v0.10.2e 追加点 (2026-05-24) ★★★
\ v0.10.2d から下記2点をオリジナル v0.10.2 寄りに戻す:
\   1. STOR-TEST 末尾の STAT試験を削除 (オリジナルどおり 'P' のみ)
\   2. FILEMGR-TEST-TASK 内で FILEMGR-PUTC を使用 (オリジナルどおり)
\ バッファアドレスは TEST-SRC-BUF+$80 のまま (FT-NAME-BUF=$EC00 はまだ使わない)
\ 期待動作:
\   - これでもデッドロックするなら FILEMGR-PUTC 使用が原因
\   - 動くなら STOR-TEST 末尾追加 or バッファ位置が原因
\
...
\ --------------------------------------------------------------------
\ FILEMGR-TEST-TASK  ( -- )                                   v0.10.2d
\ 仮説ε検証用。起動直後に FILE_STAT を即送信する。
\ オリジナル v0.10.2 FILEMGR-TEST-TASK と等価な負荷をかける。
\ バッファは TEST-SRC-BUF/DST-BUF と衝突しないように +$80 オフセット使用。
\ --------------------------------------------------------------------
: FILEMGR-TEST-TASK  ( -- )

...
\ Version: 0.10.15  v0.10.14 + PROC-LOAD-ADDR $C000->$D400 (memmap v2.4)
...
\ --------------------------------------------------------------------
: PROC-DISPATCH  ( arg2 arg1 arg0 op tid -- )
    OVER PROC-EXEC-OP = IF DROP PROC-EXEC-IMPL EXIT THEN
    OVER PROC-KILL-OP = IF DROP PROC-KILL-IMPL EXIT THEN
    OVER PROC-WAIT-OP = IF DROP PROC-WAIT-IMPL EXIT THEN
    OVER PROC-LIST-OP = IF DROP PROC-LIST-IMPL EXIT THEN

// ============================================================
//  ysd8800_v4_membus_v0_1.sv   v0.1  (2026-07-12)
//  YSD8800 FPGA V4 : メモリサブシステム統合ラッパー
//                    （MMU + YSD8004割込 + ★YSD8001 UART★）
//
//  設計根拠: v3_5_design_memo_v0_2.md §3.1 / §3.2（V3.5・継承）
//            v3_7_design_memo_v0_2.md §4.2（YSD8004・案B・継承）
//            v4_design_memo_v0_2.md   §4.1（YSD8001・案B・承認済 v1.0）
...
module ysd8800_v4_membus_v0_1 #(
    ...
    // ============================================================
    // (2) MMIOスタブ ★v0.4★ … MMUレジスタ(PTR/MCR) + YSD8004 + ★YSD8001★
    //     ★MMUの【前段】に位置し、アドレス変換を受けない★
    //     YSD8004($FCB2-$FCB5) / YSD8001($FC80-$FC87) は内部で
    //     インスタンス化される（案B）。UART の割込源は内部結線。
    // ============================================================
    ysd8800_mmio_stub_v0_4 u_mmio_stub (
        .clk(cpu_clk), .rst_n(cpu_rst_n),
        .mmio_addr(mmio_addr), .mmio_wdata(mmio_wdata), .mmio_rdata(mmio_rdata),
        .mmio_rd(mmio_rd), .mmio_wr(mmio_wr), .mmio_ready(mmio_ready),
        .ptr_flat_o(ptr_flat), .mmu_en_o(mmu_en),
        // ★V3.7: 割込I/F（STORのみ外部ポートへ素通し）★

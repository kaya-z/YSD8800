...
//                                bit1 FLUSH(★W1T★ self-clearing・読み出しは常に0)
//                                bit7:2 予約(RO・読み出しは常に0)
//            $FF13 = 上位バイト  bit8 HIT_EN (★初版では非実装★・RO 0を返す)
//                                bit15:9 予約(RO・読み出しは常に0)
//          → 出力 ccr_cen_o / ccr_flush_pulse_o を新設。
//          ★段1 時点ではキャッシュ本体が存在しないため、両出力は
//            membus 側で未接続であり、動作には一切影響しない★。
//          これにより G-0(CEN=0 でサイクル完全一致)が成立する。
//      (2) アドレス衝突の実源確認(KY-B10・本日のKY活動):
//            $FF00-$FF0F MMU PTR / $FF10 MMU MCR / ★$FF11 以降は空き★
//            emu23 v2.15 のキャッシュ下地は --mem-latency 等の
//            ★コマンドラインオプション制御でMMIOレジスタを持たない★
//            → CCR の新設に衝突なし(実源照合済)。
//      (3) ロジック変更を伴うため案Xに従いモジュール名も
//          ysd8800_mmio_stub_v0_8 → ★ysd8800_mmio_stub_v0_9★ へ昇格。
//          → 参照元 membus は v0.7 で参照先を繰上げる。
//    ★未実装(意図的)★: HIT_EN は設計書 M-7/D-B5 により初版非実装。
//      $FF14-$FF1B はカウンタ用に予約(デコードせず=未接続応答 $FF)。
//    ------------------------------------------------------------
//    以下 v0.8 までの記録(欠落させない)
...
module ysd8800_mmio_stub_v0_9 (
    ...
    // --- ★v0.9新規: CCR(キャッシュ制御レジスタ)出力★ ---
    //   段1 時点ではキャッシュ本体が無いため未接続でよい。
    //   段2 以降で cache 本体へ供給する。
    output logic        ccr_cen_o,          // CCR bit0 CEN (1=キャッシュ有効)
    output logic        ccr_flush_pulse_o,  // CCR bit1 FLUSH の W1T 1clk パルス
    ...
    // ★v0.9新規: CCR(キャッシュ制御レジスタ) 設計書 §5.1
    //   $FF12=下位 / $FF13=上位。偶数番地=下位は他デバイス(UART $FC80/$FC81)と同型。
    localparam logic [15:0] CCR_LO_ADDR  = 16'hFF12;   // CEN / FLUSH
    localparam logic [15:0] CCR_HI_ADDR  = 16'hFF13;   // HIT_EN(初版非実装)
    ...
    logic hit_ccr_lo, hit_ccr_hi;                                        // ★v0.9★
    assign hit_ptr = (mmio_addr >= MMU_PTR_BASE) && (mmio_addr <= MMU_PTR_LAST);
    assign hit_mcr = (mmio_addr == MMU_MCR_ADDR);
    assign hit_ccr_lo = (mmio_addr == CCR_LO_ADDR);                      // ★v0.9★
    assign hit_ccr_hi = (mmio_addr == CCR_HI_ADDR);                      // ★v0.9★
    ...
    // ★v0.9新規: CCR(キャッシュ制御レジスタ) 設計書 §5.1
    // ------------------------------------------------------------
    //   $FF12 bit0 CEN   : R/W・リセット値 0 (disable) → G-0 の前提
    //   $FF12 bit1 FLUSH : ★W1T(write-1-to-trigger, self-clearing)★
    //                      1clk パルスを出すのみ。★保持しない★ので
    //                      読み出しは常に 0 (起動中も 0)。W1S ではない。
    //   $FF12 bit7:2     : 予約 RO 0
    //   $FF13 bit8 HIT_EN: 初版非実装(M-7/D-B5)。★書いても保持せず 0 を返す★
    //   $FF13 bit15:9    : 予約 RO 0
    logic ccr_cen_r;
    logic ccr_flush_pulse_r;

    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ccr_cen_r         <= 1'b0;   // ★リセット値=disable(決定済)★
            ccr_flush_pulse_r <= 1'b0;
        end else begin
            // W1T: 書込のあった1clkだけ立てる。次clkで必ず落ちる。
            ccr_flush_pulse_r <= (mmio_wr && hit_ccr_lo) ? mmio_wdata[1] : 1'b0;
            if (mmio_wr && hit_ccr_lo) begin
                ccr_cen_r <= mmio_wdata[0];
            end
            // $FF13 は書込を受理しない(HIT_EN 初版非実装)。
        end
    end

    assign ccr_cen_o         = ccr_cen_r;
    assign ccr_flush_pulse_o = ccr_flush_pulse_r;
    ...
    always_comb begin
        ...
        // ★v0.9: CCR。FLUSH(bit1)は W1T のため常に0。予約bitも0。
        else if (hit_ccr_lo) mmio_rdata = {7'b0, ccr_cen_r};
        // ★v0.9: HIT_EN(bit8)は初版非実装のため常に0。予約bitも0。
        else if (hit_ccr_hi) mmio_rdata = 8'h00;

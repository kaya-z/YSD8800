// ============================================================
//  ysd8800_v5_membus_v0_7.sv   v0.7  (2026-08-30 ②-B 段1 CCR追従)
//
//    ★v0.7 変更点(工程②-B 段1・段2)★:
//      【段1】
//      (1) u_mmio_stub の参照を ysd8800_mmio_stub_v0_8 → ★v0_9★ へ追従
//          (CCR $FF12/$FF13 追加版)。
//      (2) CCR 出力 ccr_cen / ccr_flush_pulse を受ける信号を新設。
//      【段2】★ここからロジック構成の変更を伴う★
//      (3) ★キャッシュ段 ysd8800_cache_v0_1 を新設・挿入★。
//          配置は MMU 出力と CDC ブリッジの間(cpu_clk 側・設計書 §1.2 (A)案)。
//      (4) u_cdc_bridge の CPU 側接続を MMU 直結 → キャッシュ段下流へ付替。
//      (5) ★M-10 ①★ psram_ctrl.burst_len を定数 PS_BLEN_W'(1) から
//          ★キャッシュ駆動 cache_burst_len★ へ変更（段2 は常に 1 を出力）。
//      (6) ★M-10 ②★ psram_ctrl.beat_valid を未接続から
//          ★キャッシュへ直結★（cdc_bridge を経由しない）。段2 では未使用。
//      (7) localparam PS_BLEN_W の宣言位置を (5)節 → (3.5)節の前へ移設
//          （使用前に宣言する必要があるため。値・意味は不変）。
//      ★段2 における動作不変の根拠★:
//        キャッシュ段は ★always_ff を1つも持たない純組合せ★であり、
//        burst_len は ②-A の PS_BLEN_W'(1) と★ビット単位で同一★。
//        よって CPU⇔PSRAM の波形は v0.6 と完全一致する。
//      期待される効果: ★システムレベルで効果ゼロ(G-0 サイクル完全一致)★
//      設計根拠: v11_cache_core_design_v0_4.md §1.2 / §5 / §7.1 / §9 段1・段2
//    ------------------------------------------------------------
//    以下 v0.6 までの記録(欠落させない)
...
module ysd8800_v5_membus_v0_1 #(
    ...
    // ============================================================
    // ★v0.7(②-B 段1): CCR 出力信号。段1 では受けるだけで未使用。
    //   段2 でキャッシュ本体へ接続する。
    /* verilator lint_off UNUSED */
    logic ccr_cen;
    logic ccr_flush_pulse;
    /* verilator lint_on UNUSED */

    // ★v0.7(②-B 段1): 参照を v0_8 → ysd8800_mmio_stub_v0_9 へ追従(CCR追加)★
    //   ccr_cen / ccr_flush_pulse は段1 時点では ★意図的に未使用★。
    //   キャッシュ本体は段2 で追加する。段1 では動作不変=G-0 成立が目的。
    ysd8800_mmio_stub_v0_9 u_mmio_stub (
        ...
        // ★v0.7新規: CCR 出力。段2 でキャッシュ本体へ供給する★
        .ccr_cen_o        (ccr_cen),
        .ccr_flush_pulse_o(ccr_flush_pulse),
    ...
    // ★v0.7: PS_BLEN_W はキャッシュ段でも使うため、
    //   ②-A で (5) 節にあった宣言を ★キャッシュ段の前へ移設★した。
    //   （値・意味は不変。使用前に宣言する必要があるため）
    localparam int PS_BLEN_W = $clog2(32) + 1;   // ★v0.6追加: =6★

    // ============================================================
    // ★(3.5) キャッシュ段 v0.1 … 【②-B 段2 で新設】★
    //     配置: MMU 出力(物理アドレス)と CDC ブリッジの間・cpu_clk 側。
    //           設計書 §1.2「(A) 側配置」。ヒット時に CDC を越えない
    //           ことが利得の源泉であり、選択の余地はない。
    //
    //     ★段2 では CEN によらず純組合せバイパス★であり、
    //     キャッシュ段は ★always_ff を1つも持たない★。
    //     よって CPU⇔ブリッジ間に遅延段は生じず、
    //     G-0(サイクル完全一致)が構造的に保証される。
    //
    //     burst_len は 1 固定で、②-A の PS_BLEN_W'(1) と
    //     ★ビット単位で同一★。beat_valid / psram_rdata は
    //     ★cdc_bridge を経由せず psram_ctrl から直結★する(M-10 ②③)。
    //     段2 では受けるだけで使用しない。
    // ============================================================
    logic [PHYS_AW-1:0] cbr_addr;
    logic [7:0]         cbr_wdata, cbr_rdata;
    logic               cbr_rd, cbr_wr, cbr_ready;
    logic [PS_BLEN_W-1:0] cache_burst_len;
    logic                 ps_beat_valid;

    ysd8800_cache_v0_1 #(
        .PHYS_AW(PHYS_AW), .BLEN_W(PS_BLEN_W), .LINE_SIZE(32)
    ) u_cache (
        .cpu_clk(cpu_clk), .cpu_rst_n(cpu_rst_n),
        // CCR（段2 では未使用・段3 で経路選択に使う）
        .ccr_cen_i        (ccr_cen),
        .ccr_flush_pulse_i(ccr_flush_pulse),
        // CPU 側（MMU 出力）
        .cpu_phys_addr_i(phys_addr),
        .cpu_wdata_i    (ram_wdata),
        .cpu_rdata_o    (ram_rdata),
        .cpu_rd_i       (ram_rd),
        .cpu_wr_i       (ram_wr),
        .cpu_ready_o    (ram_ready),
        // ブリッジ側
        .br_phys_addr_o (cbr_addr),
        .br_wdata_o     (cbr_wdata),
        .br_rdata_i     (cbr_rdata),
        .br_rd_o        (cbr_rd),
        .br_wr_o        (cbr_wr),
        .br_ready_i     (cbr_ready),
        // psram_ctrl 直結（M-10 ①②③）
        .burst_len_o    (cache_burst_len),
        .psram_clk      (psram_clk),
        .psram_rst_n    (psram_rst_n),
        .beat_valid_i   (ps_beat_valid),
        .psram_rdata_i  (psram_rdata_w)
    );
    ...
    ysd8800_cdc_bridge_v0_4 #(.PHYS_AW(PHYS_AW)) u_cdc_bridge (   // ★v0.5: v0_4追従★
        .cpu_clk(cpu_clk), .cpu_rst_n(cpu_rst_n),
        // ★v0.7: 接続元を MMU 直結 → ★キャッシュ段の下流ポート★へ変更。
        //   段2 はキャッシュが純組合せバイパスのため、
        //   ここに現れる波形は v0.6 と ★ビット単位・サイクル単位で同一★。
        .cpu_phys_addr(cbr_addr),           // ←旧: phys_addr
        .cpu_mem_wdata(cbr_wdata),          // ←旧: ram_wdata
        .cpu_mem_rdata(cbr_rdata), .cpu_mem_rd(cbr_rd),   // ←旧: ram_rdata / ram_rd
        .cpu_mem_wr(cbr_wr), .cpu_mem_ready(cbr_ready),   // ←旧: ram_wr / ram_ready
    ...
    // ============================================================
    // ★v0.7: localparam PS_BLEN_W の宣言は (3.5) キャッシュ段の前へ移設済み★
    ...
    ysd8800_psram_ctrl_v0_3 #(
        ...
        .burst_len (cache_burst_len),  // ★v0.7(M-10①): キャッシュが駆動。段2 は常に1★
        .beat_valid(ps_beat_valid),    // ★v0.7(M-10②): キャッシュへ直結(CDC非経由)★

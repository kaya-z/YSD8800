; ================================================================
;  v5t_ack.asm   v0.2  (2026-07-15)
;  ★t2_ack.asm v0.1 からの派生（FPGA-TB専用）★
;  変更点: START直後に SP初期化 1命令追加（LDW SP,#$FC7E）
;          理由=CPU-RTLはreset時SP=$0000。emu23のSP=$fc7e暗黙初期化に
;          依存しないよう、起動コードでSPを明示設定する（MC6809/OS-9作法）。
;          値$FC7E=emu23初期値に一致（回帰CNT=30の前提維持）＋MMIO境界$FC80未満で安全。
...
START:
    ...
    ; ★SP初期化（v5t_ack派生の追加点）★
    ;   CPU-RTLはreset時SP=$0000のため、割込push前に必ず設定する。
    ;   位置=DI直後・EI(割込許可)より前・起動シーケンスで1回だけ（KY62）。

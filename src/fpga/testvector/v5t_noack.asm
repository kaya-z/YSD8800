; ================================================================
;  v5t_noack.asm   v0.1  (2026-07-15 CHAT93 派生)  ★ACK削除版★
...
START:
    ...
    ; ★SP初期化（v5t_ack と同一・KY62位置）★
    LDW  SP, #$FC7E         ; reset時SP=$0000対策。$FC80境界未満・emu23初期値一致
...
TIMER_ISR:
    ...
    ;--- ★★T6: ACK を書かない（対照実験・再武装しない）★★ ---
    ;   t2_ack.asm から ACK 2命令を意図的に削除。
    ;   → irq_timer_o(レベル)がclearされず、2回目の再武装が起きない。
    ;   → 期待: CNT=1 で頭打ち（周期発火しない）。
    ;   T4(CNT>=2)との対照で「差分がACKである」ことを機械的に立証。
    ;-------------------------------------------

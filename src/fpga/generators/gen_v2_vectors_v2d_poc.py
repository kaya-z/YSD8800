...
def main():
    ...
    # --- golden テキスト ---
    with open(f'{OUTDIR}/golden_v2d.txt','w') as f:
        f.write("# V2-d golden(emu23 v1.09). id grp PC SP F A B X wantF\n")
        for r in rows:
    ...
    #   ★V2-d拡張: 6word目にSP追加。SP突合の可否はTB側でgrp判定(stk/subのみ突合)。
    #   既存64(leg/br/mem)もSP値は出力するがTBで突合対象外(HANDOVER §2 Q4)。
    with open(f'{OUTDIR}/expected_v2d.hex','w') as f:
        for r in rows:
            (vid,grp,pc,sp,fl,a,b,x,wf) = r
            f.write(f"{a:04X}\n{b:04X}\n{x:04X}\n{fl:04X}\n{pc:04X}\n{sp:04X}\n")

    # --- veclist: id grp (TBがSP突合対象grp[stk/sub]を判定するのに使用) ---
    with open(f'{OUTDIR}/veclist_v2d.txt','w') as f:
        for r in rows:
            f.write(f"{r[0]} {r[1]}\n")

    # --- SP突合対象(stk/sub)の件数レポート ---
    n_sp = sum(1 for r in rows if r[1] in ('stk','sub'))
    print('-'*52)
    print(f"golden written: {OUTDIR}/golden_v2d.txt ({len(rows)} vectors)")
    print(f"expected hex  : {OUTDIR}/expected_v2d.hex ({len(rows)*6} words)")
    print(f"SP突合対象(stk/sub): {n_sp} vectors / 全{len(rows)}")

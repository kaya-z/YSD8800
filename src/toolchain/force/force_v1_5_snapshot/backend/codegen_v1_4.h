...
typedef struct {
    target_t    tgt;
    prim_table_t ptbl;
    FILE        *out;        /* 最終出力ファイル (.asm) */
    /* v1.4: VARIABLE データ部とコード部を分離出力するためのバッファ
     *   data_section: VAR_xxx/VAL_xxx の DW 等データ部 (DATA-START側)
     *   code_section: 通常コード (CODE-START側、getter ワード本体含む)
     *   最終的に out へ「ヘッダ → code_section → data_section」の順で出力。
     *   これにより hasm23 の .org 後退による重ね書き問題を回避する。
     */
    FILE        *data_section;
    FILE        *code_section;

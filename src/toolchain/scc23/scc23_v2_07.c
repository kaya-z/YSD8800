/* scc23_v2_07.c - Small C Compiler for YSD8800 ISA2.3
 * Version: 2.07 (2026-08-13)
 *
 * v2.07変更点（設計書 scc23_v2_07_fmul_rounding_design_v0_2.md
 *               ・レビュー指摘書 v1.0（C=0/M=0/N=2/E=1・承認判定）反映済
 *               ・ベース=scc23_v2_06.c）:
 *   FMUL 丸め方向不一致の修正（案A: コンパイル時をランタイムに合わせる）。
 *   [発見経緯] C-2（Q8.8ヘルパ）完了後の申し送り §7.5「ランタイム _fmul/_fdiv の
 *              丸めが fold_binop_val と一致するか」の短時間調査で発見。
 *   [バグ機序] 同一の意味論が2箇所に実装され、丸め方向が食い違っていた:
 *              ・fold_binop_val（コンパイル時畳み込み）= (int16_t)(tmp >> 8)
 *                → 算術右シフト＝floor（負の無限大方向）
 *              ・_fmul（実行時ランタイム）= 両オペランドを NOT+ADDI#1 で絶対値化し
 *                符号フラグ(_C_DIV_A)を XOR 管理、非負でシフト加算、末尾で符号適用
 *                → ゼロ方向（truncate）
 *              積が負かつ 256 の倍数でないとき 1 LSB（1/256）ずれる。
 *   [実発現] 「潜在」ではなく現行 v2.06 で既に発現していた（設計書 §1.6.1）:
 *              0.1 * -0.1        → 畳み込まれず _fmul で -2
 *              (0.0 - 0.1) * 0.1 → 定数畳み込みされて -3   ★同じ値なのに食い違う★
 *            単項マイナスは 0-x に実行時展開され畳み込まれないが、減算で負値を
 *            作れば畳み込み経路に入るため。
 *   [修正/案A] fold_binop_val の FMUL を絶対値方式（絶対値化→>>8→符号付与）に変更し
 *              ランタイム _fmul と一致させる。コンパイル時畳み込みは「実行時と同じ
 *              結果を先に計算する」最適化に過ぎず、★最適化が挙動を変えてはならない★
 *              ため、実行時側を正とする。FMUL と FDIV の丸めがゼロ方向で統一される。
 *   [等価性] レビュアにより境界値324＋ランダム20万＝計 200,324 ケースで
 *            修正案とランタイムの完全一致を検証済（不一致 0 件）。
 *   [不変] FDIV は元から両者ゼロ方向で一致（修正対象外）。正値 FMUL の結果も不変。
 *          オーバーフロー warning の条件・位置も不変。
 *   [影響] 既存ソース6本（dhry_all_ansi2/fib/sd_sample/v6_sdread_test_poc/
 *          v8_catls_demo_poc/dhry_timer）は float ランタイム参照 0 件＝実害ゼロ。
 *   [波及] tools/q88.py（fold_binop_val の再現実装）は v1.1 へ同期修正が必要。
 *          self-test グループC の期待値 C2 -3→-2 / C3 -13→-12。
 *   [別件] _cc_div/_cc_mod に符号処理が無い問題を調査中に発見（設計書 §7.4）。
 *          -7/2→0（正 -3）、7/-2→16381（16382回ループ）、_cc_mod は
 *          ゼロ除算ガードも無く x%0 が無限ループ。★1変更1検証の原則により
 *          本 v2.07 では修正せず別チケットで起票する★。_cc_mul は問題なし。
 *   [検証] V1(不一致解消・値と経路のAND) / V2(Dhrystone絶対ゲート 826/48405/P:20/21846B) /
 *          V3(-O1 cycles 47795) / V4(既存ソース回帰) / V5(正値FMUL不変) /
 *          V6(C-1マトリクス24/24) / V7(q88.py 同期修正と再検証)
 *
 * --- 以下、v2.06の変更点（保持）---
 *
 * (v2.06) scc23_v2_06.c - Small C Compiler for YSD8800 ISA2.3
 * (v2.06) Version: 2.06 (2026-08-13)
 *
 * v2.06変更点（設計書 scc23_v2_06_C3_undecl_call_error_design_v0_2.md
 *               ・レビュー指摘書 v1.0（C=0/M=1/N=2/E=1）反映 承認済
 *               ・ベース=scc23_v2_05.c）:
 *   Phase 1 / C-3追補：未定義関数呼び出しのエラー化（案G）。
 *   [発見経緯] Phase 1 / C-1（マトリクステスト整備）作業中に発見。24ケースの期待値を
 *              事前確定させ実測と突合したところ m1_f_add_c_n / m1_f_div_c_n
 *              （float×右辺call×プロトタイプなし）が期待 error_stop に対し実測
 *              warning_only で MISMATCH。v2.05 案D の網羅性の穴が判明した。
 *   [穴の機序] v2.05 の案D は binop_promote の float 経路（lf||rf が真）に error 判定を
 *              置いていた。しかし「両辺とも未定義 call」の場合、両方 T_INT フォールバック
 *              となり lf=0,rf=0 で int 経路へ入りそのまま return するため、案D の判定に
 *              到達しない。結果 Q8.8 float 演算が _cc_mul/_cc_div（int演算）で生成され
 *              値が壊れる。例: Q8.8 の 2.5/4.0 を int 除算すると 0x0280/0x0400=0
 *              （正解 0.625=0x00A0）。加減算は Q8.8 と int がビット演算として同一のため
 *              偶然正しく、乗除算のみ確実に壊れる。
 *   [穴の範囲] 代入だけでなく return 文・関数引数でも同じ穴（probe1/probe2 で実証、
 *              いずれも _cc_div 生成・_fdiv 0件）。ゆえに parse_assign 限定修正（案E）は
 *              取りこぼす。
 *   [修正/案G] parse_primary の未定義関数呼び出し検出を warning → error に格上げ。
 *              型不明な call を式に持ち込ませないことで、float 文脈の種類・複合式の深さ・
 *              両辺の組合せを問わず原理的に値破壊を阻止する（fail-safe）。
 *   [仕様変更] 案G は実質「前方宣言必須化（C99 準拠）」である。後方定義関数を
 *              プロトタイプなしで前方から呼ぶ書き方は新たに error となる。
 *              組込関数（putchar/getchar/puts/printf/sprintf/strcpy/strcmp/memcpy/strlen）は
 *              compile 冒頭で SC_FUNC 事前登録済のため影響なし。
 *              ★今後 C ソースを新規に書く際は、関数を使用前に宣言すること★
 *   [後方互換] 既存 C ソース6本（dhry_all_ansi2.c/fib.c/sd_sample.c/v6_sdread_test_poc.c/
 *              v8_catls_demo_poc.c/dhry_timer.c）で undeclared warning 0 件を実測済。
 *              実害ゼロ・Dhrystone 絶対ゲート無傷。
 *   [案D残置] v2.05 の binop_promote 内 error 判定は多重防御として残置。案G により実質
 *              デッドコードとなるが、将来案G が緩和された場合の保険とする。
 *   [検証] V1(穴の閉塞) / V2(Dhrystone絶対ゲート 826/48405/P:20/21846B) /
 *          V3(-O1 cycles 47795) / V4(既存ソース回帰) / V5(C-3元PoC case1〜6) /
 *          V6(C-1マトリクス24ケース) / V7(案G・案D の error 二重出力実測)
 *
 * --- 以下、v2.05の変更点（保持）---
 *
 * (v2.05) scc23_v2_05.c - Small C Compiler for YSD8800 ISA2.3
 * (v2.05) Version: 2.05 (2026-08-02)
 *
 * v2.05変更点（設計書 scc23_v2_05_C3_forward_decl_design_v0_3.md ・レビュー
 *               scc23_v2_05_C3_forward_decl_design_v0_2_review_closure_v1_0.md
 *               全3項目クローズ+CONF-1反映 承認済・ベース=scc23_v2_04.c）:
 *   Phase 1 / C-3 対応：前方未定義 float 関数呼び出しの値破壊阻止（fail-safe error 化）。
 *   [バグ機序] 2段構造:
 *              1段目: parse_primary (L1877-1884) の call 型伝播で、SC_FUNC 未登録の
 *                     呼び出しを T_INT にフォールバック（従来動作・int文脈では正当）。
 *              2段目: binop_promote (L1186-1218) の float 経路で wrap_i2f が
 *                     T_INT ノードに SIR_I2F を挿入 → emit で LDW B,#8 / SHL A,B が
 *                     生成され、既に Q8.8 の戻り値を 8bit 左シフトして値破壊。
 *   [影響] Dhrystone は float 非使用のため実害なし・絶対ゲート不変。
 *          将来 Q8.8 float を使うユーザランドコードで実害顕在化する。
 *   [修正] 案D＋案C 併用（値破壊の確実阻止＋早期通知）。
 *          案D: is_undecl_call() 判定を新設し binop_promote の float 経路直前で
 *               error 停止。ボトムアップ評価により未定義 SIR_CALL が float 演算に
 *               最初に触れる binop_promote で必ず捕捉され複合式でも漏れない。
 *          案C: parse_primary の T_INT フォールバック分岐で warning 出力
 *               （プロトタイプ宣言追加を促す早期通知・int 文脈では警告のみ）。
 *   [不変] プロトタイプ宣言済み関数呼び出し・int 文脈のみでの未定義関数呼び出しの
 *          コード生成は完全に v2.04 と同一。Dhrystone -O0-strict byte-exact。
 *   [検証] V1: PoC (case1/case2) / V2: Dhrystone絶対ゲート維持(826/48405/P:20/21846B) /
 *          V3: -O1 cycles 47795 維持 / V4: 既存 float テスト回帰 /
 *          V5: 追加 PoC (case3〜case6 判定精度確認)
 *
 * --- 以下、v2.04の変更点（保持）---
 *
 * (v2.04) scc23_v2_04.c - Small C Compiler for YSD8800 ISA2.3
 * (v2.04) Version: 2.04 (2026-07-26)
 *
 * v2.04変更点（設計書 scc23_v2_04_char_load_fix_design_v1_0.md §3.3
 *               ・レビュー scc23_v2_04_char_load_fix_design_v1_0_review_v1_0.md 承認
 *               ・ベース=scc23_v2_03.c）:
 *   char ロード幅不一致（潜在バグ）の是正。
 *   [バグ] emit_expr() SIR_SYM スカラ変数参照の非グローバル分岐において、
 *          is_char を算出しながら参照しておらず、ローカル/パラメータの
 *          char を LDW（2バイト）で読んでいた。store 側は
 *          emit_store_b_at_a() が STB（1バイト）を出すため、
 *          「1バイト書込 / 2バイト読出」の非対称が発生。
 *          読み出した上位バイトが未定義となり、値が RAM 初期値に依存していた。
 *   [影響] emu23 は mem[] がグローバル配列＝ゼロ初期化のため偶然通過。
 *          実 FPGA は PSRAM 電源投入時内容が不定のため非決定的動作となる。
 *          FPGA V8-D Dhrystone RTL 完走調査（CHAT119）で発見。
 *   [修正] char は LDB（1バイト・ゼロ拡張）で読み、store と対称化。
 *          LDB は [imm16]/[X] の2形式のみで [X+off] 形式が無いため
 *          （hasm23_v1_04.c L105-108）、X を一時的にずらして読み直後に戻す。
 *          off==0 は LDB A,[X] の1命令、off!=0 は3命令。
 *   [不変] グローバル char（既に LDB 使用）、char ポインタ経由 *p、
 *          配列/構造体のアドレス値ロード（16bit=LDW 正当）は変更なし。
 *   [検証] V1: emu23 Dhrystone 完走 / V2: 生成asm確認 /
 *          V3: RTL 4初期化パターン($00/$FF/$AA/$55) / V4: 回帰
 *
 ...
 * バージョン
 * ============================================================ */
#define SCC_VERSION   "2.07"
#define SCC_DATE      "2026-08-13"
    ...
    if (type==T_FLOAT) {
        ...
        case SIR_FMUL: {
            ...
            /* [v2.07] 丸め方向をランタイム _fmul に一致させる。
               旧: (int16_t)(tmp >> 8) は算術右シフト＝floor（負の無限大方向）。
               ランタイム _fmul は両オペランドを NOT+ADDI#1 で絶対値化し、
               符号フラグ(_C_DIV_A)を XOR 管理して非負でシフト加算、末尾で
               符号を適用する方式＝ゼロ方向丸め。両者は積が負かつ 256 の
               倍数でないとき 1 LSB ずれていた（C-2 §7.5 調査で発覚）。
               例: (0.0-0.1)*0.1 は畳み込みで -3、0.1*-0.1 は実行時で -2。
               コンパイル時畳み込みは「実行時と同じ結果を先に計算する」最適化で
               あり挙動を変えてはならないため、実行時側に合わせる。
               これにより FMUL と FDIV の丸めがゼロ方向で統一される。
               tmp は |a|,|b| <= 32768 より |tmp| <= 2^30 であり、
               -tmp が INT32_MIN 未定義動作に到達することはない。
               (設計書 scc23_v2_07_fmul_rounding_design_v0_2.md §2.5) */
            {
 ...
 * 判定条件: n->op == SIR_CALL かつ sym_find(n->fname) が SC_FUNC を返さない。
 * ボトムアップ評価により、未定義 SIR_CALL が float 演算に最初に触れる
 * binop_promote 呼び出しで必ず捕捉され、外側複合式へ値破壊が伝播する前に停止する。
 * (設計書 scc23_v2_05_C3_forward_decl_design_v0_3.md §2.4/§2.4.1) */
...
static sir_node_t *binop_promote(sir_op_t int_op, sir_node_t *l, sir_node_t *r) {
     ...
     * (設計書 scc23_v2_05_C3_forward_decl_design_v0_3.md §2.4) */
    if (is_undecl_call(l)) {
               ...
               未定義(前方宣言なし)の場合のみT_INTフォールバック。 */
            {
                ...
                if (cfs && cfs->sclass==SC_FUNC) {
                    ...
                    /* [v2.06 C-3追補/案G] 未定義関数呼び出しを error 化。
                       ...
                       落ち（lf=0,rf=0）、案D の float 文脈判定に到達しないまま
                       _cc_mul/_cc_div が生成され Q8.8 の値が壊れる穴があった
                       （C-1 マトリクステスト整備中に発見）。
                       穴は代入だけでなく return 文・関数引数にも存在するため、
                       型不明な call を式に持ち込ませない fail-safe とする。
                       ★仕様変更: 実質「前方宣言必須化（C99 準拠）」。後方定義関数を
                         プロトタイプなしで前方から呼ぶ書き方は error となる。
                         組込関数は compile 冒頭で SC_FUNC 事前登録済のため影響なし。
                       (設計書 scc23_v2_06_C3_undecl_call_error_design_v0_2.md §2.4) */
                    error("call to undeclared function '%s': "
                          "add a prototype declaration before use "
                          "(implicit int return is not assumed)",
                          name);
                    /* error() は生成を止めないため、後段解析の NULL 参照等を避ける
                       目的で型は入れておく（設計書 §2.4）。 */
    ...
    case SIR_SYM: {
        ...
        if (s->is_array||s->type==T_STRUCT||s->type==T_UNION) {
            ...
            if (s->sclass==SC_GLOBAL) {
                ...
                int off=real_offset(s);
                /* ── v2.04 修正: char ロード幅不一致の是正 ──────────────
                 * 【バグ】v2.03 以前は is_char を算出しながら本分岐で参照せず、
                 *   ローカル/パラメータの char を LDW(2バイト) で読んでいた。
                 *   store 側は emit_store_b_at_a() が STB(1バイト) を出すため、
                 *   「1バイト書込 / 2バイト読出」の非対称が生じ、上位バイトが
                 *   未定義（実FPGAのPSRAM電源投入時内容は不定）になっていた。
                 * 【修正】char は LDB(1バイト・ゼロ拡張) で読み、store と対称化する。
                 *   LDB は [imm16] と [X] の2形式しか無く [X+off] 形式が
                 *   存在しない (hasm23_v1_04.c L105-108) ため、X を一時的に
                 *   off だけずらして読み、直後に必ず戻す。
                 *   X はフレームポインタだが、割込ハンドラは X を値保存/復元
                 *   以外に使わないため一時変更中の割込受理も安全
                 *   (startup_harness23 の全ハンドラを実源確認済)。
                 * 【副作用】A とフラグ Z/N のみ。scc23 は条件分岐前に必ず
                 *   CMPI A,#0 を明示出力するためフラグ差異の実害は無い。
                 * 【表記】SUBI に渡す -off は正の値になるため %d 表記とする。
                 *   既存 LDW 経路の [X + #$%04X]（符号なし16bit hex）とは
                 *   表記が異なるが、アセンブル結果は等価。
                 * 設計書: scc23_v2_04_char_load_fix_design_v1_0.md §3.3
                 * ────────────────────────────────────────────────── */
                if (is_char) {
                    if (off==0) {
                        emit("    LDB  A, [X]");

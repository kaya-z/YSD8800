...
// Version: 1.22
// v1.22: -q オプション復活（HALT まで無制限実行、UART 出力のみ stdout、診断は stderr）
// v1.21: スタック初期値を ISA2.2 仕様に準拠（$FFFE -> $FC7E、stack area $F800-$FC7F）
// v1.20: YSD8001 UART サポート（-i オプション）、MMU 対応（--mmu）、quiet モード（-q）
// v1.10: MMU + UART 基本機能
// v1.01: UART_TX write support
// v1.00: ISA2.2 commands from emu21
//
// ISA2.1からの追加点 (ISA2.2):
//   ビット演算命令: AND/ANDI/OR/ORI/XOR/XORI/NOT/SHL/SHR/SAR
//
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        fprintf(stderr,
            "usage: emu prog.bin [prog.dbg [prog.sym]] [-n N] [-b ADDR] [-q]\n"

// emu2.c - ISA2.1 Emulator (Full Implementation)
// Modified by Grok on 2026-03-18
// Added support for ISA2.1 EXT prefix 0x1F + PUSH/POP/LDB/STB
// Fixed HALT double-output bug (HALT後にtimer IRQが発火していた問題を完全解決)
...
void exec_one(void) {
    if (cpu.halted) {
        return;  // ★ HALT後は即return（timer IRQも完全にブロック）
    }

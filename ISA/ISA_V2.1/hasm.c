...
int main(int argc, char **argv) {
    ...
    while (fgets(linebuf, sizeof(linebuf), fp)) {
        ...
        if (strcmp(mnem, "LDB") == 0 || strcmp(mnem, "STB") == 0) {
            ...
            int reg = parse_reg(ins);
            if (reg >= 0) {
                // [X] など → 即値なし → 1F + subcode の 2バイト
                pc += 2;
            } else {
                // [addr] → 即値16付き → 1F + subcode + imm16 の 4バイト
                pc += 4;
            }
            continue;
        }
...
//	
    ...
    printf("Assembled: %s  (v2.12: ISA2.1 EXT + LDB/STB 2-op + LDW/STW + EQU/DW/DB)\n", outbin);
    return 0;
}

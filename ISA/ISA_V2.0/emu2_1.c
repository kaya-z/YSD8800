...
void exec_one(void) {
    ...
    if (cpu.irq_pending >= 0 && (cpu.flags & FL_IE)) {
      ...
      uint16_t vec = rd16(cpu.irq_pending * 2);
      cpu.pc = vec;
    ...
    case 0x05: { /* SYSCALL imm16 */
        ...
        // Soft IRQとして処理 (IRQ3を使用する例)
        cpu.irq_pending = 3;
        break;
    }
    case 0x06: /* BRK */
        printf("** DEBUG TRAP (BRK) at %04x **\n", pc0);
        cpu.halted = 1;
        break;
    ...
    case 0x23: { /* STW rS, [imm16] */
        uint8_t rb = mem[cpu.pc++];
        uint16_t addr = fetch16(cpu.pc); cpu.pc += 2;
        // STW rS, ... なのでレジスタ指定は下位4bit(rSフィールド)と解釈
        uint16_t *rs = get_reg_ptr(rb & 0x0F); 
        if (rs) {
            wr16(addr, *rs);
        }
    ...
    if ((cpu.cycle % 100) == 0 && cpu.irq_pending < 0) {
      cpu.irq_pending = 0; // Timer Interrupt simulation
    }

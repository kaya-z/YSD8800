...
static void parse_toplevel(parser_t *p) {
    ...
    while ((tok = lexer_next(p->lex))->type != TOK_EOF) {
        ...
        if (strcmp(up, "CODE") == 0) {
             ...
             *
             * lexer_next()がワード名を読んだ後の状態:
             *   a) pushback >= 0 かつ非改行 → まだ同一行にデータがある → 行末まで読み飛ばす
             *   b) pushback == '
' → 改行は消費済み → 読み飛ばし不要
             *   c) pushback == -1  → lexerがすでに改行を消費済み → 読み飛ばし不要
             */
            char linebuf[256];
            int  li = 0;
            int  c;
            /* Step1: CODE name行の残りを読み飛ばす
             * lexer.cの修正によりワード名読み後の区切り文字がpushbackに入る:
             *   - pushback=='
': ワード名直後が改行 → 次行先頭にいる → 読み飛ばし不要
             *   - pushback==' '等(非改行): 同一行に残りがある → 行末まで読み飛ばす
             */
            if (p->lex->pushback == '\n') {
                /* 改行 → 次行先頭にいる、読み飛ばし不要 */
                p->lex->pushback = -1;
            } else {
                /* 非改行(空白等)または-1 → 行末まで読み飛ばす */
                p->lex->pushback = -1;

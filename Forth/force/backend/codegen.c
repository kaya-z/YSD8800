...
void codegen_free(codegen_t *cg) { free(cg); }
...
static void emit_node(codegen_t *cg, ir_node_t *n) {
    ...
    case IR_VAR_DEF: {
        /* VARIABLEは2つの部分を生成:
         * 1. データ領域（.orgでデータ位置に配置）
         * 2. ワード定義（アドレスをpushするルーチン）*/
        int addr = cg->data_org;
        cg->data_org += n->ival;

        fprintf(out, "\n; VARIABLE %s\n", n->sval);
        fprintf(out, "    .org %s%04X\n", hpfx, (unsigned)addr);
        fprintf(out, "VAR_%s:\n", n->sval);
        fprintf(out, "    DW   0\n");
        /* ワード本体: アドレスをpushして返す */
        fprintf(out, "%s%s:\n", pfx, n->sval);
    ...
    case IR_VALUE_DEF: {
        ...
        fprintf(out, "    .org %s%04X\n", hpfx, (unsigned)addr);
        fprintf(out, "VAL_%s:\n", n->sval);
        fprintf(out, "    DW   %s%04X\n", hpfx, (unsigned)(n->ival & 0xFFFF));
        /* ワード本体: アドレスをpushして@で読む */
        fprintf(out, "%s%s:\n", pfx, n->sval);
        fmt_num(cg, addr, num, sizeof(num));
        fprintf(out, "    LDW  %s, %s\n", tos, num);
        fprintf(out, "    SUBI %s, #2\n", dsp);
...
static void emit_header(codegen_t *cg) {
    FILE *out = cg->out;
    fprintf(out, "; Generated by Force Forth Cross Compiler v1.0\n");
    ...
    /* 16bit乗算ルーチン ( a b -- a*b ) */
    /* ワーク変数: $E060=a $E062=b $E064=result */
    fprintf(out, "; 16bit multiply runtime (uses $E060-$E064 as temp)\n");
    fprintf(out, "_FMUL_A  EQU $E060\n");
    fprintf(out, "_FMUL_B  EQU $E062\n");
    fprintf(out, "_FMUL_R  EQU $E064\n");

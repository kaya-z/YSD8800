...
| scc23 | **v2.02** | scc23_v2_02.c | Small-C 派生 C コンパイラ | **v1.04（ポインタバグ P1〜P7 修正系列）→ v2.00（float Q8.8/定数畳み込み/最適化レベル -O0/-O0-strict/-O1）→ v2.01（asm peephole を -O1 へ新設: P2a 完全同一連続MOV削除・P2b dead store除去。命令ストリームバッファ基盤）→ v2.02（peephole P3 追加: `ADD B,A`/`MOV A,B`→`ADD A,B` 可換吸収。可換性+dead proof で結果A・フラグ保存）。-O0/-O0-strict は byte不変、-O1 で Dhrystone cycles 48055→47885・835 DPS・P:20** |

...
| **emu23** | **v1.12** | **emu23_v1_12.c** | エミュレータ（UART/Timer/SD/IRQ統合 + stack watermark計測 + FM-11方式16ページMMU〈--mmu〉 + インタラクティブモード〈-it〉 + TCR-ACK方式タイマー再武装 + 発火EN=AND〈EN是正/案B〉 + **c_iflag対応〈B-1〉+ MMIOデコード層/8bit全被覆/バス・プルアップ既定〈B-C 改良2〉**） | **v1.12: Phase B' 完了（B-1 TKT-03 + B-C 改良2）。B-1: it_enable_raw_mode()にc_iflagクリア追加。B-C: 4経路にmmio_classify()新設、8bit被覆解消17本、第4状態追加、MMU --mmu非依存化、未接続既定$FF/$FFFF・警告機構・--strict-mmio。分類器全数検算(65536×2幅・不一致0)。絶対ゲート各段全PASS。既定動作v1.11とstdout byte-exact一致・警告0件。設計書emu23_interactive_mode_design_v1_6.md/emu23_device_design_v1_11.md** |

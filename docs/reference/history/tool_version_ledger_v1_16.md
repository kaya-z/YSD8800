...
| emu23_debug_manual | **v1.4** | emu23 デバッグ・ユーザマニュアル（~~台帳 v1.14 まで「v1.1（v1.05 対応）」と実体乖離~~→v1.15 で是正＝KY41。**v1.16 で emu23 v1.13 対応の v1.4 に追従**：§2.1 引数解析変更・§2.2 `--dbg`/`--sym`・§2.5 シンボル読込の診断・制限事項の `MAX_SYM` 128→2048 是正・EMU-D 追加） |

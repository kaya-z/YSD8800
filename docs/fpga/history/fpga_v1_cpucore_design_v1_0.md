...
| 項目 | 内容 |
|---|---|
| 文書名 | fpga_v1_cpucore_design_v1_0.md |
| 版数 | **v1.0（本体・初版）** |
| 作成日 | 2026-06-30 |
...
#### 3.1 構成
A/B/X/SP/PC/FLAGS の6本、各16bit。レジスタ番号は emu23 `get_reg_ptr()` の割付に従い 0=A/1=B/2=X/3=SP/4=PC/5=FLAGS（V0本体 §3.1）。命令のオペランドバイト `rb` は上位4bit=rD、下位4bit=rS（emu23 全レジスタ命令で共通、L1227 等実照合）。
...
| opcode | 命令 | 動作 | フラグ |
|---|---|---|---|
| 0x04 | IRET | FLAGS←pop / PC←pop（§6.3） | pop で復帰 |
...
- **クラスD（PUSH A）**：`SP←SP-2` → S_PUSH_LO（`mem[SP]←A[7:0]`）→ S_PUSH_HI（`mem[SP+1]←A[15:8]`）。POP は逆。
...
- fpga_v0_impl_spec_v1_1.md（V0 本体最新正式版）
- fpga_v1_cpucore_design_skeleton_v1_1.md（承認済骨子・本書の入力）

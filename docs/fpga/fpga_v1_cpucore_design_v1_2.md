...
| 項目 | 内容 |
|---|---|
| 文書名 | fpga_v1_cpucore_design_v1_2.md |
| 版数 | v1.1（レビュー反映・本体クローズ）→ **v1.2（V3統合検証で発見された実装バグの修正を反映）** |
...
> **【v1.2追記・重大バグ修正】** V3（PSRAM統合・CDCブリッジ）実装時の統合検証で、実装（`ysd8800_cpu_v0_1.sv`）のバス出力`always_comb`（状態別`mem_addr`/`mem_rd`/`mem_wr`振り分け）に**S_SUBOP（EXTプリフィックス0x1Fのサブopcodeフェッチ状態）のcaseが欠落**しており、`mem_rd`が常に0のまま出力される潜在バグが発覚した（PUSH/POP/LDB/STB等、0x1F経由の全命令が対象）。regfile制御側・subop_rラッチ側は`if(mem_ready)`の正しいガードを持っていたが、そもそも要求信号(`mem_rd`)を発行していなかった。V1/V2の理想メモリTB（`mem_ready`固定1・要求信号非依存）ではこの欠落が結果に影響せず見過ごされ、V3の実req/ack型メモリで初めて顕在化した（詳細: v3_design_memo_v0_3.md §9／kaizen.txt原則63）。**修正**: バス出力`always_comb`に`S_SUBOP: begin mem_addr = rf_pc; mem_rd = 1'b1; end`（S_FETCH等と同型）を追記。修正確認済み（V3統合TB全PASS）。
...
| **v1.2** | **2026-07-11** | **V3（PSRAM統合・CDCブリッジ）実装時の統合検証で発見された実装バグの修正を反映（§7.1追記）。** RTL実装（`ysd8800_cpu_v0_1.sv`）のバス出力`always_comb`にS_SUBOP（EXTプリフィックス0x1Fサブopcodeフェッチ）のcaseが欠落しており、`mem_rd`が常に0のまま出力されていた（PUSH/POP/LDB/STB等0x1F経由命令が対象）。V1/V2の理想メモリTB（`mem_ready`固定1）ではこの欠落が最終結果に影響せず見過ごされ、V3の実req/ack型メモリ統合で初めて顕在化。修正は`S_SUBOP: begin mem_addr=rf_pc; mem_rd=1'b1; end`の1行追記（S_FETCH等と同型）。V3側の全TB（実CPUコア+V3メモリサブシステム、emu23協調等価）およびV1/V2由来の既存TB（理想メモリ）双方でALL PASSを確認済み。本設計書自体（アーキテクチャ・状態遷移設計）に誤りはなく、RTL実装段階での反映漏れであった。詳細: v3_design_memo_v0_3.md §9／kaizen.txt原則63。 |

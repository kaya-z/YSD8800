# V2-e 設計メモ  v0.2  (2026-07-10)
## YSD8800 FPGA V2-e : C8制御割込（EI/DI/SYSCALL/IRET/割込受理）外部観測等価検証

対象読者: レビュアー（かやぬまさん）
位置づけ: 原則43に基づく実装前設計メモ（レビュー承認済(Mなし)・TB作成着手可）
土台: V2-d設計メモ v0.1 / gen_v2_vectors_v2d_poc.py / tb_cpu_v2d_v0_1.sv
レビュー: v2e_design_review_reply_v1_0.docx (2026-07-10・承認Mなし) を反映済(v0.2)
...
★N-1(実装者注意): SYSCALLは「その場でvecへ飛ばない」。irq_pending=4を
立てるのみで、受理は次のfetch前チェック(L1176)で起こる別サイクル動作。
RTL FSMでは S_EXEC_SYSCALL(irq_pending<=4) → 次の S_IRQCHK → S_IRQ_ACCEPT
の順で対応する。SYSCALLを「その場でvec飛び」と誤実装しないこと。
...
### 3.2 SYSCALL の irq番号（★C-1修正: 起票時の誤記を訂正・KY41）
...
↑上記は起票時の誤り（§5 R-1・実測で否定）。正しくは:

  **SYSCALL単独では 4→2正規化は発動しない。** 正規化コード(L318-325)は
  ysd8004_raise() 関数内かつ `if(irq_stat!=0)` ガード内にあり、UART等の
  デバイスが irq_stat をセットした時のみ通る経路。SYSCALL単独では
  ysd8004_raise が呼ばれず irq_stat=0 のため、irq_pending=4 が維持される。
  → 受理で vec=rd16(4*2)=rd16(**$0008**)。§5 R-1 実測($0008)と一致。
  → gen側は SYSCALL を置くだけでよい（黄金が正・emu23をそのまま走らせる）
    点は変わらないが、ベクタは **$0008** に置く（§2.3/§4参照）。
...
## 変更履歴
- v0.2 (2026-07-10): V2-e設計レビュー回答書(v1.0)を反映。総合判定=承認(Mなし)。
  - C-1(要修正)対応: §3.2の旧記述(4→2正規化・vec=$0004)を取り消し線で保持し、
    正しい記述(SYSCALL単独は非正規化・vec=$0008・§5参照)に改版。§5との矛盾解消。
  - N-1(軽微)対応: §1.2にSYSCALL別サイクル受理の実装者注意を補強
    (S_EXEC_SYSCALL→S_IRQCHK→S_IRQ_ACCEPT・その場vec飛び誤実装防止)。
  - Q1〜Q5承認・R-1レビュアー独立追認を確認。C-2/E-1は実装時/kaizenで織り込み。
  - E-1(kaizen登録): §5教訓「emu内部コメントの経路依存性・断定前に実走(KY34)」を
    本日のPDCA-Actでkaizen.txtへ登録予定(V0レビューのirq_pending二義性の再来)。

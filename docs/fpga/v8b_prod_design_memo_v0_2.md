...
- 対象   : `tb_cpu_v8b_prod_v0_1.sv`（V8-b 本番用テストベンチ、v0.2 承認後に着手）
- 版数   : **v0.2**（v0.1 差戻し対応・実源照合 6 箇所実施済）
- 前提資料 :
  - HANDOVER_CHAT125.md（V8-b Pre 完了・本番着手引継）
  - **v8b_prod_design_memo_v0_1_review_v1_0.md（本改版の直接契機）**
...
```systemverilog
// tb_cpu_v8b_prod_v0_1.sv (v0.2 承認後に実装)
...
// --- DUT 2: v5 membus (ファイル v0.2 / module 名は _v0_1) ---
...
| **CL-3** | CPU 初期状態 | **リセット直後**（`_kstart` 実行前）: **`dbg_pc == 16'h0E00`**（V0.2 訂正：$5100→$0E00）、`_kstart` 実行後（数 cycle 後）: **`dbg_sp == 16'h477E`**（v0.2 訂正：$E000→$477E, KERN_SP_TOP） | CPU コアのリセット挙動異常 |
...
- **v0.2 (2026-07-31) : レビュー指摘（差戻し）5 件必修正 + 3 件補足 + 実源照合副産物 5 件反映。実源照合 6 項目（A/B/C/D/E/F）完了。**
- 想定 TB 実装ファイル名 : `tb_cpu_v8b_prod_v0_1.sv`（v0.2 承認後に着手）
- 想定生成物 : `uart_out.log`, iverilog 実行ログ, PASS/FAIL 判定
- 関連原則 : 76（実源照合）, 82, 83, 86, 87（弁別性・失敗マーカー活用）, 88, 93, 94（PSRAM 全域初期化）, 95, 96, 97

**レビュー観点（推奨）**：
1. 実源照合 6 項目（A/B/C/D/E/F）の反映漏れがないか
2. UC-1（sd_image.hex の V8-a→V8-b 十分性）の判定
3. UC-2（CL-1 の 4 番目 opcode 値）の v0.2 での扱い（情報表示のみ）が妥当か
4. UC-3（M-1 タイムアウト 10,000）の妥当性
5. 失敗マーカー 'i'/'g'/'v' の即時 FAIL 判定が過剰でないか（false positive リスク）
...
3. **実源照合結果の副産物発見も設計書に明示**（本 v0.2 の §0.2 追加改善 5 件）

YUI OS ビルド手順書 v1.16 / 2026-08-30

**YUI OS ビルド手順書**

~~yuios_build_procedure_v1_12.docx~~ → ~~yuios_build_procedure_v1_13.md~~ → ~~yuios_build_procedure_v1_14.md~~ → ~~yuios_build_procedure_v1_15.md~~ → **yuios_build_procedure_v1_16.md**（★v1.13で拡張子を実体に合わせ `.md` へ是正。v1.12までは `.docx` でありながら実体はUTF-8プレーンテキストだった＝KY41 4点整合）
...
| --- | --- |
| 文書種別 | YUI OS ビルド手順書 |
| バージョン | ~~v1.4~~ → ~~v1.6~~ → ~~v1.7~~ → ~~v1.8~~ → ~~v1.9~~ → ~~v1.10~~ → ~~v1.11~~ → ~~v1.12~~ → ~~v1.14~~ → ~~v1.15~~ → **v1.16**（★v1.16 で §4.14.2 の誤記3件を是正＝①版数確認構文の誤り（`hasm23`/`emu23` に `-v` は誤動作）②`lnk23` の版数確認欠落 ③TB 版数が `v0_2` のまま（★v0.2 は M-5 判定が速度依存で必ず誤 FAIL する★）。工程②-A' の帰結を反映★）（★v1.15 で §4.14「RTL シミュレーション環境の構築」を新設＝工程②-A で判明した環境起因の躓き6件を恒久化★）（KY41是正: 旧項目表は v1.4 のまま放置されていたため v1.6 に更新。v1.7 で Step 8-B Makefile 化を反映。v1.9 で §4.13 ツール実体検証・環境同期を新設。v1.10 で scc23 v2.04 char ロード幅是正対応・絶対ゲート更新 819/48785/P:20。v1.11 で startup_harness23 v1.6→v1.7 `_irq1_handler` レジスタ退避追加対応） |
| 作成日 | 2026-04-29 |
| 最終改版日 | ~~2026-06-02~~ → ~~2026-06-21~~ → ~~2026-07-04~~ → ~~2026-07-26~~ → ~~2026-07-27~~ → ~~2026-08-09~~ → ~~2026-08-14~~ → ~~2026-08-28~~ → **2026-08-30** |
...
> ★**v1.16 是正（誤記E-1・E-2）**★
> 上記1行は **v1.15 までの誤記**である。正しくは下記のとおり。
>
> ```bash
> # ---- ★版数確認は構文がツールごとに違う★ ----
> ./force  -v      # Force v1.5
> ./lnk23  -v      # lnk23 - YSD8800 ISA2.3 Linker v2.01   ★v1.15 まで欠落★
> ./scc23  -v      # scc23 v2.07 (2026-08-13) for YSD8800 ISA2.3
> ./hasm23         # ★引数なし★ hasm23 v1.04 (2026-06-20)
> ./emu23          # ★引数なし★ emu23 v2.15 (2026-08-23)
> ```
>
> | 誤記 | 内容 | 是正 |
> | --- | --- | --- |
> | **E-1** | `./hasm23` / `./emu23` に `-v` を付ける記述が混在していた | ★この2本は `-v` が**誤動作する**★。**引数なし**でバナーを出す |
> | **E-2** | ★`lnk23` の版数確認が**丸ごと欠落**していた★ | `./lnk23 -v` を追加。5本すべてを確認すること |
>
> ★5本すべてを毎回確認する。1本でも飛ばすと版数取り違えに気づけない。★
...
# ★v1.16 是正(E-3): TB は v0_2 ではなく ★v0_3★ を使うこと。
#   v0.2 は M-5 判定が「末尾一致」で★速度依存★であり、
#   LATENCY_NORMAL が短いビン(4/5/6)で★必ず誤 FAIL する★。
#   （工程②-A' の真因。RTL は正常であった。原則136 / KY76 参照）
rm -f v8b.vvp
iverilog -g2012 -s tb_cpu_v8b_prod_v0_3 \
         -Ptb_cpu_v8b_prod_v0_3.MAX_CYCLES=10000000 \
         -o v8b.vvp -f rtl.f
...
| **R-7【v1.16】** | ★**`-s` を省くと `-P` が黙って無視される**★ | ★エラーも警告も出ない★。MAX_CYCLES が既定値(1,000,000)のまま走り、M-5 未到達を FAIL と誤認する | ★`-s <トップモジュール名>` を必ず明示する★。`-Ptb_x.MAX_CYCLES=…` の `tb_x` と一致させること |
| **R-8【v1.16】** | ★**TB の PASS/FAIL 判定器が速度依存**★ | LAT=4/5/6 で FAIL、7/12 で PASS。★真の不具合と区別できない★ | ★`tb_cpu_v8b_prod_v0_3` 以降を使う★。v0.2 は使用禁止。原則136「判定器自体を被疑対象から外さない」 |
| **R-9【v1.16】** | `uart_out.log` が★固定名★ | 複数ビンを同一ディレクトリで走らせると**上書きされる** | ★ビンごとに実行ディレクトリを分ける★（`run_lat5/` 等）。HEX はシンボリックリンクで共有してよい |
...
rm -f v8b.vvp
iverilog -g2012 -s tb_cpu_v8b_prod_v0_3 \
         -Ptb_cpu_v8b_prod_v0_3.MAX_CYCLES=10000000 -o v8b.vvp -f rtl.f
...
**フェーズ定義**（`tb_cpu_v8b_prod_v0_3.sv` L16-19）：

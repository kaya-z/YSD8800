YUI OS v2.5 マイクロカーネル設計書	Version 2.5 / 2026-06-06
...
| v2.5 | 2026-06-06 | **Ph.5 ProcMgr 詳細化（案C）**：①§8 を全面詳細化（KY42 衝突解消・PROC_EXEC フロー・SVC API・エラー処理・単一プロセス制約・ワーク変数・レビュー観点 R1-R6）、②§8.4 で $C000-$DFFF を再分割（ProcMgr ロード領域 $C000-$C7FF 2KB ＋ MemMgr プール $C800-$DFFF 6KB/24ページ）、③§5 MemMgr を $C800 起点・24ページに整合（ビットマップ初期値 $FFF0）、④§10.1 Ph.5 を「設計中」に更新。**レビュー未完・実装着手禁止**。memmap は v1.7、本書確定後に最終同期 |
...
| $C000-$C7FF | 2 KB | **ProcMgr ロード領域**（Phase 1 単一プロセス固定） | **【v2.5 案C新設】§8.4** |
| $C800-$DFFF | 6 KB | 固定ページプール（MemMgr管理・24ページ） | **【v2.5 案C：$C000-$DFFF→縮小移設】** |
...
| 管理方式 | ビットマップ | 32bit 1変数で管理（bit0-23 有効・bit24-31 無効ページとして使用中マーク）。初期値 PAGE-BMP-HI-INIT=$FFF0【v2.5 案C】 |
...
**24ページ**を 32bit 変数 1 つで管理します【**v2.5 案C：32→24ページ**】。bit0 = $C800ページ、bit23 = $DF00ページ。bit24-31 は無効ページとして恒久使用中マーク（PAGE-BMP-HI-INIT=$FFF0）。
...
| Phase | 内容 |
| --- | --- |
| **Phase 1** | 単一プロセス・ロードアドレス $C000 固定・非PIC バイナリ（本節で設計） |
| Phase 2（将来） | MMU によりプロセスごと独立仮想空間（仮想 $C000 固定・物理は別ページ）。OS-9 メモリモジュール思想に接近 |
...
Phase 1 はロード領域が1つ（$C000-$C7FF）のため同時1プロセスのみ。ProcMgr は `LOAD-SLOT-BUSY` フラグで占有状態を管理し、稼働中プロセスの PROC_WAIT/PROC_KILL/TASK-EXIT 完了でクリアします。
...
| PROC_LIST | 0x0304 | arg0=buf_addr | エントリ数 | タスク一覧を buf へ書込み |

**IPC メッセージ形式（msg0=TOS）：** msg0=opcode / msg1=arg0 / msg2・msg3=予約(0)。
...
カーネル実体（kernel_forth v0.10.11）：`TASK-CREATE ( entry -- tid )` / `TASK-EXIT ( -- )`。ロードした C バイナリは main 終了時に TASK-EXIT を呼ぶ必要があり、scc23 出力の起動規約（startup_proc 相当の crt0）に「main return → TASK-EXIT」を組み込む前提とします（依存事項・レビュー観点 R4）。PROC_WAIT は Phase 1 単一プロセスでは対象 TCB 状態のポーリングで実現し、終了コードは TCB 終了コードフィールドが無ければ Phase 1 では 0 固定とします（レビュー観点 R2）。
...
| 変数 | アドレス（案） | サイズ | 用途 |
| --- | --- | --- | --- |
| LOAD-SLOT-BUSY | $47C0 | 2B | ロード領域占有フラグ（0=空き/非0=稼働 tid） |
| PROC-CUR-TID | $47C2 | 2B | 現稼働プロセス tid |
| PROC-EXIT-CODE | $47C4 | 2B | 直近終了プロセスの終了コード |

$47C0-$47FF は memmap v1.6 「カーネルワーク予約（reserved growth）」。ユーザ空間サーバのワークをここに置く是非はレビュー観点 R3（Forth ワークスペース $E000- の VARIABLE 化も候補）。

## **8.8 レビュー観点一覧（Ph.5 設計）**

| # | 観点 | 連動 |
| --- | --- | --- |
| R1 | MemMgr 24ページ化のビットマップ初期値 $FFF0・PAGE-TOTAL=24 でALLOCループが bit0-23 のみ正走査するか実体確認 | §8.4 |
| R2 | PROC_WAIT 終了コード保持方式（TCB拡張 or Phase1=0固定） | §8.5 |
| R3 | ProcMgr ワーク変数を $47C0系（カーネル予約）に置く是非 | §8.7 |
| R4 | scc23 出力が main 終了後 TASK-EXIT を呼ぶ crt0 の有無 | §8.5 |
| R5 | Phase 1 C バイナリが 2KB に収まるか dhry_timer 等で実測 | §8.4 |
| R6 | 本書確定後 memmap v1.7・design 本節の最終反映 | §8.4 |
...
| Ph.5 | ProcMgr | Cバイナリロード実行（案C：$C000-$C7FF 固定ロード／MemMgrプール $C800-$DFFF へ移設） | 🔧 **設計中（v0.1 ドラフト・レビュー前）** |

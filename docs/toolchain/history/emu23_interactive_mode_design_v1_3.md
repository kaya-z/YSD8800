...
~~Version 1.2  /  2026-06-27~~  **Version 1.3  /  2026-08-10**
...
| 作成日 | 2026-06-26 |
| ステータス | ~~**v1.2 実装完了**（emu23_v109.c 実装・全モード回帰 PASS・YUI OS 実起動で ver/help/ps 応答到達＋Ctrl+D終了を PTY 実証済み。yuios.bin=56416 byte md5一致でビルド健全性確認済み）~~ → **v1.3 レビュー待ち**（TKT-03＝`c_iflag` 未規定の是正。実装未着手・原則43によりレビュー承認前は実装しない） |
...
| `BRKINT` / `INPCK` / `PARMRK` | 環境依存 | 標準的 raw mode の定石。入力の加工を一切させない |
...
| **T03-2** | LF 単独の扱い | 同上で `ver` + **LF** を送出 | 行確定しない（**YUI OS 側の仕様どおり**。emu23 が LF を CR に変換してはならない） |
...
| **T03-6** | raw mode 復帰 | `-it` 終了後に `stty -a` を確認 | `icrnl`／`ixon` が復元されている（`it_orig_termios` の丸ごと復元） |

...
| 項目 | 内容 |
|---|---|
| ファイル名 | hasm23_xref_yof_design_v2_0.md |
| Version | v2.0 |
| Status | **DRAFT（レビュー待ち）** |
| 作成日 | 2026-06-20 |
| 対象工程 | Step 8-F-2（ビルド手順書 §10 将来課題 No.2/3/6 の恒久対処） |
| 関連 | yuios_build_procedure_v1_5.docx §10 / kaizen 原則31 / lnk23_design_v1_3.docx / yuios_memmap_design_v2_4.md / review_hasm23_xref_yof_v1_0.md / review_hasm23_xref_yof_v2_0.md |
...
**【v2.1・C-1 確定（v1.1）→ v2.0 で全面撤回】** ~~YOF オンディスク形式不変・lnk23 無改修の保証~~。
...
- **load_yof でセクションの load_addr フィールドを読み、非 0 ならそのアドレスへ固定配置**
  （place_sections で自動連続配置せず、load_addr を使う）。load_addr=0 のセクションは
  従来通り自動配置（後方互換）。
...
| **I3** | **道2 yuios.bin と従来 .bin リンク版 yuios.bin のバイト比較**【v2.0】 | **実コード領域が一致（配置・内容の非回帰確認）。空白部の差異のみ許容** |
...
6. **単体テスト T7-T8（道2 obj 生成・内部参照保持）。**
7. **lnk23_v2_00.c → lnk23_v2_01.c（KY38）。改修 (D) 道2：YOF load_addr 尊重配置（§3.5.3）。**
8. **単体テスト T9（lnk23 固定配置・実行正解）。**
...
| **Q6** | **load_addr=0 の扱い** | 道2 で load_addr=$0000 のセクション（kernel・Dhrystone harness 等）は「固定$0000配置」か「自動配置」か。本設計は **load_addr=0 は従来通り自動配置（後方互換）**とするが、$0000 への意図的固定配置と区別できない懸念。レビューで方式確認 | 要確認 |

...
| 項目 | 内容 |
|---|---|
| ファイル名 | hasm23_xref_yof_design_v2_3.md |
| Version | v2.3 |
| Status | **実装完了・検証済み（I1-I3 PASS・全56416バイト一致／実装中に判明した `.vector` 不具合修正を反映）** |
| 作成日 | 2026-06-20 |
...
| **v2.2** | **2026-06-20** | **【KY41 整合是正・FIX 候補】v2.1 までで取り消し線化を漏らしていた旧版（lnk23 無改修）記述3箇所を是正。①§1.3 の旧結論「lnk23 は無改修とする」を取り消し線化＋§3.1 への是正注記。②§3.1 改修方針サマリ表（旧 v1.1）の「lnk23 ｜ 無改修」行および No.2/3/6 限定記述を取り消し線化（正は §3.1.1 の hasm23＋lnk23 両改修）。③§3.7.2 の「SECTION 順序…後勝ちルール」を取り消し線化＋固定配置注記（正は §3.7.3）。設計内容の変更なし＝KY41 整合（同一文書内の新旧矛盾解消）のみ。v2.1 までの記述は取り消し線で保持。 | Claude |
| **v2.3** | **2026-06-21** | **【実装反映・`.vector` 不具合修正】本実装（CHAT62）で改修(C)hasm23 を v1.04、改修(D)lnk23 を v2.01 として実装し、単体 T7-T10・回帰・結合 I1-I3 を全 PASS。実装段階で結合テスト I1（YUI OS 実ビルド）により、§3.5.2 の道2 `.org` 処理が `.vector` を持つ kernel（kernel_v12_7.asm は `.vector reset…`→`.org $0020` の順）で2つの不具合を生むことが判明し、§3.5.2 に追補（§3.5.2.1 新設）で対処。①最初の `.org $0020` を sec_origin と誤記録しベクタ領域 $0000-$001F が欠落→`.vector` を1つでも持つセクションは has_vector 判定で sec_origin=$0000 を強制（ベクタは $0000 固定基準）。②`.vector` を code_buf へ即書き込みすると後続 `.org` パディング（$0000→$0020 のゼロ埋め）が上書き→ベクタを (pos,val) 記録し write_yof 直前に遅延反映する方式へ変更。本修正で I3 が道2版と従来 .bin リンク版の **yuios.bin 全56416バイト完全一致**を達成（領域別比較を超える完全一致）。版数注記：実装版は hasm23=v1.04／lnk23=v2.01（§4 の「v1.03」は (A)(B) 確定版のため道2 追加は v1.04）。v2.2 までの記述は削除せず保持。本修正は承認済み道2設計（review v4.0）の実装中に判明した不具合への対処であり、妥当性は I1-I3／T9／T11／Dhrystone 回帰で実証済み（事前レビュー対象の新規設計ではない）。 | Claude |
...
**【v2.2・KY41 是正】** 上記 v1.1 時点の結論は **v2.0 で覆った**。実ビルド I1 で「lnk23 無改修」前提が
崩壊し、現在の正しい結論は「**改修対象は hasm23＋lnk23＋ビルド手順**」である（§3.1.1・§3.5）。
...
**【v2.2・KY41 是正】以下の表は v1.1 時点の旧サマリ（lnk23 無改修前提）。v2.0 で覆ったため
取り消し線とする。現行の正しいサマリは §3.1.1（hasm23＋lnk23 両改修）を参照。**

| 対象 | 改修内容 | 規模 |
|---|---|---|
| ~~hasm23~~ | ~~(A) `.global` 疑似命令の追加（D-1 対処）~~ | ~~小~~ |
| ~~hasm23~~ | ~~(B) UNDEF シンボル複数参照対応（D-2 対処）~~ | ~~小~~ |
| ~~lnk23~~ | ~~無改修~~ → **§3.1.1 で両改修に是正** | ~~—~~ |
...
（注：この「lnk23 無改修」は v1.03 仕様確定**当時**の前提。v2.0 でこの前提自体は撤回されたが、
GLOBAL-UNDEF を発生させない判断は道2 でも有効なため本仕様は維持する。）
...
##### 3.5.2.1 `.vector` を持つセクションの扱い【v2.3・実装で判明・追補】

**背景**: 上記 §3.5.2 の素の道2 仕様は、結合テスト I1（YUI OS 実ビルド）で
`.vector` を持つ kernel（`kernel_v12_7.asm`）に適用したとき2つの不具合を生んだ。
kernel は次の構造を持つ:

```
    .vector reset   _kstart     ; ベクタテーブル（$0000-$0009、id*2 に配置）
    .vector irq0    IRQ0_HANDLER
    ...
    .org $0020                  ; 実コード開始（最初の .org が $0000 ではない）
_kstart:
    ...
```

**不具合① sec_origin 誤記録によるベクタ欠落**: 素の §3.5.2 は「最初の `.org`」を
sec_origin にするため、`.org $0020` を sec_origin=$0020 と誤記録し、kernel obj が
$0020 から始まってベクタ領域 $0000-$001F が欠落した（load_addr=$0020・ベクタ全 $00）。

→ **対処**: `.vector` を1つでも持つセクションは「$0000 基準のベクタテーブルを持つ」
とみなし、`has_vector` フラグ（PASS1 で `.vector` 検出時に立てる）が真なら、
最初の `.org` が $0020 等でも **sec_origin=$0000 を強制**し、$0000→NEW を従来通り
ゼロパディングする。`.vector` を持たないセクション（forth $5100 等）は §3.5.2 の
素の道2 動作（sec_origin=NEW・パディング廃止）のまま。

**不具合② `.vector` 即書き込みのパディング上書き**: `.vector` を code_buf へ即時
書き込むと、その後に来る最初の `.org $0020` のゼロパディング（$0000→$0020 を OUT_BYTE(0)）
が、先に書いたベクタを **$00 で上書き**してしまう（kernel は `.vector`→`.org` の順）。

→ **対処**: YOF モードの `.vector` は code_buf へ即書きせず、**(vec_pos, handler) を
配列 `pending_vec[]` に記録**だけしておき、全アセンブル完了後（write_yof 直前）に
code_buf へ**遅延反映**する。これにより `.org` パディング後に書かれるため上書きされない。
vec_pos = id*2 - sec_origin（has_vector セクションは sec_origin=0 ゆえ id*2 そのもの）。

擬似コード（追補後の `.vector`／`.org` 処理）:
```
# PASS1: .vector を見たら has_vector = 1（ファイル全体で1回でも）

on ".org NEW" (yof_mode, 最初の .org):
    if has_vector:
        sec_origin = 0; sec_origin_set = 1
        while pc < NEW: OUT_BYTE(0); pc++      # ベクタ領域込みでパディング
    else:
        sec_origin = NEW; sec_origin_set = 1   # §3.5.2 の素の道2
        pc = NEW

on ".vector NAME HANDLER" (yof_mode):
    vec_pos = id(NAME)*2 - sec_origin
    record pending_vec += (vec_pos, HANDLER)   # 即書きせず記録

before write_yof:
    for (pos, val) in pending_vec:
        if 0 <= pos and pos+1 < code_buf_pos:
            code_buf[pos]   = val & 0xFF
            code_buf[pos+1] = val >> 8
```

**検証結果（I1-I3）**: 本追補により、道2版 yuios.bin が従来 .bin リンク版と
**全56416バイト完全一致**（§5.2 I3）。リセットベクタ `$0000 = $0e00`（_kstart 相当）
が正しく保持され、YUI OS 起動出力も従来版と一致（I2）。単体 T9（`.vector reset`→
`.org $0010` の最小 kernel）でもリセットベクタが $0010 を正しく指すことを確認。

**この追補が影響しない範囲**: `.vector` を持たないセクション（forth・Dhrystone の
非YOF .bin）には has_vector=0 で従来通り。Dhrystone 回帰 826/48405/P:20 不変を確認済み。
...
| forth load_addr=$5100 固定配置（has_org=1）※ | ✅ |
...
※【v2.2・E-4】PoC は has_org 導入前のコードで「load_addr を読むが has_org 判定なし」で通った
記録。本実装（has_org 判定あり）では §6 ステップ8 の T9 で has_org=1 経路として再検証する。
...
| ~~SECTION 順序 forth 先 → kernel 後~~ | ~~必須~~ | ~~必須（不変・後勝ちルール）~~ → **【v2.2・KY41 是正】道2 は load_addr 固定配置のため後勝ちに依存しない（§3.7.3）** |
...
| **T11** | **道2: `.vector`→`.org $0020` 順の kernel セクション obj 生成・リンク・実行**【v2.3・実装で追加】 | **has_vector 判定で sec_origin=$0000・load_addr=$0000・ベクタが遅延書き込みで保持。リセットベクタが `_kstart` を正しく指す（T9 の最小 kernel で実証済み）** |
...
**【v2.3・実装結果（CHAT62・2026-06-21）】結合テスト I1-I3 全 PASS**:
- **I1**: 道2版ビルド（`build_road2.sh`）で yuios.bin 生成。forth@$5100/kernel@$0000 固定配置、
  WORD_OS_START を 418 global symbol と共に解決、**reloc=2**（kernel_v12_7.asm の `#$e96e`2箇所→
  `#$WORD_OS_START` UNDEF 参照）。後処理1 は固定リストでなく `#WORD_xxx` 全自動抽出（実態6個）。
- **I2**: 道2版・従来版とも起動出力 `YUIOS Booted!` `0YUI> 1X23MD`（Ph.6 Shell＋5サービス）で一致。
- **I3**: §3.5.2.1 の `.vector` 修正後、道2版と従来 .bin リンク版の yuios.bin が
  **全56416バイト完全一致**（領域別比較を超え、隙間パディングを含め全バイト一致）。
  sed系統②（No.6）廃止後も従来と等価なバイナリを生成することを最強の形で実証。

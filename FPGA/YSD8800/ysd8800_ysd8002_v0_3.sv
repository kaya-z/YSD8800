// ============================================================
//  ysd8800_ysd8002_v0_3.sv   v0.3  (2026-07-18 EN是正)
//  ★正式版★ (EN是正工程・原則43承認済 2026-07-18)
//  ┌─ v0.3 変更点（EN是正工程 / 2026-07-18）───────────────
//  │ ・発火EN条件を OR→AND化（案B）。L199:
//  │     fire_en = timer_en_r | irq_en_r  →  timer_en_r & irq_en_r
//  │ ・目的: IRQ_EN(bit1)=0 で割込マスクが名前どおり機能（契約回復）。
//  │ ・emu23 v1.11(L696 (tcr&0x01)&&(tcr&0x02)) と対称・真理値等価。
//  │ ・★将来課題(案A): TIMER_EN=0 でのカウンタ停止は未実装★
//  │   （発火のみ抑止。詳細 v6_en_fix_design_memo_v0_1.md §3）。
//  │ ・回帰: Dhrystone不変 / 負例TCR=$01でOR/AND弁別（KY54）。
//  └────────────────────────────────────────
//  ~~ysd8800_ysd8002_v0_2.sv   v0.2  (2026-07-17 正式版昇格)~~ ←v0.3で改版
//  ★正式版★ (S5昇格・原則43承認済 2026-07-17)
//  ┌─ 昇格記録（S5 / CHAT100 / 2026-07-17）─────────────────
//  │ ・v0.2_poc → v0.2 正式版へ昇格（ユーザ承認 2026-07-17）。
//  │ ・★現行 OR 実装（fire_en=|tcr_r[1:0]）のまま昇格★。
//  │   TCR EN 是正（OR→AND）は v5_design_memo §3.5.2 通り
//  │   ★V6 以降の独立工程★。本正式版は暫定的にデバイス契約違反
//  │   （IRQ_EN 名前どおり非機能）を管理下の負債として含む。
//  │ ・昇格前確認: YUI OS 側「タイマー回すが割込マスク」実装の
//  │   近接予定なし（ユーザ確認 2026-07-17）→ 負債は V6 送り維持。
//  └────────────────────────────────────────
...
module ysd8800_ysd8002_v0_3 (
    ...
    // ------------------------------------------------------------
    // ★発火 EN 条件【v0.3 EN是正: AND】★
    //   emu23 v1.11 L696: irq_enabled = (tcr&0x01) && (tcr&0x02) ? 1:0;
    //   → TIMER_EN(bit0) かつ IRQ_EN(bit1) の両方が1で発火許可。
    //   これで IRQ_EN=0 が名前どおり割込マスクとして機能する（契約回復）。
    //   原則59: always_comb 内の定数ビット選択を避け assign で外出し
    //   原則76: 黄金リファレンス(emu23 v1.11)に合わせる。emu の &&(論理AND) と
    //           RTL の &(1bit AND) は真理値等価。
    //   ★将来課題(案A): TIMER_EN=0 でのカウンタ停止は未実装（発火のみ抑止）。
    //     詳細は v6_en_fix_design_memo_v0_1.md §3 / ysd8002_timer_design。
    // ------------------------------------------------------------
    logic fire_en;
    assign fire_en = timer_en_r & irq_en_r;    // ★AND★ (v0.3 EN是正・emu L696対称)

// ================================================================
//  tb_cpu_v5timer_short.sv   v0.2  (2026-07-17 正式版昇格 / 元 CHAT98)  ★短縮版★
//  ★正式版★ S5昇格: module=tb_cpu_v5timer_short, 下位 u_membus は ysd8800_v5_membus_v0_1
//  ── 改版履歴(KY41: 前版情報は取消線で保持) ──
//    v0.2 (2026-07-16): 案Y+X+Z併用の短縮版。
//      - hex を short 版に差替(内ループ1000→100)。emu23 v1.10黄金値:
//          ack   : CNT=3 / OUTC=200 / 正常HALT
//          noack : CNT=1 / OUTC=200 / 正常HALT
//      - psram_clk 比 20:1→8:1 (#0.5→#1.25) : 案X。論理はクロック比非依存(S3実証)。
//      - MAX_CYC 4000000→3000000 : 案Z。比8:1/内100の実測 cyc/OUTC≒14545、
//          OUTC=200完走に約291万cyc必要(800kでは不足しHALT未達だった実測に基づく修正)。
//      - 判定は不変(T2/T5:OUTC==200据置, T4:CNT>=2, T6:noack==1)。
//        ★絶対CNT一致はT6(noack=1)のみ。ackは範囲判定ゆえCNT30→3でも判定不変(案Z思想)★
//    ~~v0.1 (2026-07-15 CHAT93): 内1000×外200, 比20:1, MAX_CYC=4M。実行約11.5分。~~
//  ── 派生元 ──
...
module tb_cpu_v6mask;   // EN是正 負例TB: TCR=$01(IRQ_EN=0)でCNT=0を検証
    ...
    initial psram_clk = 0;
    always #1.25 psram_clk = ~psram_clk;   // ★v0.2: 比20:1→8:1(案X)★
    ...
    localparam integer MAX_CYC   = 3000000;   // ★v0.2: 実測cyc/OUTC≒14545×200=291万に余裕(比8:1/内100)★
    ...
    initial begin
        $display("============================================================");
        $display(" tb_cpu_v6mask  EN是正 負例TB  YSD8800 FPGA");
        $display("   YSD8002=v0.3 (発火EN OR->AND化・案B)");
        $display("   検証: TCR=$01(TIMER_EN=1,IRQ_EN=0) で割込が出ない(CNT=0)");
        $display("   対照: 旧OR実装なら CNT>0 だった(是正の直接証明)");
        $display("============================================================");

        // ---------- 負例プログラム(TCR=$01)を走行 ----------
        //  ★初期化順序はV5-TB実績を逐語踏襲★
        $display("[LOAD] v6t_mask_short.hex (TCR=$01, IRQ_EN=0)");
        cpu_rst_n = 0;
        psram_rst_n = 0;
        uart_rx_valid_i = 1'b0;
        uart_rx_data_i  = 8'h00;
        ...
        $readmemh("v5timer/v6t_mask_short.hex", u_membus.u_psram_ctrl.mem);
        repeat (3) @(negedge cpu_clk);
        cpu_rst_n = 1;
        ...
        $display("--- N0: TB健全性(KY54): CNTが0でないと主張しFAIL検出 ---");
        chk_expect_fail("N0_tb_sanity", cnt_ack, /*wrong_exp=*/999);

        $display("--- N1: ★是正の本丸★ IRQ_EN=0 で割込が出ない (CNT==0) ---");
        chk("N1_irqen_masks", cnt_ack, 0);

        $display("--- N2: ハンドラ未実行ゆえ OUTC は主処理完走値(200) ---");
        chk("N2_main_completed", outc_ack, 200);

        $display("--- N3: 正常HALT到達 ---");
        chk("N3_halt", (dbg_halt===1'b1) ? 1 : 0, 1);

        // ---------- 総括 ----------
        $display("============================================================");
        if (errors == 0)
            $display("V6MASK_TB: ALL PASS  (%0d checks) ★EN是正 RTL負例 成立★", passes);
        else
            $display("V6MASK_TB: %0d FAIL / %0d PASS", errors, passes);
        $display("============================================================");
        $finish;
    end

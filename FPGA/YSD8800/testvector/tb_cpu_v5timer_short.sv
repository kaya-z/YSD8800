...
module tb_cpu_v5timer_short;
    ...
    initial begin
        ...
        $display("[LOAD] v5t_ack_short.hex (ACKあり)");
        cpu_rst_n = 0;
        psram_rst_n = 0;
        uart_rx_valid_i = 1'b0;
        uart_rx_data_i  = 8'h00;
        ...
        // (2) プログラムロード
        $readmemh("v5timer/v5t_ack_short.hex", u_membus.u_psram_ctrl.mem);
        // (3) ロード完了後にreset解除
        repeat (3) @(negedge cpu_clk);
        cpu_rst_n = 1;
        ...
        $display("[LOAD] v5t_noack_short.hex (ACKなし=対照)");
        cpu_rst_n = 0;
        psram_rst_n = 0;
        repeat (3) @(negedge psram_clk);
        psram_rst_n = 1;
        for (int i = 0; i < 16'h0200; i = i + 1)
            u_membus.u_psram_ctrl.mem[i] = 8'h00;
        $readmemh("v5timer/v5t_noack_short.hex", u_membus.u_psram_ctrl.mem);
        repeat (3) @(negedge cpu_clk);
        cpu_rst_n = 1;

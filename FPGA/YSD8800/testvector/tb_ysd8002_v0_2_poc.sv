...
module tb_ysd8002_v0_1;
    ...
    // ------------------------------------------------------------
    //  ★v0.2 (B-1a): irq_timer_o が【パルス】→【レベル】に変わった★
    //
    //  v0.1 の観測: if (rst_n && irq_timer) fire_count++;
    //    → パルス前提。レベルだと ACK が来るまで【毎クロック】+1 され爆増する。
    //      (実測: T3 で 204 まで増加。v0.1 期待値 1 に対し FAIL)
    //
    //  v0.2 の観測: ★立ち上がりエッジ★を数える = 発火【回数】
    //    これにより T2〜T6 の期待値(1/1/1/2/2)がそのまま使える。
    // ------------------------------------------------------------
    logic irq_timer_d;
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) irq_timer_d <= 1'b0;
        else        irq_timer_d <= irq_timer;
    end

    wire irq_rise = irq_timer & ~irq_timer_d;   // ★立ち上がりエッジ★

    // 発火回数カウント (★エッジ検出★)
    always_ff @(posedge clk) begin
        if (rst_n && irq_rise) fire_count <= fire_count + 1;
    end

    // ------------------------------------------------------------
    //  ★新設: レベル保持の直接観測 (B-1a の中核を検証する)★
    //    irq_level = irq_timer の【現在値】。
    //    ・発火後 ACK 前  → 1 が【保持】されていること  ★これが B-1a★
    //    ・ACK 直後       → 0 に落ちること
    //    エッジカウントだけでは「立った瞬間に落ちる(=パルスのまま)」バグを
    //    検出できない。★レベルであることを直接見る★
    // ------------------------------------------------------------
    wire irq_level = irq_timer;

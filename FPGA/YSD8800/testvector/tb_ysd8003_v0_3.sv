//==============================================================
// tb_ysd8003_v0_3.sv
//   YSD8003 ストレージコントローラ v0.3 テストベンチ
//   DUT: ysd8800_ysd8003_v0_1 (ファイル ysd8800_ysd8003_v0_3.sv)
//   SDモデル: sd_spi_model_v0_1_poc (ファイル sd_spi_model_v0_2_poc.sv)
//   Version: v0.3 (2026-07-19)
//
// 【変更履歴】
//   v0.1 (2026-07-19) 初版。T1/T2/T3/T4/T6骨格。
//   v0.3 (2026-07-19/CHAT109/Step1) irq_stor_o パルス化に伴うT4更新:
//     - irq_stor_o は完了時1クロックパルスに変更(RTL v0.3)。
//       別プロセスで立ち上がりを捕捉する irq_stor_seen(スティッキー)を追加。
//     - T4を「パルス捕捉検証(seen==1)＋線がLレベル(pulse,not level)」へ更新。
//       旧「レベルH持続＋ackで落ちる」判定を廃止。ackは未駆動(0固定)。
//     - 検証後 irq_stor_seen を手動クリア(YSD8004 W1C相当の擬似)。
//   v0.2 (2026-07-19/CHAT107) 全PASS化に伴うTB修正:
//     - T6判定を2段化（EXEC受理でS_IDLE離脱を待ってからTO→ERROR復帰を待つ）。
//       mmio_write NBA遅延でEXEC未受理のS_IDLEを誤検出しno-hangが偽PASSする問題を是正。
//     - T6のSTAT読みを2回読み契約に修正（busy_latch=1回目BUSYを消費後にERROR確認）。
//       案D/emu23互換のbusy_latch挙動に整合。→ T6 STAT ERRORビットPASS。
//     結果: T1〜T4・T6 全PASS（9 PASS / 0 FAIL）。
//
// 【検証方針（設計メモ§検証・案D）】
...
//   T4: 完了IRQ(IRQ1)を1クロックパルスで通知（seen捕捉・線はLレベル）
//==============================================================
`timescale 1ns/1ps
...
module tb_ysd8003_v0_1;
    ...
    //----------------------------------------------------------
    // v0.3: 完了IRQパルス捕捉（irq_stor_o は1クロックパルスに変更）
    //   別プロセスで立ち上がりを捕捉しスティッキー保持。
    //   T4で seen を検証し、検証後に手動クリアする。
    //----------------------------------------------------------
    logic irq_stor_seen;
    initial irq_stor_seen = 1'b0;
    always @(posedge clk) begin
        if (irq_stor_o) irq_stor_seen <= 1'b1;  // パルス捕捉（スティッキー）
    end
    ...
    initial begin
        ...
        begin
            integer t; t=0;
            // まずEXEC受理でfsmがS_IDLEを離れるのを待つ（mmio_write NBA遅延吸収）
            while (dut.fsm === dut.S_IDLE && t < 1000) begin
                @(posedge clk); t=t+1;
            end
            // 次にタイムアウト→S_ERROR→S_IDLE復帰を待つ
            t=0;
        ...
        // busy_latch=1回目STAT読でBUSYを返す案D契約のため、READY/ERROR確定まで読む
        rd = 8'h00;
        begin
            integer u; u=0;
            while (!(rd[1] || rd[2]) && u < 100) begin
        ...
        // T4: 完了IRQパルス確認（v0.3: irq_stor_o は1クロックパルス）
        //   完了サイクル近傍を跨ぐため数クロック待ってから判定する。
        //   捕捉プロセス irq_stor_seen が完了パルスをスティッキー保持済のはず。
        repeat (3) @(posedge clk);
        check("T4: IRQ pulse seen on completion", irq_stor_seen === 1'b1);
        //   パルスは1クロックで自然に落ちる（現時点はLレベルのはず）。
        check("T4: IRQ line is low (pulse, not level)", irq_stor_o === 1'b0);
        //   捕捉フラグを手動クリア（YSD8004側W1C相当・単体TBでは擬似）。
        irq_stor_seen = 1'b0;

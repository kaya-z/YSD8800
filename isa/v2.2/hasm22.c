// hasm22.c - YSD8800 ISA2.2 Assembler v1.00
//
//  ISA2.2 追加点 (ビット演算命令 0x50-0x59):
//    AND  rD, rS    / ANDI  rD, #imm16
//    OR   rD, rS    / ORI   rD, #imm16
//    XOR  rD, rS    / XORI  rD, #imm16
//    NOT  rD
//    SHL  rD, rS    (論理左シフト)
//    SHR  rD, rS    (論理右シフト)
//    SAR  rD, rS    (算術右シフト)
//
//  ISA2.0 からの追加点 (拡張プレフィックス 0x1F):
...
//
//  書式例:
//    PUSH A          ; 2 bytes: 0x1F 0x00
//    POP  B          ; 2 bytes: 0x1F 0x04
//    LDB  A,[addr]   ; 4 bytes: 0x1F 0x10 <lo> <hi>
//    LDB  A,[X]      ; 2 bytes: 0x1F 0x11
//    STB  B,[addr]   ; 4 bytes: 0x1F 0x16 <lo> <hi>
//    STB  B,[X]      ; 2 bytes: 0x1F 0x17
//
//  その他は ISA2.0 (hasm.c) と完全互換
//
// build: gcc -std=c99 -O2 -Wall hasm21.c -o hasm21
...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "usage: hasm22 file.asm\n"
                        "  ISA2.2 assembler for YSD8800\n"
                        "  Extensions over ISA2.1:\n"
                        "    PUSH/POP A|B|X\n"
                        "    LDB/STB  A|B, [addr] | [X]\n");
        return 1;
    }
    ...
    printf("hasm22: assembled '%s' -> %s  (ISA2.2 / EXT PUSH/POP/LDB/STB + AND/OR/XOR/NOT/SHL/SHR/SAR)\n",
           argv[1], outbin);
    return 0;
}

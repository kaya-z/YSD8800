...
//   拡張時は ysd8001_t.rx_data を uint8_t rx_buf[N] と
...
static void ysd8001_write_stat(uint8_t v) {
    ...
    ysd8001.stat = (ysd8001.stat & ro_bits) | (v & ~ro_bits);
}

// YSD8001 RXデータ受信（外部からの入力をバッファに積む）
// stdin入力や将来のファイル入力はここを通じてバッファに入れる
// RX_IRQ_ENが立っていれば IRQ2 を発生させる
// ※ cpu.irq_pending への書き込みはcpu定義後なのでforward宣言を使う
static void ysd8001_rx_receive(uint8_t v);  // 実装は cpu_t 定義後
...
// 将来のstdin/ファイル入力対応時に使用する予備実装のためunused警告を抑制

// emu23_v2_15.c v2.15 (2026-08-23): [MW-6 peek/peekw 新設 + MW-7 未知コマンド表示]
//   設計書: emu23_mw6_peek_design_v0_2.md（v0.2・条件付き承認／M-1・C-1〜C-3・N-1〜N-3 反映済）
//   レビュー: emu23_mw6_peek_design_v0_1_review_v1_0.md（再レビュー不要）
//
//   【MW-6】MMIO を実機同等経路で読む手段（読み出し側の欠けた1マス）
//   ①dbg_peek8()/dbg_peek16() を新設。rd8()/rd16() を dbg_access で包んで呼ぶ。
//     ★dbg_rd8()/dbg_rd16() と共通化してはならない（役割が正反対・MW-2 が退行する）★
//   ②peekw は dbg_peek8() の合成にしない。8bit 経路は「下位バイトのみ副作用」
//     規則を持つため、合成すると★実機と異なる副作用列★になる。
//   ③REPL に peek/peekw を追加（repl_peek() に集約）。count 既定1・上限64。
//     範囲外は★1バイトも読まずに戻る★（どこまで副作用が起きたか不明にしない）。
//   ④MMIO 帯は読み出しの★前★に警告。count>=2 なら発火回数を明示。
//   ★読み出し副作用を持つレジスタは4本: CYCLE_LO/SCORE_LO/SD_STAT/SD_DATA★
//     （設計書§2.2 で実源全走査。v0.1 は SD_DATA のみ、レビューが +2、
//       全走査で SD_STAT が出た。★列挙に依存する実装は作らない★＝KY65）
//   ★CYCLE_LO/SCORE_LO を peek すると①.5 の測定値が壊れる（ラッチ更新）★
//
//   【MW-7】未知コマンドを黙殺せず "Unknown command:" を表示（MW-1 の未達分）
//   ★MW-6 と同梱した理由: MW-6 の受入試験 N-6/N-7 が MW-7 なしでは成立しない。
//     また前方一致時代は「一致しなかった」を判定できず、MW-1 完了で初めて
//     実装可能になった是正である（KY58 に対する説明・設計書§8.3）★
//   ★空行は従来どおり黙殺（プロンプト連打は常用操作）★
//
// ~~emu23_v2_14.c v2.14 (2026-08-22): [MW-1 REPL照合の構造是正 + MW-2 ダンプ経路の非対称是正]~~ ←v2.15で改版
//   設計書: emu23_mw12_fix_design_v0_5.md（v0.5・条件付き承認／M-5 反映済）
//   レビュー: v0_1/v0_2/v0_3 review v1.0（M-1〜M-5 全消化）
//   ★本版は「是正のみ」。新機能は含まない（KY58）。MW-3〜MW-6 は別版★
//
//   【MW-1】REPL ディスパッチを前方一致＋順序依存から完全一致へ
//   ①repl_kw() を新設（§3.3）。kw 直後が英字/'-' なら不一致、数字/'$' なら引数。
//     → "bd0" が "b" に食われず、分岐の並び順に依存しなくなった（§3.4）。
//   ②既存15分岐を repl_kw() 化。sscanf(cmd+N,...) の N リテラルを全廃（残存0）。
//     ★mmu の cmd+3 は 2 箇所あった（KY59 の取りこぼし注意点）★
//   ③'?' のみ完全一致（"?5" を help に通す意味がないため・C-4）
//   ★破壊される形式: bmain / mmuptr / memff00(a-f始まり16進) / s-5 / ?help
//     いずれも空白1個で代替可能・機能喪失なし（§3.2）★
//
//   【MW-2】ダンプ経路の非対称是正（★対象は4つ。コマンド名でなく関数経由で定義★）
//   ④dbg_rd8()/dbg_rd16() を新設（§4.2）。MMU 変換は行うが MMIO ディスパッチをしない。
//   ⑤dump_mem() を dbg_rd8() 経由へ（16進部・ASCII部の2箇所）。
//     → MMU 有効時に論理アドレスで mem[] を引いていた欠陥を解消。
//   ⑥dump_memw() と ⑦CLI -m を dbg_rd16() へ一本化（原則122・両者とも rd16() 経由だった）
//     → SD_DATA($FCAA) 読み出しで sd_buf_ptr が進む副作用が消滅。
//   ★起票文言「memw $FC80 は UART FIFO を進める」は誤りだった。
//     ysd8001_rx_read() は副作用なし。真の被害箇所は SD_DATA（§2.5）★
//   ★代償: MMIO レジスタは mem/memw/-m で一切読めなくなる（副作用ありは
//     SD_DATA ただ1つだが、副作用なしの全レジスタも巻き添え）。
//     実機同等経路で読む手段は MW-6(peek) として優先度「高」で起票済（§4.5.1）★
//   ★未マップ警告と bus_pullup の $FF 合成も失われる（既定有効のため常に）★
//
//   ⑧版数識別系を機械抽出し 4 箇所を更新（L689/L3305/L3306/L3824）。
//     ★前版ヘッダは「3 箇所」と記録していたが、機械抽出すると 4 箇所だった。
//       前版の箇所数を鵜呑みにせず毎回 grep で数え直すこと（原則124/128）★
//   ★絶対ゲート: Dhrystone 819/48,785/P:20・dhry_final.bin 21,846B/e7e9836f
//     ・-q stdout は v2.13 と byte-exact（-q はダンプ経路を通らないため）★
//   ★未決起票: MW-3(ledger) / MW-4(レジスタ書込) / MW-5(YUI OSシェル前方一致)
//     / MW-6(peek・優先度高)★
//
// ~~emu23_v2_13.c v2.13 (2026-08-22): [任意アドレス書き込み機能 + dbg_access]~~ ←v2.14で改版
//   設計書: emu23_memwrite_design_v0_3.md（v0.3・承認済／M指摘0件）
//   レビュー: emu23_memwrite_design_v0_2_review_v1_0.md v1.0
//   ①REPL に setb/setw/setpb/setpw を追加（★'s' 分岐より前に配置・M-1/KY60★）
//   ②dbg_access 追加。デバッガ由来アクセスを非課金化（A-1〜A-7・M-3）
//     ★抑止するのは課金/統計/トレースのみ。MMIO 副作用は残る（C-1・→MW-2）★
//   ③既存欠陥の是正: dump_memw()/reset/CLI -m が課金経路にあった問題を解消
//   ④版数識別系 3 箇所を v2.13 に更新（L677/L3203/L3714）。
//     ★L3714 は v2.10 のまま 2 版取り残されていた（KY41 破れ・§5.1.2）★
//   ★既定経路は mem_charge() が cache_enable==0 で即 0 を返すため
//     Dhrystone 絶対ゲート(819/48,785/P:20)および -mc 単独(339,240)は不変★
//   ★未決起票: MW-1(REPL照合の構造是正) / MW-2(mem/memw/physmem の非対称)★
// ~~emu23_v2_12.c v2.12 (2026-08-21): [段10 校正結果の反映: LAT 既定値 6→5 確定]~~ ←v2.13で改版
//   v2.11 からの変更は lat[] 初期値のみ（機能追加なし）。
//   根拠: emu23_cache_base_design_v1_0.md §9.3（周波数掃引 9 点による内訳分離）
//   ★既定(--mem-latency 無指定かつ cache_enable==0)では mem_charge() が即 0 を返すため
//     Dhrystone 絶対ゲート(819/48,785/P:20)および -mc 単独(339,240)は不変★
// ~~emu23_v2_11.c v2.11 (2026-08-21): [サイクルカウントテーブル v1.1 是正]~~ ←v2.12で改版
//   v2.10 からの変更は mc_init_default_table() の 6 エントリのみ（機能追加なし）。
//   根拠: cycle_table_v1_1_revision_design_v0_3.md（v0.3・承認済）
//   是正: 0x24 7→6 / 0x25 6→5 / 0x69 7→5 / X:11,13,15,17 6→5
//         ★0x68 JSR は 7 のまま是正不要（v0.1 の 6 案は誤り。理由は当該行コメント）★
//   効果: -mc 単独 351,761 → 339,240（RTL 理想メモリ score_r と完全一致・残差 0）
//   ★Dhrystone 絶対ゲート(819/48,785/P:20)は mc_enable==0 時に表を参照しないため不変★
// ~~emu23_v2_10.c v2.10 (2026-08-20): [改良4: キャッシュ下地（メモリ課金＋物理アドレストレース）]~~ ←v2.11で改版
...
#include <errno.h>    /* [v2.13] dbg_parse_hex() の strtoul 値域チェック用 */
...
/* ===== [v2.13] デバッガ由来アクセスのフラグ（設計書 §4.1）==============
 * デバッガ由来のメモリアクセス中は 1。
 * CPU の命令実行に由来しないアクセスは★サイクルを消費しない★。
 *
 * ★効力範囲（設計書 §4.1.1.2 / レビュー C-1）★
 *   抑止する : サイクル課金 / キャッシュ統計 / 物理アドレストレース
 *   抑止しない: ★MMU 変換 と MMIO 副作用★（rd16()/wr8() 内部のため）
 *               → memw $FC80 が UART FIFO を進める既存欠陥は MW-2 で対処
 *
 * ★戻し忘れ厳禁（KY59）★ 立てた箇所と戻す箇所の数を grep -c で照合すること。
 * 立てるべき箇所は設計書 §4.1.1 の A-1〜A-7 で全数確定済み。
 * ★L3280(main のリセットベクタ読み) は CPU 動作であり包まない（§4.1.1.1）★ */
int dbg_access = 0;
...
static void mc_init_default_table(void)
{
    ...
    /* [v2.11 是正] 7→6。増分法実測＋状態連鎖突合＋区間計数の三重で確定。
       状態連鎖 = FETCH,OPFETCH,DECODE,MEMR_LO/HI(2),WRITEBACK = 6
       設計書 cycle_table_v1_1_revision_design_v0_3.md §10.1 */
    mc_table[0x24] = 6;   /* LDW  rD,[rS]    */
    /* [v2.11 是正] 6→5。同上。
       状態連鎖 = FETCH,OPFETCH,DECODE,MEMW_LO/HI(2) = 5 */
    mc_table[0x25] = 5;   /* STW  rS,[rD]    */
    ...
    /* [v2.11] ★是正不要・値は 7 のまま★
       状態連鎖 = FETCH,OPFETCH,IMML,IMMH,EXEC_JSR,PUSH_LO/HI(2) = 7 で一致。
       RTL 一次情報: ysd8800_cpu_v0_1_FIXED.sv L821-823
         S_EXEC_JSR: next_state = S_PUSH_LO;
         PC←imm_r は S_PUSH_HI 内で完結し、専用状態を持たない(L818-820 コメント)
       ★注意★ 設計書 v0.1 で一度 7→6 に是正しようとしたが誤りと判明した。
         誤りの原因: JSR+RET を対で測定して合計 12 を得た際、内訳を「各 6」と
         等分したこと。JSR は IMML/IMMH を持つが RET は持たないため等分は不成立。
         Dhrystone では JSR/RET の実行回数がほぼ同数のため合計検算では露見しない。
         この経緯を残さないと、後日また同じ是正が提案されうる。 */
    mc_table[0x68] = 7;   /* JSR imm16 */
    /* [v2.11 是正] 7→5（−2）。RTL FSM 直読＋区間計数で確定。
       状態連鎖 = FETCH,OPFETCH,EXEC_RET,POP_LO/HI(2) = 5
       RTL 一次情報: 同 L826-828  S_EXEC_RET: next_state = S_POP_LO;
       区間内回数 JSR=270 / RET=271 の非対称が 1 サイクル分解能で本案を判別した */
    mc_table[0x69] = 5;   /* RET       */
    ...
    /* [v2.11 是正] レジスタ間接形式(X:11/13/15/17) を 6→5。
       状態連鎖 = FETCH,OPFETCH,SUBOP,DECODE,MEMR8(or MEMW8) = 5
       即値形式([imm16]・X:10/12/14/16) は IMML/IMMH を持つため 7 のまま正しい。
       ★誤りの系統性★ 是正対象は「即値を伴わないレジスタ間接」で例外なく揃う
         (0x24/0x25/X:11/13/15/17)。v1.0 作成時に前置命令列の差し引きを誤ったもの。
       ★.cyc 定義ファイルで上書きする際のキー表記は X:0x11 形式★
       設計書 cycle_table_v1_1_revision_design_v0_3.md §10.4 / §23 */
    mc_table_ext[0x10] = 7;  /* LDB A,[imm16] */
    mc_table_ext[0x11] = 5;  /* LDB A,[X]     (v2.11: 6→5) */
    mc_table_ext[0x12] = 7;  /* LDB B,[imm16] */
    mc_table_ext[0x13] = 5;  /* LDB B,[X]     (v2.11: 6→5) */
    mc_table_ext[0x14] = 7;  /* STB A,[imm16] */
    mc_table_ext[0x15] = 5;  /* STB A,[X]     (v2.11: 6→5) */
    mc_table_ext[0x16] = 7;  /* STB B,[imm16] */
    mc_table_ext[0x17] = 5;  /* STB B,[X]     (v2.11: 6→5) */
...
enum { LAT_FILL_IDX = 5 };
/* [v2.12 確定] LAT 既定値を仮値 6 → ★確定値 5★ に更新。
   根拠: emu23_cache_base_design_v1_0.md §9.3（段10 校正結果）
     運用点 psram_clk=32MHz / cpu_clk=4MHz において LAT-1 = 3.957
     内訳: CDC(cpu ドメイン) 2.967 = 75.0% / PSRAM(psram ドメイン) 0.990 = 25.0%
     B = 203,938 バイト（score_r 区間・F:159,356 R:25,544 W:19,038）
   ★LAT は psram_clk の関数であり単一定数ではない★
     128/64MHz→2.97、32MHz→3.96、16MHz→5.94、8MHz→8.90、4MHz→16.82
     周波数を変える場合は設計書 §9.3.1 の実測テーブルを参照すること。
   ★種別ごとの差は無い★ RTL 側はフェッチ/読み/書きを区別せず同一経路を通るため
     全種別に同一値を設定する（機能検証で差を付けたい場合は --mem-latency で上書き）。
   ~~static unsigned lat[6] = { 6, 6, 6, 6, 6, 6 };~~ ←v2.11 まで（机上推定 5.75 の丸め） */
static unsigned lat[6] = { 5, 5, 5, 5, 5, 5 };  /* F,R8,W8,R16,W16,FILL */
...
static void trace_write_header(void)
{
    if (!trace_fp) return;
    fprintf(trace_fp,
            "# emu23 v2.15 trace / mc=%d lat=F%u,R%u,W%u,FILL%u"
...
static unsigned mem_charge(uint32_t pa, acc_kind_t kind)
{
     ...
     * 抑止対象は「課金・キャッシュ統計・物理アドレストレース」のみ。
     * MMIO 副作用は rd16()/wr8() 内部で起きるため本フラグでは止まらない
     * （設計書 §4.1.1.2 / 残存分は MW-2 で対処）。 */
 ...
 *   （設計書 emu23_mw12_fix_design_v0_5 §4.2）★
 * MMU 変換は行うが MMIO ディスパッチを行わない。
 * → SD_DATA 等の読み出し副作用・未マップ警告が発生しない。
 *
 * ★dbg_access を立てていないのは付け忘れではない（v0.1 レビュー C-1）★
 *   phys_rd8() は配列を直読するのみで mem_charge() を通らないため、
 *   課金・統計・トレースは原理的に発生しない。
 *   ここでフラグを立てると、外側の dbg_access 区間(7箇所)の内側から
 *   呼ばれた際に★内側の =0 が外側の抑止を解除する★事故を招く。
 * ★このコメントを消さないこと。消すと後日「親切に」足される。★
 *
 * ★副作用が無い代償（§4.5.1）★ MMIO レジスタの内容は一切読めなくなる。
 *   sd_buf_ptr 等は C 変数であり mem[] 上に存在しない。
 *   実機同等の経路で読む手段は MW-6 (peek) として別途起票済み。 */
...
/* ★[v2.14 MW-2] ワード合成の単一実装（設計書 §4.3）★
 * dump_memw() と CLI -m の 2 実装が rd16() を各々呼んでいた（原則122）。
 * 内側の取得をここに一本化し、片方だけ直す事故を構造的に消す。
 * バイト順は LE（rd16() と同一）。
 * ★(uint16_t) キャストはラップが意図的であることを示す★ */
...
static void dump_mem(uint16_t addr, int n) {
    for (int i = 0; i < n; i += 16) {
        ...
        /* ★[v2.14 MW-2] mem[] 直読 → dbg_rd8() 経由（設計書 §4.3-1）★
         * v2.13 まで論理アドレスで mem[] を引いていたため、MMU 有効時に
         * 別の場所を見ていた（setb 0400 の直後に mem 0400 が 00 を返す）。
         * ★16進部と ASCII 部の 2 箇所を必ず同時に直すこと（原則122）★
         * ★非 MMU 時は phys_rd8() が mem[] に落ちるため恒等（G-4 保持）★ */
        for (int j = 0; j < 16; j++) {
            if (i + j < n) printf("%02x ", dbg_rd8((uint16_t)(addr + i + j)));
            else printf("   ");
        }
        printf(" |");
        for (int j = 0; j < 16 && i + j < n; j++) {
            uint8_t c = dbg_rd8((uint16_t)(addr + i + j));
...
static void dump_memw(uint16_t addr, int n) {
    ...
    /* ~~★v2.13 A-1: デバッガ由来アクセス（設計書 §4.1.1）★
     *   rd16() 経由のため課金・統計・トレースを汚染する。ここで抑止する。
     *   ★ただし MMU 変換と MMIO 副作用は止まらない（§4.1.1.2）★
     *     → memw $FC80 は依然 UART FIFO を進める。解消は MW-2。
     *   ★KY59: 途中 return を作らず、必ず末尾で 0 に戻すこと★
     *   dbg_access = 1; ... dbg_access = 0;~~
     * ★[v2.14 MW-2] 上記は dbg_rd16() 経由化により不要となったため削除
     *   （設計書 emu23_mw12_fix_design_v0_5 §4.3-2。KY41 で旧記述を保持）★
     *   dbg_rd16() は phys_rd8() を呼ぶだけで mem_charge() を通らないため、
     *   フラグを立てずとも課金・統計・トレースは発生しない。
     * ★なお起票文言「memw $FC80 は UART FIFO を進める」は誤りであった★
     *   ysd8001_rx_read() は副作用を持たない。実際に副作用を持つのは
     *   SD_DATA($FCAA) であり、読むと sd_buf_ptr が +1 進む（§2.5）。 */
    for (int i = 0; i < n; i += 8) {
        uint16_t base = (uint16_t)(addr + i * 2);
        printf("%04x: ", base);
        for (int j = 0; j < 8 && i + j < n; j++) {
            uint16_t a = (uint16_t)(base + j * 2);
            printf("%04x ", dbg_rd16(a));   /* ★[v2.14 MW-2] §4.3-2★ */
        }
        printf("\n");
    }
}
...
/* ★[v2.13] コマンド名のトークン完全一致判定（設計書 §5.1.1）★
 * kw と完全長一致し、かつ直後が空白/改行/終端であるときのみ真。
 * 戻り値: 引数部分の先頭ポインタ（不一致なら NULL）
 *
 * ★なぜ必要か★
 * 既存 REPL は cmd[0]=='s' 等の前方一致で分岐しており、
 * 短いキーワードが長いキーワードを食う事故が既に2件起きている
 *   ・bd が b に食われる（L2756 が L2768 より前にある理由）
 *   ・memw が mem に食われる（同上）
 * setb/setw/setpb/setpw は3例目にあたる（KY60）。
 * 本ヘルパは新規4コマンドにのみ適用する。既存15分岐の構造是正は
 * 回帰範囲が広いため MW-1 として別チケットに分離した。 */
static const char *cmd_is(const char *cmd, const char *kw)
{
    size_t n = strlen(kw);
    if (strncmp(cmd, kw, n) != 0) return NULL;
    if (cmd[n] != '\0' && cmd[n] != ' ' && cmd[n] != '\t' &&
        cmd[n] != '\n' && cmd[n] != '\r') return NULL;   /* setbx 等を弾く */
    return cmd + n;
}

/* ★[v2.14 MW-1] REPL キーワード照合（設計書 emu23_mw12_fix_design_v0_5 §3.3）★
 * 返り値 = 引数開始位置 / NULL = 不一致
 *   kw 直後が 区切り(NUL/空白/改行) → 通常形     例 "mem 0400"
 *   kw 直後が 数字 or '$'           → 区切り無し引数形（後方互換・§3.2）
 *                                                例 "mem0400" "s5"
 *   kw 直後が それ以外（英字・'-' 含む）→ ★不一致★
 *
 * ★これが順序依存を消す★ "bd0" は kw="b" では 'd' が英字ゆえ不一致となり、
 *   分岐の並び順に関係なく "bd" へ届く。mem/memw, s/setb も同様（§3.4）。
 *
 * ★'-' を許容しない理由（M-4 案b）★
 ...
 *
 * ★cmd_is() は廃止せず残す（KY41）★ setb/setw/setpb/setpw は区切り無し
 *   引数形を持たないため、より厳格な cmd_is() のままとする。 */
static const char *repl_kw(const char *cmd, const char *kw)
{
    size_t n = strlen(kw);
    unsigned char c;
    if (strncmp(cmd, kw, n) != 0) return NULL;
    c = (unsigned char)cmd[n];          /* ★負値UB回避★ */
    if (c == '\0' || c == ' ' || c == '\t' || c == '\n' || c == '\r')
        return cmd + n;
...
/* ★[v2.13] MMIO 帯判定（設計書 §3.3 / §4.4-1）★
 * ★この判定は「警告表示」と「読み返し省略」の両方で共用する単一箇所である★
 * 原則122。判定が2箇所に分かれると片方だけ範囲が変わる事故が起きる。
 *
 * ★さらに重要（設計書 §3.3.1 / レビュー C-3）★
 * 本判定は二重変換事故の安全機構でもある。MMU 制御レジスタ(PTR/MCR)は
 * MMIO 帯にあり、ここで読み返しを省略するからこそ
 * 「書き込みでマッピングが変わった後に再変換して別番地を読む」事故が起きない。
 * ★判定範囲を変更する際は、警告表示だけでなく §3.3.1 の安全性も評価すること★ */
static int dbg_is_mmio(uint16_t addr) { return addr >= MMIO_BASE; }

/* ★[v2.13] 16進パース（設計書 §3.2）★
 * '$' 前置 / '0x' 前置 / 裸の16進を許容。全体が16進であることを要求する。
 * 戻り値: 1=成功 0=失敗（★失敗時 *out は変更しない。切り詰めもしない★） */
static int dbg_parse_hex(const char *s, unsigned long *out)
{
    char *end;
    unsigned long v;
    if (*s == '$') s++;
    ...
    errno = 0;
    v = strtoul(s, &end, 16);
    if (errno != 0 || *end != '\0') return 0;   /* 'zz' や '12g' を弾く */
    *out = v;
    return 1;
}

/* ★[v2.13] アドレス解決（設計書 §3.2）★
 * 16進が駄目ならシンボルとして解決する（既存 b コマンドと同じ経路 lookup_sym）。
 * ★N11: 解決失敗時は $0000 等へフォールバックせず、必ず失敗を返す★ */
static int dbg_parse_addr(const char *s, uint16_t *out)
{
    unsigned long v;
    uint16_t sym;
    if (dbg_parse_hex(s, &v)) {
        if (v > 0xFFFF) return 0;
        *out = (uint16_t)v;
        return 1;
    }
    if (lookup_sym((char *)s, &sym)) { *out = sym; return 1; }
    return 0;
}

/* ★[v2.13] デバッガ由来のバイト書き込み（設計書 §3.3.1 方法b）★
 * ★変換を1回だけ行い、確定した物理アドレスを *pa_out に返す★
 * 呼び出し側はこの pa をそのまま読み返しに使うこと。
 * 呼び出し側が mmu_translate() を再度呼ぶ(方法a)と二重変換になり、
 * MMU PTR を書き換えた場合に「書いた先とは別の物理アドレス」を読む。 */
static void dbg_write8(uint16_t la, uint8_t v, uint32_t *pa_out)
{
    dbg_access = 1;
    *pa_out = mmu_mode ? mmu_translate(la) : (uint32_t)la;
    wr8(la, v);                 /* MMIO 副作用は仕様どおり発火する(§4.3) */
    dbg_access = 0;
}

/* 確定済み物理アドレスからの読み返し（変換を再度行わない） */
static uint8_t dbg_readback8(uint32_t pa)
{
    uint8_t v;
    dbg_access = 1;
    v = phys_rd8(pa);
...
/* ★[v2.15 MW-6] デバッガ由来の「実機同等」バイト読み出し（設計書 §4.1）★
 * dbg_rd8() と対になる関数だが、★役割は正反対★である。
 *   dbg_rd8()  : 観測が対象を変えない（mem/memw/-m 用）
 *   dbg_peek8(): 意図的に MMIO 副作用を発火させる（peek 用）
 * ★この 2 つを共通化してはならない。共通化した瞬間に MW-2 が退行する★
 * ★このコメントを消さないこと。消すと後日「重複だから」と統合される。★
 *
 * dbg_access=1 は課金・キャッシュ統計・トレースの抑止のみを行い、
 * MMIO 副作用は止めない（L752-758 のコメント参照）。それでよい。
 * デバッガ操作で Dhrystone のサイクル数が変わることを防ぐのが目的である。
 *
 * ★読み出し副作用を持つレジスタは 4 本（設計書 §2.2 / F-11）★
 *   CYCLE_LO / SCORE_LO / SD_STAT / SD_DATA
 *   ここで分岐して除外することは★しない★（一覧保守は漏れる・§1.4）。
 *   実際、v0.1 は SD_DATA のみ、レビューが +2 本、実源全走査で更に
 *   SD_STAT が出た。★列挙に依存する実装は作らないこと★（KY65） */
static uint8_t dbg_peek8(uint16_t la)
{
    uint8_t v;
    dbg_access = 1;
    v = rd8(la);            /* ★MMIO 副作用は仕様どおり発火する★ */
    dbg_access = 0;
    return v;
}

/* ★[v2.15 MW-6] 実機同等のワード読み出し★
 * ★dbg_peek8() を 2 回呼んで合成してはならない★
 *   rd16() は 16bit スロットを 1 回で扱うのに対し、8bit 経路は
 *   「下位バイト読出時のみ副作用」規則を持つ（F-8）。合成すると
 *   ★実機と異なる副作用列★になる。
 *   （dbg_rd16() が dbg_rd8() の合成でよいのは、副作用が無いからである） */
static uint16_t dbg_peek16(uint16_t la)
{
    uint16_t v;
    dbg_access = 1;
    v = rd16(la);           /* バイト順は rd16() の実装に従う（独自定義しない） */
    dbg_access = 0;
    return v;
}

/* ★[v2.15 MW-6] peek / peekw の共通処理（設計書 §3）★
 * is_word=0 で peek（バイト）、1 で peekw（ワード）。
 * ★引数解析・上限判定・警告を 1 箇所に集約する★
 *   幅ごとに 2 つ書くと、片方だけ直す事故が起きる（原則122）。
 *   幅による差は「上限値の意味」と「1 アクセスの読み方」だけである。 */
#define PEEK_MAX_COUNT 64       /* ★設計書 §3.4：誤入力で sd_buf_ptr を
                                 * 一周(512)させないための上限★ */
static void repl_peek(const char *rest, int is_word)
{
    const char *name = is_word ? "peekw" : "peek";
    char as[64];
    uint16_t la;
    int n = 1, nread;
    const char *p = rest;

    if (sscanf(p, "%63s%n", as, &nread) != 1) {
        printf("Usage: %s <addr|label> [<count>]\n", name);
        return;
    }
    p += nread;
    if (!dbg_parse_addr(as, &la)) {
        printf("Bad address or unknown label: %s\n", as);
        return;
    }
    if (sscanf(p, "%d", &n) != 1) n = 1;    /* 省略時 1（§3.1） */

    /* ★範囲外は 1 バイトも読まずに戻る（§3.4）★
     * 部分実行すると「どこまで副作用が起きたか」が利用者に分からなくなる。 */
    if (n < 1 || n > PEEK_MAX_COUNT) {
        printf("Bad count: %d (must be 1..%d; nothing was read)\n",
               n, PEEK_MAX_COUNT);
        return;
    }

    /* ★警告は読み出しの★前★に出す（setb と対称・§3.5）★
     * count>=2 のときは発火回数を明示する（レビュー C-2）。 */
    if (dbg_is_mmio(la)) {
        if (n >= 2)
            printf("[WARN] MMIO read: side effects may occur (%d accesses)\n", n);
        else
            printf("[WARN] MMIO read: side effects may occur\n");
    }

    printf("%04x:", la);
    for (int i = 0; i < n; i++) {
        if (is_word)
            printf(" %04x", dbg_peek16((uint16_t)(la + i * 2)));
        else
            printf(" %02x", dbg_peek8((uint16_t)(la + i)));
    }
    printf("\n");
}
...
static void repl(void) {
    ...
    while (1) {
        ...
        /* ★[v2.14 MW-1] 分岐チェーン全体で使う引数開始位置★
         * repl_kw() の返り値を受ける。各分岐は sscanf(rest,...) を使い、
         * ★sscanf(cmd + N, ...) のリテラル数値は使わないこと★
         * （数値が残るとキーワード長を変えた際に無言でずれる）。
         * ★setb 分岐内にも同名のローカル rest があるが、あちらは
         *   cmd_is() 用でスコープが内側のため干渉しない（意図的に別物）★ */
        const char *rest;

        /* ★★[v2.13] 書き込み4コマンドは必ず 's' 分岐より前に置く（設計書 §5.1.1）★★
         * ここより下に置くと cmd[0]=='s' に食われ、
         * エラーも出さず「黙って1命令 step する」サイレント失敗になる（KY60）。
         * ★この4分岐を 's' より後へ移動してはならない★ */
        if (cmd_is(cmd, "setb") || cmd_is(cmd, "setw") ||
            cmd_is(cmd, "setpb") || cmd_is(cmd, "setpw")) {
            const char *rest;
            if ((rest = cmd_is(cmd, "setb")) != NULL) {
                char as[64], vs[32];
                uint16_t la;
                unsigned long v;
                uint32_t pa, pa0 = 0;
                ...
                if (sscanf(p, "%63s%n", as, &nread) != 1) {
                    printf("Usage: setb <addr|label> <val> [<val>...]\n");
                    continue;
                }
                p += nread;
                if (!dbg_parse_addr(as, &la)) {          /* N3 / N11 */
                    printf("Bad address or unknown label: %s\n", as);
                    continue;
                }
                if (sscanf(p, "%31s", vs) != 1) {         /* N2 */
                    printf("Usage: setb <addr|label> <val> [<val>...]\n");
                    continue;
                }
                if (dbg_is_mmio(la))                      /* §4.4-1 */
                    printf("[WARN] MMIO write: side effects may occur\n");

                while (sscanf(p, "%31s%n", vs, &nread) == 1) {
                    p += nread;
                    if (!dbg_parse_hex(vs, &v) || v > 0xFF) {   /* N12: 切詰めない */
                        printf("Bad byte value: %s (aborted after %d byte(s))\n",
                               vs, cnt);
                        break;
                    }
                    dbg_write8((uint16_t)(la + cnt), (uint8_t)v, &pa);
                    if (cnt == 0) pa0 = pa;
                    cnt++;
                }
                if (cnt == 0) continue;

                /* 読み返し（§3.3）。MMIO 帯は★明示付きで★省略する */
                if (dbg_is_mmio(la)) {
                    printf("%04x: <%d byte(s) written; readback skipped (MMIO)>\n",
                           la, cnt);
                } else {
                    printf("%04x:", la);
                    for (int i = 0; i < cnt; i++)
                        printf(" %02x", dbg_readback8(pa0 + (uint32_t)i));
                    printf("\n");
                }
                continue;
            }
            /* ---- setw: 論理アドレスへワード書き込み（§3.2）----
             * ★エンディアンは既存 wr16() の実装に従う（C-2）。本コマンドで
             *   独自にバイト順を定義しない。 */
            if ((rest = cmd_is(cmd, "setw")) != NULL) {
                char as[64], vs[32];
                uint16_t la;
                unsigned long v;
                uint32_t pa, pa0 = 0;
                ...
                if (sscanf(p, "%63s%n", as, &nread) != 1) {
                    printf("Usage: setw <addr|label> <val> [<val>...]\n");
                    continue;
                }
                p += nread;
                if (!dbg_parse_addr(as, &la)) {
                ...
                while (sscanf(p, "%31s%n", vs, &nread) == 1) {
                    p += nread;
                    if (!dbg_parse_hex(vs, &v) || v > 0xFFFF) {
                        printf("Bad word value: %s (aborted after %d word(s))\n",
                               vs, cnt);
                        break;
                    }
                    {   /* 変換は1回（§3.3.1 方法b）。wr16 は下位バイト側 pa を基点とする */
                        uint16_t a = (uint16_t)(la + cnt * 2);
                        dbg_access = 1;
                        pa = mmu_mode ? mmu_translate(a) : (uint32_t)a;
                        wr16(a, (uint16_t)v);
                        dbg_access = 0;
                    }
                    if (cnt == 0) pa0 = pa;
                ...
                if (dbg_is_mmio(la)) {
                    printf("%04x: <%d word(s) written; readback skipped (MMIO)>\n",
                           la, cnt);
                } else {
                    printf("%04x:", la);
                    for (int i = 0; i < cnt; i++) {
                        uint8_t lo = dbg_readback8(pa0 + (uint32_t)(i * 2));
                        uint8_t hi = dbg_readback8(pa0 + (uint32_t)(i * 2 + 1));
                        printf(" %02x%02x", hi, lo);   /* 表示は memw と同じ語表現 */
                    }
                    printf("\n");
                }
                continue;
            ...
            /* ---- setpb / setpw: 物理アドレスへ書き込み（§4.2）----
             * ★案1: MMIO ディスパッチを行わない。物理配列を直接書く★
             *   （既存 physmem の読み出しと対称。原則122） */
            {
                int is_w = (cmd_is(cmd, "setpw") != NULL);
                rest = is_w ? cmd_is(cmd, "setpw") : cmd_is(cmd, "setpb");
                if (rest != NULL) {
                    char as[64], vs[32];
                    unsigned long a, v;
                    int cnt = 0, nread;
                    ...
                    if (!mmu_mode) {                    /* N4 */
                        printf("set%s requires --mmu\n", is_w ? "pw" : "pb");
                        continue;
                    }
                    if (sscanf(p, "%63s%n", as, &nread) != 1) {
                        printf("Usage: set%s <phys-addr> <val> [<val>...]\n",
                               is_w ? "pw" : "pb");
                        continue;
                    }
                    p += nread;
                    if (!dbg_parse_hex(as, &a) || a > 0xFFFFF) {  /* N5: マスクしない */
                        printf("Bad physical address: %s (must be 20-bit)\n", as);
                        continue;
                    }
                    if (sscanf(p, "%31s", vs) != 1) {
                        printf("Usage: set%s <phys-addr> <val> [<val>...]\n",
                    ...
                    while (sscanf(p, "%31s%n", vs, &nread) == 1) {
                        unsigned long lim = is_w ? 0xFFFFUL : 0xFFUL;
                        uint32_t pa = (uint32_t)a + (uint32_t)(cnt * (is_w ? 2 : 1));
                        p += nread;
                        if (!dbg_parse_hex(vs, &v) || v > lim) {
                            printf("Bad value: %s (aborted after %d)\n", vs, cnt);
                            break;
                        }
                        if (pa + (is_w ? 1u : 0u) > 0xFFFFFu) {   /* N5: 範囲外は書かない */
                            printf("Physical address out of range (aborted after %d)\n",
                                   cnt);
                            break;
                        }
                        dbg_access = 1;
                        phys_wr8(pa, (uint8_t)(v & 0xFF));
                        if (is_w) phys_wr8(pa + 1, (uint8_t)(v >> 8));
                        dbg_access = 0;
                        cnt++;
                    }
                    if (cnt == 0) continue;
                    printf("%05lx:", a);
                    for (int i = 0; i < cnt; i++) {
                        uint32_t pa = (uint32_t)a + (uint32_t)(i * (is_w ? 2 : 1));
                        if (is_w) printf(" %02x%02x", dbg_readback8(pa + 1),
                                                      dbg_readback8(pa));
                        else      printf(" %02x", dbg_readback8(pa));
                    }
                    printf("\n");
                    continue;
                }
            }
            printf("[stub] write command reached\n");   /* 到達しない（保険） */

        // s [N]  — step N instructions
        } else if ((rest = repl_kw(cmd, "s")) != NULL) {  /* [v2.14 MW-1] */
        ...
        } else if (repl_kw(cmd, "c") != NULL) {          /* [v2.14 MW-1] */
            if (cpu.halted) { printf("CPU is halted\n"); continue; }
            do {
                exec_one();
        ...
        } else if (repl_kw(cmd, "t") != NULL) {          /* [v2.14 MW-1] */
            if (cpu.halted) { printf("CPU is halted\n"); continue; }
            do {
                exec_one();
                dump_regs();
        ...
        } else if ((rest = repl_kw(cmd, "bd")) != NULL) { /* [v2.14 MW-1] */
            int idx;
            if (sscanf(rest, "%d", &idx) == 1 && idx >= 0 && idx < bp_count) {
        ...
        } else if ((rest = repl_kw(cmd, "b")) != NULL) {  /* [v2.14 MW-1] */
            /* ★[v2.14 MW-1] cmd[1] → *rest（リテラル添字を残さない・§3.5）★ */
            if (*rest == '\n' || *rest == '\0') {
                printf("Num  Addr\n");
                for (int i = 0; i < bp_count; i++)
                    printf("%-4d %04x\n", i, breakpoints[i]);
        ...
        // regs  — dump registers
        } else if (repl_kw(cmd, "regs") != NULL) {       /* [v2.14 MW-1] */
            dump_regs();

        // disas [addr [n]]  — disassemble
        } else if ((rest = repl_kw(cmd, "disas")) != NULL) { /* [v2.14 MW-1] */
        ...
        } else if ((rest = repl_kw(cmd, "memw")) != NULL) {  /* [v2.14 MW-1] */
            uint16_t addr = cpu.pc;
            int n = 8;
            sscanf(rest, "%hx %d", &addr, &n);
            dump_memw(addr, n);
        ...
        } else if ((rest = repl_kw(cmd, "mem")) != NULL) {   /* [v2.14 MW-1] */
            uint16_t addr = cpu.pc;
            int n = 64;
            sscanf(rest, "%hx %d", &addr, &n);
            dump_mem(addr, n);

        /* ★[v2.15 MW-6] peekw / peek — 実機同等経路の読み出し★
         * 配置は「mem の直後」だが、★位置に機能上の意味はない★。
         * repl_kw() により全分岐が完全一致化された v2.14 以降、分岐順は
         * 挙動に影響しない（MW-1 の T-1-5 順序入替え等価試験で実証済）。
         * 読み出し系を 1 箇所に集めるのは★可読性のため。移動してよい★。
         *
         * ただし ★peekw は peek より前に置くこと★。repl_kw() の仕様上
         * "peekw" は kw "peek" に一致しない（直後 'w' が英字）ため順序
         * 非依存のはずだが、★依存しないことと依存しても安全なことは両立する★。
         * 将来 repl_kw() が変更された場合の保険である。 */
        } else if ((rest = repl_kw(cmd, "peekw")) != NULL) {  /* [v2.15 MW-6] */
            repl_peek(rest, 1);

        } else if ((rest = repl_kw(cmd, "peek")) != NULL) {   /* [v2.15 MW-6] */
            repl_peek(rest, 0);

        // irq N  — inject IRQ
        } else if ((rest = repl_kw(cmd, "irq")) != NULL) {   /* [v2.14 MW-1] */
            int n;
        ...
        } else if (repl_kw(cmd, "reset") != NULL) {      /* [v2.14 MW-1] */
            /* ★v2.13 A-2: REPL 由来のベクタ読み（設計書 §4.1.1）★
             * ★L3307 の main 側リセットとは扱いが異なる★
             *   main 側 = CPU のリセット動作そのもの → 包まない（§4.1.1.1）
             *   ここ    = REPL コマンド由来          → 包む
             * 基準は「REPL/CLI 操作に由来するか」であり
             * 「命令実行ループの外か」ではない。 */
            dbg_access = 1;
            cpu.pc          = rd16(0x0000);
            dbg_access = 0;
        ...
        // q  — quit
        } else if (repl_kw(cmd, "q") != NULL) {          /* [v2.14 MW-1] */
            break;

        // ===== MMU デバッガコマンド (v1.08移植: emu22 v1.10 :1253-1311) =====
        } else if ((rest = repl_kw(cmd, "mmu")) != NULL && mmu_mode) { /* [v2.14 MW-1] */
            // mmu / mmu en / mmu dis / mmu ptr N V
            char sub[32] = "";
            sscanf(rest, " %31s", sub);            /* ★MW-1: 2箇所のうち1★ */
            if (sub[0] == '\0') {
                ...
                int n, v;
                if (sscanf(rest, " ptr %d %x", &n, &v) == 2 &&   /* ★MW-1: 2箇所のうち2★ */
                    n >= 0 && n < 16 && v >= 0 && v < 256) {
                    mmu.ptr[n] = (uint8_t)v;
                    printf("PTR[%d] = %02X\n", n, v);
        ...
        } else if ((rest = repl_kw(cmd, "physmem")) != NULL && mmu_mode) { /* [v2.14 MW-1] */
            // physmem <physaddr_hex> [n]
            uint32_t pa = 0; int n = 16;
            sscanf(rest, " %x %d", &pa, &n);
            pa &= 0xFFFFF;
        ...
        // help
        } else if (repl_kw(cmd, "help") != NULL ||
                   strcmp(cmd, "?") == 0 || strcmp(cmd, "?\n") == 0) {
            /* ★[v2.14 MW-1] C-4: '?' は完全一致（"?5" を help に通す意味がない）★
             * fgets は改行を残すため "?\n" も受理する。 */
            printf(
                "Commands:\n"
                ...
                "  peek  <a> [n]  read n bytes via real path (MMIO fires)\n"
                "  peekw <a> [n]  read n words via real path (MMIO fires)\n"
                "  setb <a> <v>...  write bytes to a (logical; MMIO fires)\n"
                "  setw <a> <v>...  write words to a (logical; MMIO fires)\n"
                "  irq <id>       inject IRQ\n"
            ...
            if (mmu_mode) printf(
                ...
                "  setpb A <v>...   write bytes to physical A (no MMIO dispatch)\n"
                "  setpw A <v>...   write words to physical A (no MMIO dispatch)\n"
            );
        /* ★[v2.15 MW-7] 未知コマンドの黙殺を是正（設計書 §8）★
         * ~~v2.14まで: unknown command: silently ignore~~ ←v2.15で是正(KY41)
         *
         * MW-1 の試験計画(T-1-2/T-1-3/T-1-6)は "Unknown command" を期待値と
         * していたが v2.14 は未達だった（黙って捨てていた）。
         * ★誤入力と「機能が無い」ことが利用者から区別できなかった★
         * これは MW-6 と正面から衝突する: peek を打ち間違えた利用者が
         * 「MMIO はやはり読めない」と誤解する（§1.5）。
         *
         * ★空行(改行のみ)は従来どおり黙って無視する★
         *   プロンプト連打は常用操作であり、これを叱ると使い勝手を損なう。 */
        } else if (cmd[0] != '\n' && cmd[0] != '\r' && cmd[0] != '\0') {
            printf("Unknown command: %s", cmd);   /* cmd は改行を含む */
        }
    }
}
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        fprintf(stderr,
            "emu23 v2.15 (2026-08-23) for YSD8800 ISA2.3\n"
            "  - v2.15: MW-6 peek/peekw (read MMIO via real path; side effects\n"
            "           fire intentionally); MW-7 'Unknown command' for unknown input\n"
            "  - v2.14: MW-1 REPL dispatch (exact-match repl_kw); "
            "MW-2 dump path (dbg_rd8/dbg_rd16, no MMIO side-effect)\n"
            "  - v2.13: debugger memory write (setb/setw/setpb/setpw); dbg_access\n"
            "  - v2.12: LAT default 6->5 (calibrated; see cache_base_design v1.0)\n"
            "  - v2.11: cycle count table v1.1 correction (6 entries; -mc only)\n"
    ...
    cpu.irq_pending = -1;
    /* ★v2.13: ここは dbg_access で包まない（設計書 §4.1.1.1 / レビュー C-2）★
     * 判定基準は「REPL/CLI 操作に由来するか」であり
     * 「命令実行ループの外か」ではない。実機 CPU もリセット時にベクタを読む。
     * ・-mc のサイクル計上   : 非計上（cpu.cycle は以降 0 起点。339,240 は不変）
     * ・--cache-stats/trace  : ★2 アクセスが記録される。既知の仕様★
     *   （mc_design §3.4「リセット非計上」は -mc の方針であり、
     *     統計・トレースへの記録可否とは別論点。両者は矛盾しない） */
    ...
    if (run_steps > 0) {
        ...
        if (dump_addr >= 0) {
             ...
             *   dbg_access = 1; ... dbg_access = 0;~~
             * ★[v2.14 MW-2] rd16() → dbg_rd16() 経由化（設計書 §4.3-4）★
             *   ★これは MW-2 の「4 人目の兄弟」である（原則122）★
             *   起票文言が mem/memw/physmem とコマンド名で書かれていたため
             *   当初対象から漏れていた。同じ rd16() を呼ぶ以上、同じ欠陥を
             *   持つ。「どのコマンドか」ではなく「どの関数経由か」で対象を
             *   定義すること（KY63）。
             *   dbg_access は dbg_rd16() 経由化により不要（§4.2 と同じ理由）。
             * ★-m は -n 併用時のみ到達する。-q は L3739 で早期 return する
             *   ため本ブロックを通らない＝G-4 に影響しない（§2.6.1）★ */
            for (int i = 0; i < dump_len; i++)
                printf(" %04X", dbg_rd16((uint16_t)(dump_addr + i*2)));
            printf("\n");
        }
        return 0;
    }

    printf("YSD8800 ISA2.3 Emulator (emu23 v2.15%s) — reset vector = %04x\n",
           mmu_mode ? " +MMU" : "", cpu.pc);

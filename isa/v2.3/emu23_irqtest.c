...
/* ===== [Step8-I irqtest] 計測プローブ・対照実験用カウンタ ===== */
static unsigned long irq_accept_count[5] = {0,0,0,0,0}; /* IRQ0..4 受理回数 */
static unsigned long irq1_protect_count  = 0; /* 保護でIRQ1上書きをブロックした回数 */
static unsigned long irq1_revive_count   = 0;
#ifdef FIX_IRET
static int timer_in_service = 0;  /* [Step8-I試行] タイマーハンドラ実行中フラグ */
#endif /* 再評価でブロック済IRQ1を再pendingした回数 */
#ifndef COLLIDE_INTERVAL
#define COLLIDE_INTERVAL 40001  /* R3確定: timer period(40000)と互いに素 */
#endif
...
static void ysd8004_raise(uint16_t bits) {
    ...
    if (irq_stat != 0) {
        ...
        if (cpu.irq_pending < 0 ||         /* 何もpending無し */
            ...
            irq1_protect_count++;  /* [irqtest] 保護発動を計測 */
        }
...
void exec_one(void) {
    ...
    if (cpu.irq_pending >= 0 && (cpu.flags & FL_IE)) {
        ...
        cpu.pc = vec;
        if (irq >= 0 && irq <= 4) irq_accept_count[irq]++;  /* [irqtest] 受理計測 */
#ifdef FIX_IRET
        if (irq == 1) timer_in_service = 1;  /* タイマーハンドラ実行中 */
#endif
#ifdef TRACE_COLLIDE
        if (cpu.cycle>=39990 && cpu.cycle<=40120)
          fprintf(stderr,"[T] cyc=%llu ACCEPT irq=%d stat=%04x next=%llu\n",
            (unsigned long long)cpu.cycle, irq, irq_stat,
            (unsigned long long)ysd8002.next_irq_cycle);
#endif
        if (!quiet_mode)
...
#ifdef FIX_IRET
        if (timer_in_service) {
            YSD8002_iret(cpu.cycle);   /* タイマーハンドラ復帰時のみ再設定 */
            timer_in_service = 0;
        }
#else
    ...
    if (cpu.halted) {
        ...
        /* [Step8-I irqtest] 計測サマリ */
        fprintf(stderr, "[IRQTEST] cycles=%llu  theoretical_timer_irq=%llu\n",
                (unsigned long long)cpu.cycle,
                (unsigned long long)(cpu.cycle / 40000ULL));
        fprintf(stderr, "[IRQTEST] accept by irq_pending: p1(timer)=%lu p2(YSD8004)=%lu p3(NMI)=%lu p4(SYSCALL)=%lu p0(unused)=%lu\n",
                irq_accept_count[1], irq_accept_count[2],
                irq_accept_count[3], irq_accept_count[4], irq_accept_count[0]);
        fprintf(stderr, "[IRQTEST] irq1_protect=%lu  irq1_revive=%lu\n",
                irq1_protect_count, irq1_revive_count);
#ifdef NO_IRQ_PROTECT
        fprintf(stderr, "[IRQTEST] build=poc-A (NO_IRQ_PROTECT: 旧バグ再現)\n");
#else
        fprintf(stderr, "[IRQTEST] build=poc-B (protect ON: v1.06現行)\n");
#endif
        return;
    }
...
#ifdef FIX_REEVAL
    /* [Step8-I 試行修正] 再評価前にタイマー(IRQ0)を先に評価し、§3.2の優先順位を守る */
    if (cpu.irq_pending < 0 && YSD8002_tick(cpu.cycle)) {
        cpu.irq_pending = 1;  /* タイマー優先 */
    }
    ...
    if (cpu.irq_pending < 0 && irq_stat != 0) {
        ...
        irq1_revive_count++;  /* [irqtest] 再評価復活を計測 */
    }
#endif

#ifdef IRQ_COLLIDE
    /* [irqtest] 合成衝突注入: period(40000)と素な間隔でIRQ1を撃ち込み衝突最大化 */
    if ((cpu.cycle % COLLIDE_INTERVAL) == 0) ysd8004_raise(IRQ_STAT_BIT_STOR);
#endif
#ifdef TRACE_COLLIDE
    if (cpu.cycle>=39990 && cpu.cycle<=40120)
      fprintf(stderr,"[T] cyc=%llu TAIL pend=%d stat=%04x next=%llu\n",
        (unsigned long long)cpu.cycle, cpu.irq_pending, irq_stat,
        (unsigned long long)ysd8002.next_irq_cycle);
#endif
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        fprintf(stderr,
            "emu23 v1.06 (2026-06-20) for YSD8800 ISA2.3\n"
    ...
    printf("YSD8800 ISA2.3 Emulator (emu23 v1.06) — reset vector = %04x\n", cpu.pc);

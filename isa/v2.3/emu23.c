...
//
// 追加機能:
//   YSD8002 タイマー抽象化 (ysd8002_t)
//   Dhrystones/sec 算出 (SYSCALL 0x0010/0x0011)
//
// v1.23からの変更:
//   [ISA2.3] SYSCALL: imm16フェッチ削除・Aレジスタディスパッチ・常時IRQ4発火
//   [YSD8002] cpu_t.timer_cycle削除 → ysd8002_t に移管
//   [YSD8002] YSD8002_init/tick/iret/dhry_start/dhry_stop/report 追加
//   [DHRY] SYSCALL 0x0010/0x0011 でDhrystone計測・スコア算出
//
// emu22 v1.23からの継承変更点:
...
//
// build: gcc -std=c99 -O2 -Wall emu23.c -o emu23
...
typedef struct {
    ...
    uint64_t cycles_per_irq;   /* = cpu_freq_hz / irq_hz                   */
    uint64_t next_irq_cycle;   /* 次回IRQ発火サイクル値                    */
    int      irq_enabled;      /* タイマーIRQ有効フラグ (1=有効)           */
    uint64_t dhry_start_cycle; /* Dhrystone計測開始サイクル                */
    int      dhry_measuring;   /* 計測中フラグ                             */
} ysd8002_t;
...
static void YSD8002_init(uint64_t cpu_freq_hz, uint64_t irq_hz) {
    ...
    ysd8002.dhry_start_cycle = 0;
    ysd8002.dhry_measuring   = 0;
}
...
static void YSD8002_iret(uint64_t current_cycle) {
    ysd8002.next_irq_cycle = current_cycle + ysd8002.cycles_per_irq;
}

/* Dhrystone計測開始 */
static void YSD8002_dhry_start(uint64_t current_cycle) {
    ysd8002.dhry_start_cycle = current_cycle;
    ysd8002.dhry_measuring   = 1;
}

/* Dhrystone計測終了・スコア算出・表示 */
static void YSD8002_dhry_stop(uint64_t current_cycle, int number_of_runs) {
    if (!ysd8002.dhry_measuring) return;
    ysd8002.dhry_measuring = 0;
    uint64_t elapsed_cycles = current_cycle - ysd8002.dhry_start_cycle;
    if (elapsed_cycles == 0) {
        fprintf(stderr, "[YSD8002] Dhrystone: elapsed_cycles=0 (too fast?)\n");
        return;
    }
    /* 全演算をuint64_tで実施（16bit中間値禁止） */
    uint64_t elapsed_us         = elapsed_cycles * 1000000ULL / ysd8002.cpu_freq_hz;
    uint64_t dhrystones_per_sec = (uint64_t)number_of_runs * 1000000ULL / elapsed_us;
    ...
    fprintf(stderr, "    cpu_freq=%llu Hz  Number_Of_Runs=%d\n",
            (unsigned long long)ysd8002.cpu_freq_hz, number_of_runs);
}
...
void wr16(uint16_t a, uint16_t v) {
    ...
    if (a == UART_TX) {
        putchar((int)(v & 0xFF));      // UART TX: 16bit write, use lower byte
        fflush(stdout);
        return;
    }
...
void exec_one(void) {
    ...
    case 0x05: { /* SYSCALL  [ISA2.3: 1バイト・Aレジスタ番号渡し] */
        /* imm16フェッチなし（1バイト命令）*/
        uint16_t syscall_num = cpu.a;  /* Aレジスタ = syscall番号 */
        /* ★IRQ4発火前（スタック未変化）のタイミングでフック処理
         * rd16(cpu.sp): この時点で [SP+0]=引数1（FLAGS/PC未積）*/
        if (syscall_num == 0x0010) {
            /* TIMER_START: 引数なし */
            YSD8002_dhry_start(cpu.cycle);
        } else if (syscall_num == 0x0011) {
            /* TIMER_STOP: 引数1=Number_Of_Runs が [SP+0] にある */
            uint16_t runs = rd16(cpu.sp);
            YSD8002_dhry_stop(cpu.cycle, (int)runs);
        }
        /* 案B: 番号によらず常にIRQ4を発火（FPGA実機と同じ動作）*/
        if (!quiet_mode)
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        fprintf(stderr,
            "emu23 v1.00 (2026-04-16) for YSD8800 ISA2.3\n"
            "usage: emu23 prog.bin [prog.dbg [prog.sym]] [-n N] [-b ADDR] [-q]\n"
            "  -n N    run N steps and dump trace (non-interactive)\n"
            "  -b ADDR start tracing after reaching ADDR\n"
            "  -q      quiet: run until HALT, UART output only (no diagnostics)\n"
    ...
    printf("YSD8800 ISA2.3 Emulator (emu23 v1.00) — reset vector = %04x\n", cpu.pc);

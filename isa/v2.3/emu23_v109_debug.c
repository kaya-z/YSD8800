...
/* [v1.07 Step8-I] タイマーIRQ実行中フラグ。IRET時のYSD8002_iret呼出を */
/* タイマーIRQ(IRQ0)復帰時のみに限定する（ysd8002_timer_design §5.3準拠）。 */
/* これにより他IRQ(IRQ1等)のIRETでタイマー発火が先送りされる欠陥を解消。   */
static int timer_in_service = 0;
...
static void it_enable_raw_mode(void) {
  fprintf(stderr, "[DEBUG] it_enable_raw_mode() CALLED\n");
  if (!isatty(STDIN_FILENO)) {
        fprintf(stderr, "[DEBUG] stdin is NOT a TTY - raw mode will not work\n");
    }
    struct termios raw;
    if (tcgetattr(STDIN_FILENO, &it_orig_termios) == -1) {
        fprintf(stderr, "[DEBUG] tcgetattr failed: %s\n", strerror(errno));
        return;
    }
    raw = it_orig_termios;
    ...
    if (tcsetattr(STDIN_FILENO, TCSANOW, &raw) == -1) {
        fprintf(stderr, "[DEBUG] tcsetattr failed: %s\n", strerror(errno));
        return;
    }
...
static void ysd8001_poll_rx_interactive(void) {
  ...
  if (++call_count == 1) fprintf(stderr, "[DEBUG] poll_rx_interactive FIRST CALL\n");

    if (ysd8001.stat & YSD8001_STAT_RX_READY) {
      static int skip_logged = 0;
      if (!skip_logged) { fprintf(stderr, "[DEBUG] RX_READY already set, skipping read()\n"); skip_logged = 1; }
      return;
    }
    ...
    static int read_logged = 0;
    if (!read_logged && n != 0) {
        fprintf(stderr, "[DEBUG] read() returned n=%zd\n", n);
        read_logged = 1;
    }
...
uint16_t rd16(uint16_t a) {
    ...
    /* MMUレジスタ読み出し (--mmuモード時, 16bitは各バイトをrd8委譲) */
    ...
    /* 通常メモリ: MMU変換 (非mmu時は恒等写像=mem[]直). ページ境界またぎは各バイト独立変換 */
    {
        ...
        uint8_t hi = phys_rd8(mmu_translate((uint16_t)(a + 1)));
...
void wr16(uint16_t a, uint16_t v) {
    ...
    /* MMUレジスタ書き込み (--mmuモード時, 16bitは各バイトをwr8委譲) */
    if (mmu_mode && a >= MMU_PTR_BASE && a <= MMU_MCR_ADDR) {
    ...
    /* 通常メモリ: MMU変換 (非mmu時は恒等写像=mem[]直). ページ境界またぎは各バイト独立変換 */
    phys_wr8(mmu_translate(a),                (uint8_t)(v & 0xFF));
    phys_wr8(mmu_translate((uint16_t)(a + 1)),(uint8_t)(v >> 8));
}
...
void exec_one(void) {
    ...
    if (cpu.irq_pending >= 0 && (cpu.flags & FL_IE)) {
        ...
        cpu.irq_pending = -1;
        if (irq == 1) timer_in_service = 1;  /* [v1.07 Step8-I] タイマーIRQ(IRQ0=pending1)受理 */
    ...
    case 0x04: /* IRET  FLAGS←pop / PC←pop  (§7.6) */
        ...
        /* [v1.07 Step8-I] タイマーIRQ復帰時のみ次回発火を再設定（§5.3準拠）。 */
        /* 他IRQのIRETで呼ぶと、タイマー発火が毎回先送りされ大量欠落する。   */
        if (timer_in_service) {
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        fprintf(stderr,
            "emu23 v1.09 (2026-06-27) for YSD8800 ISA2.3\n"
            "  - v1.09dd: interactive mode (-it) - termios raw mode, UART RX from keyboard (Step 8)\n"
    ...
    printf("YSD8800 ISA2.3 Emulator (emu23 v1.09%s) — reset vector = %04x\n",
           mmu_mode ? " +MMU" : "", cpu.pc);

// emu23_v112_poc.c v1.12-wip1 (2026-08-10): [TKT-03 / Phase B' B-1]
//   it_enable_raw_mode() に c_iflag のクリアを追加（2行）。ICRNL を落とさないと
//   Enter の CR($0D) が LF($0A) に変換され、YUI OS シェル SH-READLINE
//   (kernel_forth_v0_10_18.fs L3148: $0D のみ行終端) が行を確定しなかった。
//   落とす集合は cfmakeraw(3) の8フラグ + INPCK。IXON は ISIG を落とした設計
//   思想(生バイトをUARTへ渡す)との一貫性のため。
//   ★本件は実装漏れではなく設計の欠落（設計書 v1.2 §3.1 が c_iflag 未規定）。
//   根拠: emu23_interactive_mode_design_v1_5.md §3.1.1 / §4.2
//   影響範囲: -it 指定時のみ。-q/-i/既定モードは本関数を通らず非回帰。
//   ★版数は暫定(-wip1)。確定 v1.12 は B-C(改良2)完了時に一括発番する。
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        fprintf(stderr,
            "emu23 v1.12-wip1 (2026-08-10) for YSD8800 ISA2.3\n"
            "  - v1.12: TKT-03 raw mode c_iflag (ICRNL etc) - CR reaches YUI OS shell (B-1)\n"
    ...
    printf("YSD8800 ISA2.3 Emulator (emu23 v1.12-wip1%s) — reset vector = %04x\n",
           mmu_mode ? " +MMU" : "", cpu.pc);

// hasm23.c - YSD8800 ISA2.3 Assembler v1.04
// Version: 1.04 (2026-06-20)
// v1.04 [Step 8-F-2 道2/改修(C)]: YOFモードの .org 配置アドレス尊重（方式α-B「道2」）
//   現状: -cモードの .org は指定アドレスまでゼロパディングし巨大セクション(size=$510d)を生成。
//   是正: 最初の .org をセクション原点 sec_origin として記録し、空白パディングを廃止。
//         code_buf は 0 基準で詰める（code_buf位置 = pc - sec_origin）。
//   変更点（L-6 全波及箇所）:
//     1) PASS1/PASS2 最初の .org で sec_origin 記録・パディング抑止（2回目以降の .org は従来通り）
//     2) OUT_BYTE の W001 検出アドレス _wpos を絶対アドレス(code_buf_pos + sec_origin)で評価
//     3) .vector の vec_pos を id*2 - sec_origin（kernel sec_origin=0 では不変）
//        ※v1.04 追補: .vector を持つセクションは has_vector 判定で sec_origin=$0000 を強制し、
//          最初の .org が $0020 でもベクタ領域($0000-)を欠落させない（kernel_v12_7.asm 対応）。
//     4) write_yof: load_addr に sec_origin 出力（従来0固定）／flags に has_org ビット(0x10)
//        ／シンボル offset・reloc offset を sec_origin 減算で相対化（UNDEF は 0 のまま）
//   設計書: hasm23_xref_yof_design_v2_2.md（承認済 review v4.0）§3.5.2
//   ※版数注記: 設計書 §4 表記「v1.03」は設計時点(CHAT61)の暫定。v1.03 は (A)(B) 確定版の
//     ため、道2 追加は v1.04 とする（KY41 版数整合・要・設計書改版で是正）。
// v1.03 [Step 8-F-2]: クロスファイルシンボル参照対応
//   (A) .global 疑似命令追加（_始まり以外もGLOBAL export可能に・export専用/未定義はエラー）
//   (B) UNDEF複数参照対応（同一外部シンボルへの即値+分岐の複数参照を全reloc化）
//   設計書: hasm23_xref_yof_design_v1_1.md（承認済 review v2.0）
...
typedef struct {
    char name[64];
    uint16_t addr;
    int is_undef;   /* v1.03(B): UNDEF（外部参照・addr未確定）なら1。アセンブラ内部表現に閉じYOF形式には乗らない */
} symbol_t;
...
/* v1.03(A): .global 明示宣言リスト（_始まり以外もGLOBAL exportを可能にする） */
#define MAX_GLOBALS 256
static char g_globals[MAX_GLOBALS][64];
static int  n_globals = 0;
static int is_declared_global(const char *name) {
    for (int i = 0; i < n_globals; i++)
        if (strcmp(g_globals[i], name) == 0) return 1;
    return 0;
}
static void add_global_decl(const char *name) {
    if (is_declared_global(name)) return;
    if (n_globals < MAX_GLOBALS) {
        snprintf(g_globals[n_globals], 64, "%s", name);
        n_globals++;
    }
}
...
/* v1.04 道2(改修C): セクション原点。-cモードで最初の .org を記録する。
 *   sec_origin_set=0 のうちは「.org 未出現」。最初の .org 出現で sec_origin に
 *   その絶対アドレスを記録し set=1 とする。以降 code_buf は 0 基準で詰め、
 *   絶対アドレス = code_buf位置 + sec_origin の関係で W001・load_addr・offset相対化を行う。
 *   非-cモードでは未使用（従来の絶対 .org 動作を完全保持）。 */
static uint16_t sec_origin     = 0;
static int      sec_origin_set = 0;
/* v1.04 道2: .vector を1つでも持つファイルは「$0000 基準のベクタテーブルを持つ」とみなし、
 *   最初の .org が $0020 等であっても sec_origin を $0000 に固定する（ベクタ領域 $0000-
 *   を欠落させないため）。PASS1 で検出し PASS2 で参照する。kernel_v12_7.asm が該当。 */
static int      has_vector     = 0;
/* v1.04 道2: .vector を code_buf へ直接書くと、後続の最初の .org パディング（$0000→$0020 の
 *   ゼロ埋め）がベクタを上書きしてしまう（kernel_v12_7.asm は .vector→.org $0020 の順）。
 *   これを避けるため、YOFモードのベクタは (vec_pos, handler) を記録だけしておき、
 *   全アセンブル完了後（write_yof 直前）に code_buf へ反映する遅延書き込み方式とする。 */
#define MAX_PENDING_VEC 16
static struct { int pos; uint16_t val; } pending_vec[MAX_PENDING_VEC];
static int pending_vec_count = 0;
...
static char yof_sym_kind(const char *name) {
    /* v1.03(A): _始まり、または .global で明示宣言されたものをGLOBAL */
...
static int write_yof(const char *outpath, const uint8_t *code, int code_size,
    ...
    /* v1.04 道2(C-3): .org を持つセクションは flags bit4(0x10)=has_org を立てる。
       lnk23 はこれで「load_addr=$0000 でも固定配置」と「自動配置」を区別する。 */
    fputc(has_org ? (0x0B | 0x10) : 0x0B, f);  /* flags: ALLOC|EXEC|READ(=0x0B) (+has_org=0x10) */
    yof_wr16(f, 0);                     /* file_offset: 0（セクションデータ先頭） */
    yof_wr16(f, (uint16_t)code_size);  /* size */
    yof_wr16(f, sec_org);              /* v1.04 道2: load_addr = sec_origin（従来0固定） */
    ...
    for (int i = 0; i < nsym; i++) {
        ...
        /* シンボル種別を先に判定（UNDEF か否かで offset 相対化を分岐するため） */
        char kind = yof_sym_kind(syms[i].name);
        for (int r2 = 0; r2 < nrel; r2++) {
            if (strcmp(rls[r2].sym, syms[i].name) == 0) { kind = YSYM_UNDEF; break; }
        }
        /* v1.04 道2: 定義済みシンボル offset を sec_origin 減算で相対化。
           UNDEF（外部参照・addr 未確定）は 0 のまま出力する。 */
        uint16_t out_off = (kind == YSYM_UNDEF || syms[i].is_undef)
                           ? 0
                           : (uint16_t)(syms[i].addr - sec_org);
        yof_wr16(f, out_off);          /* offset（セクション相対） */
        fputc(kind, f);
    }
    ...
    for (int r = 0; r < nrel; r++) {
        fputc(0, f);                          /* sec_idx: 0 */
        /* v1.04 道2: reloc offset（パッチ位置）も sec_origin 減算で相対化 */
        yof_wr16(f, (uint16_t)(rls[r].offset - sec_org));  /* offset（セクション相対） */
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        fprintf(stderr, "hasm23 v1.04 (2026-06-20) for YSD8800 ISA2.3\n"
    ...
    while (fgets(linebuf, sizeof(linebuf), fp)) {
        ...
        if (line[0] == '.') {
            if (strncmp(line, ".org", 4) == 0) {
                if (opt_yof) {
                    /* v1.04 道2: PASS1 は無改修で正しい。pc を絶対アドレスへ進めるため
                     * ラベルは絶対値（forth なら $5108 等 sec_origin 基準）で記録される。
                     * これは道2 の「内部参照は sec_origin 基準絶対値で埋め込み（reloc 不要）」
                     * と一致。sec_origin の記録・パディング抑止は PASS2 側で行う
                     * （write_yof は PASS2 後に呼ばれるため offset 相対化に間に合う）。 */
                ...
                // サイズなし (別アドレスに書く)
                if (opt_yof) has_vector = 1;  /* v1.04 道2: ベクタ$0000基準セクション判定 */
            ...
            } else if (strncmp(line, ".global", 7) == 0) {
                /* v1.03(A): .global SYM — SYMをGLOBAL export宣言（PASS1で登録・export専用） */
                char gsym[64];
                if (sscanf(line + 7, "%63s", gsym) == 1) {
                    add_global_decl(gsym);
                } else {
                    fprintf(stderr, "Invalid .global (needs symbol) at line %d\n", lineno);
                    return 1;
                }
            }
            continue;
    ...
    rewind(fp);
    pc = 0; lineno = 0;
    sec_origin = 0; sec_origin_set = 0;   /* v1.04 道2: PASS2 で最初の .org を捕捉するため明示初期化 */
...
#define OUT_BYTE(b) do { \
    long _wpos; \
    if (opt_yof) { _wpos = code_buf_pos + sec_origin; code_buf[code_buf_pos++] = (uint8_t)(b); } \
    ...
    while (fgets(linebuf, sizeof(linebuf), fp)) {
        ...
        if (line[0] == '.') {
            if (strncmp(line, ".org", 4) == 0) {
                if (opt_yof) {
                    /* v1.04 道2(改修C): -cモードの .org 処理。
                     *  - 最初の .org: セクション原点 sec_origin として記録するのみ。
                     *    ゼロパディングは挿入しない（従来は $0000〜指定アドレスまで巨大空白を
                     *    生成し size=$510d 等になっていた問題を是正）。pc は絶対アドレスで進める。
                     *    code_buf_pos は 0 のままなので、以降 絶対アドレス pc = code_buf_pos + sec_origin。
                     *  - 2回目以降の .org: 従来通りセクション内部の前方パディング（ベクタ等の
                     *    絶対配置維持）。後退 .org はエラー。 */
                    uint16_t new_pc = parse_addr(line + 4);
                    if (!sec_origin_set) {
                        /* v1.04 道2: .vector を持つセクション（has_vector）は $0000 基準。
                         * 最初の .org が $0020 等でも sec_origin=$0000 に固定し、$0000〜new_pc を
                         * 従来通りゼロパディングする（ベクタ領域 $0000-$001F を欠落させない）。
                         * .vector を持たないセクション（forth $5100 等）は従来の道2動作
                         * （sec_origin=new_pc・パディング廃止）。 */
                        if (has_vector) {
                            sec_origin = 0;
                            sec_origin_set = 1;
                            /* pc は現在 0。new_pc までパディング（ベクタは .vector が後で埋める） */
                        ...
                        sec_origin = new_pc;
                        sec_origin_set = 1;
                        pc = new_pc;          /* pc は絶対アドレス。code_buf_pos は据え置き(0) */
                        dbg_count--;
                        continue;
                    }
                    if (new_pc < pc) {
                ...
                if (opt_yof) {
                    ...
                    /* v1.04 道2: ベクタは絶対アドレス id*2（$0000 基準）。code_buf 内位置は
                     * sec_origin 減算で相対化。ただし即書き込みは後続 .org パディングに
                     * 上書きされるため、(pos,val) を記録し write_yof 直前に反映（遅延書き込み）。 */
                    int vec_pos = id * 2 - (int)sec_origin;
                    if (vec_pos >= 0 && pending_vec_count < MAX_PENDING_VEC) {
                        pending_vec[pending_vec_count].pos = vec_pos;
                        pending_vec[pending_vec_count].val = handler;
                        pending_vec_count++;
                    }
                    dbg_count--;
                    continue;
                ...
                /* v1.03(A): PASS2では何もしない（PASS1で登録済み・コード非生成・PC不変） */
            }
            continue;
        }
        ...
        if (strcmp(mnem, "LDW") == 0 || strcmp(mnem, "STW") == 0) {
            ...
            if (is_lbl2) {
                symbol_t *s = find_sym(lbl2);
                /* v1.03(B) §3.3.2.1: need_relocフラグで3分岐合流・reloc生成は分岐外1箇所に集約 */
                int need_reloc = 0;
                if (!s) {
                    if (opt_yof && has_imm2) {
                        /* (1) s==NULL: UNDEF新規登録（is_undef=1）→ reloc生成（集約点） */
                        if (sym_count < MAX_SYM) {
                            strcpy(symbols[sym_count].name, lbl2);
                            symbols[sym_count].addr = 0;
                            symbols[sym_count].is_undef = 1;
                            sym_count++;
                        }
                        need_reloc = 1;
                    } else {
                    ...
                    /* (2) s!=NULL && is_undef: 登録済みUNDEF→再登録せずreloc生成（集約点・同一） */
                    need_reloc = 1;
                } else {
                    /* (3) s!=NULL && !is_undef: 定義済み（addr確定）→reloc不要 */
                    imm2 = s->addr;
                }
                /* ★ reloc生成の唯一の集約点（即値経路: offset = pc+2） ★ */
                if (need_reloc) {
        ...
        // ========== 基本命令 ==========
        {
            ...
            if (is_lbl2) {
                symbol_t *s = find_sym(lbl2);
                /* v1.03(B) §3.3.2.1: need_relocフラグで3分岐合流・reloc生成は分岐外1箇所に集約。
                   分岐経路は相対分岐(imm_is_rel)を別扱い（自ファイル内解決前提・reloc対象外）。 */
                int need_reloc = 0;
                if (!s) {
                    if (opt_yof && in->has_imm && !in->imm_is_rel) {
                        /* (1) s==NULL（絶対参照）: UNDEF新規登録（is_undef=1）→ reloc生成（集約点） */
                        if (sym_count < MAX_SYM) {
                            strcpy(symbols[sym_count].name, lbl2);
                    ...
                    /* 相対分岐: 定義済みシンボルへの相対変位（reloc不要） */
                    imm2 = s->addr - (pc + 1 + in->has_reg + (in->has_imm ? 2 : 0));
                } else if (s->is_undef) {
                    /* (2) s!=NULL && is_undef（絶対参照）: 再登録せずreloc生成（集約点・同一） */
                    need_reloc = 1;
                } else {
                    /* (3) s!=NULL && !is_undef: 定義済み（addr確定）→reloc不要 */
                    imm2 = s->addr;
                }
                /* ★ reloc生成の唯一の集約点（分岐経路: offset = pc+1+has_reg） ★ */
                if (need_reloc) {
                    if (rel_count < MAX_REL) {
    ...
    if (opt_yof) {
        /* v1.03(A・C-2): .global は export専用。宣言シンボルが当該ファイルで
           未定義（未登録 or UNDEF）ならエラー（実体定義必須）。 */
        for (int gi = 0; gi < n_globals; gi++) {
            symbol_t *gs = find_sym(g_globals[gi]);
            if (!gs || gs->is_undef) {
                fprintf(stderr, "hasm23: error: .global symbol '%s' is declared but not defined "
                                "(.global is export-only)\n", g_globals[gi]);
                return 1;
            }
        }
        /* v1.04 道2: 遅延記録したベクタを code_buf へ反映（.org パディング完了後なので
           上書きされない）。has_vector セクションのベクタ領域は最初の .org パディングで
           確保済みのため vec_pos は code_buf_pos 範囲内に収まる。 */
        for (int v = 0; v < pending_vec_count; v++) {
            int vp = pending_vec[v].pos;
            if (vp >= 0 && vp + 1 < code_buf_pos) {
                code_buf[vp]   = (uint8_t)(pending_vec[v].val & 0xFF);
                code_buf[vp+1] = (uint8_t)((pending_vec[v].val >> 8) & 0xFF);
            } else {
                fprintf(stderr, "hasm23: warning: vector at pos $%04X outside code (size $%04X)\n",
                        vp, code_buf_pos);
            }
        }
        /* -cモード: YOF出力（v1.04 道2: sec_origin と has_org(=sec_origin_set) を渡す。
           .org を1つも持たない asm では sec_origin=0/has_org=0 となり完全後方互換）。 */
        if (write_yof(outobj, code_buf, code_buf_pos,
                      symbols, sym_count, rels, rel_count,
                      sec_origin, sec_origin_set) < 0) {
            return 1;
        }

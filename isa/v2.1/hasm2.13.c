// hasm.c - ISA2.1 Assembler v2.13
//  - ISA2.0 + EXT(0x1F) prefix for PUSH/POP/LDB/STB (ISA2.1)
//  - LDB/STB は 2 オペランド形式に完全対応 (reg , [addr] / reg , [X])
//  - [X + imm] の imm に # が無くても OK（例: [X + 2]）
//  - EQU / DW / DB (multi-element) / .org / .vector / .byte/.db
//  - LDW/STW (word load/store) with various addressing modes
...
int main(int argc, char **argv) {
    ...
    while (fgets(linebuf, sizeof(linebuf), fp)) {
        ...
        // --- ISA2.1: PUSH/POP (EXT prefix) ---
        if (strcmp(mnem, "PUSH") == 0 || strcmp(mnem, "POP") == 0) {
            if (n != 2) {
                fprintf(stderr, "%s expects 1 operand at line %d\n", mnem, lineno);
                return 1;
            }
            pc += 2; // 1F + subcode
            continue;
        ...
        // --- ISA2.1: LDB/STB (2 operands) ---
        if (strcmp(mnem, "LDB") == 0 || strcmp(mnem, "STB") == 0) {
            if (n != 3) {
                fprintf(stderr, "%s expects 2 operands at line %d\n", mnem, lineno);
                return 1;
            }

            char regpart[32], addrpart[64];
            strcpy(regpart, trim(op1));
            strcpy(addrpart, trim(op2));

            if (addrpart[0] != '[' || addrpart[strlen(addrpart)-1] != ']') {
                fprintf(stderr, "%s expects [..] operand at line %d\n", mnem, lineno);
                return 1;
            }

            char inside[64];
            strncpy(inside, addrpart+1, strlen(addrpart)-2);
            inside[strlen(addrpart)-2] = 0;
            char *ins = trim(inside);
        ...
        if (!in) {
            fprintf(stderr, "Unknown instr at line %d: %s\n", lineno, mnem);
            return 1;
        }
        pc += 1;
        if (in->has_reg) pc += 1;
    ...
    while (fgets(linebuf, sizeof(linebuf), fp)) {
        ...
        if (line[0] == '.') {
            ...
            if (strncmp(line, ".word", 5) == 0 || strncmp(line, ".dw", 3) == 0) {
                char valstr[64];
            ...
            if (strncmp(line, ".byte", 5) == 0 || strncmp(line, ".db", 3) == 0) {
                char valstr[128];
        ...
        char mnem2[32], opA[64], opB[64];
        int n2;
        parse_line(line, mnem2, opA, opB, &n2);

        if (mnem2[0] == '\0') continue;

	//	shin

	        // --- DW 疑似命令 ---
        if (strcmp(mnem2, "DW") == 0) {
            if (n2 < 2) {
                fprintf(stderr, "DW expects 1 operand at line %d\n", lineno);
                return 1;
            }
            int is_lbl = 0;
            char lbl[64];
            uint16_t v = parse_imm(opA, &is_lbl, lbl);
            if (is_lbl) {
                symbol_t *s = find_sym(lbl);
                if (!s) {
                    fprintf(stderr, "Undefined label %s at line %d\n", lbl, lineno);
	...
	//shin2
	        ...
	        // --- ISA2.1: PUSH/POP (EXT prefix) ---
        if (strcmp(mnem2, "PUSH") == 0 || strcmp(mnem2, "POP") == 0) {
            if (n2 != 2) {
                fprintf(stderr, "%s expects 1 operand at line %d\n", mnem2, lineno);
                return 1;
            }
            int sub = -1;
            if (strcmp(mnem2, "PUSH") == 0) {
                if      (strcmp(opA, "A") == 0) sub = 0x00;
                else if (strcmp(opA, "B") == 0) sub = 0x01;
                else if (strcmp(opA, "X") == 0) sub = 0x02;
            } else {
                if      (strcmp(opA, "A") == 0) sub = 0x03;
                else if (strcmp(opA, "B") == 0) sub = 0x04;
                else if (strcmp(opA, "X") == 0) sub = 0x05;
            }
            if (sub < 0) {
                fprintf(stderr, "Invalid operand for %s at line %d\n", mnem2, lineno);
                return 1;
        ...
        // --- ISA2.1: LDB/STB (2 operands) ---
        if (strcmp(mnem2, "LDB") == 0 || strcmp(mnem2, "STB") == 0) {
            if (n2 != 3) {
                fprintf(stderr, "%s expects 2 operands at line %d\n", mnem2, lineno);
                return 1;
            }
            ...
            char regpart[32], addrpart[64];
            strcpy(regpart, trim(opA));
            strcpy(addrpart, trim(opB));
            ...
            int is_A = (strcmp(regpart, "A") == 0);
            int is_B = (strcmp(regpart, "B") == 0);
            if (!is_A && !is_B) {
                fprintf(stderr, "%s only supports A or B at line %d\n", mnem2, lineno);
                return 1;
            }
            ...
            if (reg >= 0) {
                // [X]
                if (strcmp(mnem2, "LDB") == 0) {
                    if (is_A && reg == parse_reg("X")) sub = 0x11;
                    else if (is_B && reg == parse_reg("X")) sub = 0x13;
                } else {
                    if (is_A && reg == parse_reg("X")) sub = 0x15;
                    else if (is_B && reg == parse_reg("X")) sub = 0x17;
                }
                if (sub < 0) {
                    fprintf(stderr, "Unsupported reg/[X] at line %d\n", lineno);
                    return 1;
                }
                fputc(0x1F, fb);
                fputc(sub, fb);
                ...
                if (strcmp(mnem2, "LDB") == 0) {
                    if (is_A) sub = 0x10;
                    else if (is_B) sub = 0x12;
                } else {
                    if (is_A) sub = 0x14;
                    else if (is_B) sub = 0x16;
                }
                if (sub < 0) {
                    fprintf(stderr, "Unsupported reg/[addr] at line %d\n", lineno);
                    return 1;
                }
        ...
        if (strcmp(mnem2, "LDW") == 0 || strcmp(mnem2, "STW") == 0) {
            if (n2 != 3) {
                fprintf(stderr, "Expect 2 operands for %s at line %d\n", mnem2, lineno);
                return 1;
            }
            ...
            if (strcmp(mnem2, "LDW") == 0) {
                ...
                if (op2t[0] == '#') {
                    ...
                    if (reg >= 0) {
                        ...
                        char buf[64];
                        if (off[0] == '#') strcpy(buf, off);
                        else snprintf(buf, sizeof(buf), "#%s", off);

                        opcode = 0x26;
                        has_imm = 1;
                        imm = parse_imm(buf, &is_lbl, lbl);
                        rS = 0;
                    } else {
                        opcode = 0x22;
                        has_imm = 1;
            ...
            if (has_reg) {
                rD = parse_reg(trim(opA));
                rS = (n2 >= 3) ? parse_reg(trim(opB)) : 0;
                if (rD < 0) {
            ...
            if (has_imm) {
                const char *imm_str = (n2 >= 3) ? opB : opA;
	...
	//shin3
        ...
        printf("Assembled: %s  (v2.13: ISA2.1 EXT + LDB/STB 2-op + [X+imm] #省略対応 + LDW/STW + EQU/DW/DB)\n",
               outbin);
        return 0;
    }
}

...
Version 1.3  /  2026-05-03（最終改版: 2026-05-17）
...
| 関連設計書 | emu23_v103_design_v1_4.docx (emu23 改修)、emu23_device_design_v1_2.docx (YSD8003 仕様)、yuios_ph3_uart_design_v1_2.docx (Ph.3-A5 UART 設計) |
| 上位設計書 | yuios_design_v2_0.docx §6.3 ストレージドライバ |
| 作成日 | 2026-05-03 |
| ステータス | v1.3（v1.2 FIX に加え実装デバッグ修正反映済み） |
...
T1: LBA0 にパターン書込（src_buf に "STOR_TST" を配置 → STOR_WRITE）

T2: LBA0 から読み出し（dst_buf へ STOR_READ → src と一致確認）
...
```bash
# LBA0 (0-7 byte) に "STOR_TST" が書き込まれていることを確認
hexdump -C -n 16 disk.img
# 期待: 53 54 4f 52 5f 54 53 54 ...
```

## **8.4 BUSY ガードの単体動作確認**

LBA0 と LBA1 を同時要求するテストタスクを2つ起動して、BUSY (-2) が返ることを確認する（Phase 1 では実装オプション）。

...
| **v1.6** | **2026-06-25** | **Step 8 FPGA実装 V(-1)（emu23 v1.08 MMU復活移植）成果を反映。** ①emu23 **v1.07→v1.08**（emu22 v1.10→v1.21改版で脱落した FM-11方式16ページMMU〈mmu_translate/phys_mem/PTR[16]@\$FF00/MCR@\$FF10〉を emu22-1_10.c から復活移植。`--mmu` で有効化、無効時は恒等写像で v1.07 と byte-exact 非干渉。命令フェッチ26箇所を fetch8 化。MMUデバッガコマンド mmu/physmem も移植）。②§1 現行ツール一覧の emu23 行を v1.08 に更新。③§2 emu23 系譜に v1.08 行を追加。④検証ゲート全PASS: G1（mem[cpu.pc 残存0・fetch8×26）、G3（Dhrystone 826/48405/P:20、v1.07 baseline と v1.08〈--mmu無効〉完全一致）、G4（test_mmu1〜4_poc 全PASS）、非干渉（fib_verify_combined MD5一致）、**G2（道2フルビルドで yuios.bin=56416 バイト一致・lnk23 配置 forth@\$5100/kernel@\$0000/reloc=2 が手順書 v1.8 §4.11.5 I1実績と一致）**。⑤emu23 ソースバナー「MMU対応」陳腐化（旧指摘5）は v1.08 で実体が MMU 搭載となり解消。過去版系譜は欠落させず保持。 | Claude |

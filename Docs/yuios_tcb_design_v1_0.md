...
| 2 | 状態遷移の厳密化（WAIT_IPC/WAIT_REPLY/WAIT_MSG 境界） | §5 |
...
| 4 | `WAIT_MSG` | 旧 IPC（MSG-RECV）でメッセージ待ち | RUNNING |
...
#### 5.3.9 RUNNING → WAIT_MSG（旧 IPC、レガシ）

- 現状の YUI OS では `MSG-RECV` がほぼ未使用
- IPC4 系統に統合済みなので、新規コードでは使用禁止
- WAIT_MSG → READY の遷移は旧 `MSG-SEND` のみ（v2.0 で廃止候補）

### 5.4 状態の境界に関する重要原則（v1.0 で確定）
...
| WAIT_IPC (5) | `_ipc4_enqueue`（内部）／ `TASK-WAKEUP(tid)`（ドライバ用、§原則4） |
| WAIT_REPLY (6) | `IPC4-REPLY` のみ |

→ 「クライアントを `TASK-WAKEUP` で起こす」「サーバを `IPC4-REPLY` で起こす」のような**経路の混乱は禁止**。

#### 原則 3: state を変える者は必ず DI/EI 区間内にいる

state を変更する操作は**すべて**DI で囲む。理由：state を読んだ瞬間と書いた瞬間の間に IRQ が割り込むと一貫性が崩れる。
...
**v1.0 での対応方針：**
- **API 仕様で禁止**：「IPC4-CALL の処理中（クライアント側で CALL から RECV までの間）に新たな CALL を発行してはならない」
- 設計書 §10（KY 項目）に追記
- 検出機構：将来 timeout 機構（§10）が入れば、A の WAIT_REPLY が timeout して B に ENORECVR 相当のエラーを返す
...
| C. nested CALL | × デッドロック（API で禁止することで回避） |

→ シナリオ A/B は機構上正しく動く。C はマイクロカーネル一般の問題で、API 規約で対処する。
...
- [ ] **状態遷移の境界原則**（§5.4）：サーバ／クライアントの区別、起床経路の唯一性
- [ ] **scheduler 整合性検証**（§6）：3 シナリオ（A/B/C）の机上検証で十分か
- [ ] **nested CALL の API 規約**：§6.3 の禁止条項の妥当性
...
| K14 | nested CALL を API 規約で禁止しても誤って実装される | コードレビューでチェック。timeout 実装時に検出機構を追加 |
...
## 12. オープン項目（→ Ph.3.5-b へ持ち越し）

| 項目 | 持ち越し先 | 備考 |
|---|---|---|
| TCB プール絶対アドレス | Ph.3.5-b | $4000-$44FF が叩き台 |
| MsgPool 絶対アドレス | Ph.3.5-b | 512 B 確保 |
| タスクスタック領域配置 | Ph.3.5-b | 16 × $100 = 4 KB |
| カーネル変数領域整理 | Ph.3.5-b | レビュー指摘「最大消費量設計」「成長予約」「MMIO 境界固定」反映 |
| TEST バッファ再配置 | Ph.3.5-b | 現状 $E800-$EBFF |
| Forth コード成長余地確保 | Ph.3.5-b | 将来 FileMgr/Shell/libc 増加に備える |

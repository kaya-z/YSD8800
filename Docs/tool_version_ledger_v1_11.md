...
| **emu23** | **v1.10** | **emu23_v110.c** | エミュレータ（UART/Timer/SD/IRQ統合 + stack watermark計測 + FM-11方式16ページMMU〈--mmu〉 + インタラクティブモード〈-it〉 + **TCR-ACK方式タイマー再武装**） | **v1.10: [V5]黄金リファレンス改修。タイマー再武装契機をIRETフック→TCR bit5 IRQ_ACK書込へ変更（TCRマスク0x17→0x37）。timer_in_service削除・YSD8002_iret()→YSD8002_rearm()改名。理由=IRETフックはFPGA実装不能（iret_pulse_o不可・kaizen原則73）。★ハンドラはTCRに$0023書込義務★。設計書v5_design_memo。旧v1.09（-it追加）は§2系譜に保持** |

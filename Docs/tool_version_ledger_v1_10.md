...
| scc23 | **v2.03** | scc23_v2_03.c | Small-C 派生 C コンパイラ | **v1.04（ポインタバグ P1〜P7 修正系列）→ v2.00（float Q8.8/定数畳み込み/最適化レベル -O0/-O0-strict/-O1）→ v2.01（asm peephole を -O1 へ新設: P2a 完全同一連続MOV削除・P2b dead store除去。命令ストリームバッファ基盤）→ v2.02（peephole P3 追加: `ADD B,A`/`MOV A,B`→`ADD A,B` 可換吸収。可換性+dead proof で結果A・フラグ保存）→ v2.03（peephole P1 追加: スタックラウンドトリップ除去 4命令窓 `SUBI SP,#2`/`STW A,[SP]`/`LDW B,[SP]`/`ADDI SP,#2`→`MOV B,A`。G5=窓直後Z/N分岐で非置換のフラグ意味論保護付。--no-peep 検証フラグ追加）。-O0/-O0-strict は byte不変、-O1 で Dhrystone cycles 47885→47795・836 DPS・P:20** |
...
| **emu23** | **v1.09** | **emu23_v109.c** | エミュレータ（UART/Timer/SD/IRQ統合 + stack watermark計測 + FM-11方式16ページMMU〈--mmu〉 + **インタラクティブモード〈-it〉**） | **v1.09: Step 8 インタラクティブモード追加（termios raw mode・キー入力→UART RX・Ctrl+D終了・診断ログ抑制。既存破壊変更は poll_rx 関数ポインタ化の1行のみ・-q/-i/REPL 非回帰）。検証: yuios 56416/md5 一致、-it で YUI OS 実起動し ver/help/ps 応答到達・Ctrl+D終了PTY実証。設計書 emu23_interactive_mode_design v1.2** |

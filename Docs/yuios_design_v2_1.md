YUI OS v2.1 マイクロカーネル設計書	Version 2.1 / 2026-04-23（最終改版: 2026-05-18）
...
| $E200-$FBFF | 約6.5KB | Force VARIABLE / scc23 グローバル変数 | 既存 |
...
| **サービスID** | **名称** | **担当タスク** |
| --- | --- | --- |
| 0x01 | SVC_MEM | メモリマネージャ（MemMgr） |
| 0x02 | SVC_FILE | ファイルマネージャ（FileMgr） |
| 0x03 | SVC_PROC | プロセスマネージャ（ProcMgr） |
| 0x04 | SVC_UART | UARTドライバタスク |
| 0x05 | SVC_STORAGE | ストレージドライバタスク |
| 0x10-0xFF | (予約) | 将来拡張 |

## **4.5 IPC APIワード（kernel_forth_v0_8_5.fs v0.8.5 実装済み）【v2.1更新】**

| **ワード** | **スタック効果** | **説明** |
| --- | --- | --- |
| IPC4-SEND | ( msg0 msg1 msg2 msg3 tid -- ) | 4ワードメッセージを送信（ノンブロッキング） |
| IPC4-RECV | ( -- msg0 msg1 msg2 msg3 ) | 4ワードメッセージ受信（なければ WAIT-IPC でブロック） |
| IPC4-CALL | ( msg0 msg1 msg2 msg3 tid -- r0 r1 r2 r3 ) | 送信→応答待ち（同期RPC・WAIT-REPLY でブロック） |
| IPC4-REPLY | ( r0 r1 r2 r3 tid -- ) | IPC4-CALL の送信元に応答を返す |
...
## **7.1 Phase 1: 独自フラットFS**

Phase 1 では実装の簡易さを優先した独自フラットファイルシステムを使用します。ディレクトリ階層なし、固定エントリ数。

### **7.1.1 ディスクレイアウト**

| **セクタ番号** | **用途** | **説明** |
| --- | --- | --- |
| 0 | スーパーブロック | FS識別子・バージョン・エントリ数・最終更新 |
| 1-3 | ディレクトリエントリ | 最大32エントリ（1エントリ=48B） |
| 4- | ファイルデータ | セクタ単位で連続配置 |

### **7.1.2 ディレクトリエントリ構造（48B）**

| **オフセット** | **サイズ** | **フィールド** | **説明** |
| --- | --- | --- | --- |
| +0 | 16B | name[16] | ファイル名（NULL終端・拡張子含む） |
| +16 | 4B | size | ファイルサイズ（バイト）32bit |
| +20 | 2B | start_sec | 開始セクタ番号 |
| +22 | 2B | sec_count | 使用セクタ数 |
| +24 | 2B | flags | 属性（bit0=使用中, bit1=実行可能） |
| +26 | 22B | (予約) | 将来拡張（FAT12移行時に活用） |

## **7.2 FileMgr サービス（SVC_FILE = 0x02）**

| **操作名** | **opcode** | **引数** | **説明** |
| --- | --- | --- | --- |
| FILE_OPEN | 0x0201 | arg0=名前アドレス | ファイルオープン。戻り値=ファイルID（失敗-1） |
| FILE_CLOSE | 0x0202 | arg0=ファイルID | クローズ |
| FILE_READ | 0x0203 | arg0=ファイルID, arg1=dst, arg2=サイズ | 読み込み。戻り値=実際の読み込みバイト数 |
| FILE_WRITE | 0x0204 | arg0=ファイルID, arg1=src, arg2=サイズ | 書き込み |
| FILE_SEEK | 0x0205 | arg0=ファイルID, arg1=オフセット | シーク |
| FILE_STAT | 0x0206 | arg0=名前アドレス, arg1=stat_buf | ファイル情報取得 |
| FILE_LIST | 0x0207 | arg0=buf_addr, arg1=buf_size | ディレクトリ一覧取得 |
| FILE_DELETE | 0x0208 | arg0=名前アドレス | ファイル削除 |
...
| yuios_kernel_memmap_v1_1.docx | YUI OS カーネルメモリマップ v1.1 |

...
#   ※本番ソース（kernel_forth_*.fs / kernel_v12_11.asm）は非改変。実験ファイル名で処理（KY38）
set -e
...
echo "=== Step 5: kernel_v12_11.asm の \$e96e を #WORD_OS_START へ（sed系統②=No.6 廃止・UNDEF参照化）==="
# 本番ソース非改変: 実験ファイル名 $KASM に出力。#$e96e（2箇所）を UNDEF 参照 #$WORD_OS_START に置換
# hasm23 構文: シンボルアドレス即値参照は #$LABEL 形式（# の後に $ + シンボル名）
sed 's|#\$e96e|#$WORD_OS_START|g' kernel_v12_11.asm > "$KASM"

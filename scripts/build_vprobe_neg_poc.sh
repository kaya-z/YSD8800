...
#   ※本番ソース（kernel_forth_*.fs / kernel_v12_11_negM5_vprobe_poc.asm）は非改変。実験ファイル名で処理（KY38）
set -e
SRC="${1:-kernel_forth_v0_10_18.fs}"
FASM="kf_r2.asm"          # forth 実験asm
KASM="kf_neg_kern.asm"     # kernel 実験asm
FOBJ="kf_r2.asm.obj"
KOBJ="kf_neg_kern.asm.obj"
BIN="yuios_negM5_poc.bin"
SYM="yuios_negM5_poc.sym"
...
echo "=== Step 5: kernel_v12_11_negM5_vprobe_poc.asm の \$e96e を #WORD_OS_START へ（sed系統②=No.6 廃止・UNDEF参照化）==="
# 本番ソース非改変: 実験ファイル名 $KASM に出力。#$e96e（2箇所）を UNDEF 参照 #$WORD_OS_START に置換
# hasm23 構文: シンボルアドレス即値参照は #$LABEL 形式（# の後に $ + シンボル名）
sed 's|#\$e96e|#$WORD_OS_START|g' kernel_v12_11_negM5_vprobe_poc.asm > "$KASM"

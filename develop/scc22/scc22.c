/* scc22_v3_03.c - Small C Compiler for YSD8800 ISA2.2
 * Version: 3.03
 * YSD8800プロジェクト
 *
 * v1.01: フレームレイアウト修正
 * v2.00: Dhrystone対応 大規模拡張
 * v2.11: 修正A〜K適用（break SPリーク修正等）
 * v3.00: IR（中間表現）導入
 *   - 式を完全にSirNode*木として表現（設計書 scc22_v3x_arch_spec v1.1準拠）
 *   - parse_expr() → SirNode*返却に変更（フロントエンド／バックエンド分離）
 *   - emit_expr(SirNode*) によるコード生成（gen_*系廃止）
 *   - SIR_INDEX2D：2D配列アドレス計算を安全形で展開（修正L対応）
 *   - emit_expr() レジスタ契約明文化
 *     戻り値:A / B:破壊可 / X:不変 / SP:バランス維持
 *   - 文はparse_stmt()の構造を維持（軽めIR化）し
 *     emit_expr()呼び出しに置換
 * v3.01: M23〜M26適用済み
 * v3.02: [F1] value_kind_t / node_vkind() 導入（D1,D3,D4,R1対応）
 *        [F2] SIR_CALL引数評価 node_vkind判定・ADDI SP必須化（D2,R2,A4対応）
 *        [F3] emit_data_section DW統一・sizeof論理サイズ（D6,D7,R3,A3対応）
 *        [F4] ワーク変数定数を$FBD0系に変更（D9,A6対応）
 *        [F5] block stack方式によるSP管理（D8,R4,A2対応）← 廃止
 * v3.03: [A7] block stack方式廃止・frame_size方式に完全回帰（設計書v1.6準拠）
 * v3.04: [B7] node_vkind()でT_PTRをVK_RVALUEに修正（Ptr->Ptr_Comp引数渡しバグ修正）
 * v3.05: [B9] switch breakでswitch値をpopしてからJMP（スタック不均衡バグ修正）
 *        - block_sp_stack / block_sp_top / func_frame_size / loop_frame_stack 削除
 *        - parse_block_inner(): ブロック退出時のADDI SP廃止・suppress_unwind廃止
 *        - break: break_is_switch ADDI SP #2 廃止（JMPのみ）
 *        - エピローグ: LDW X,[X] → ADDI SP,#2 → ADDI SP,#2 → RET（ISA2.2対応）
 *        - SP不変条件: epilogue到達時 frame_size=累積値・SP=X-2（guard位置）
 *
 ...
 *   [X-4]  = ローカル変数2
 *
 * build: gcc -std=c99 -O2 -Wall scc22_v3_00.c -o scc22
 ...
 * バージョン
 * ============================================================ */
#define SCC_VERSION   "3.05"
#define SCC_DATE      "2026-04-15"
...
#define C_XSAVE_ADDR    0xFBD0   /* inc/dec addr退避 */
#define C_TMP_ADDR      0xFBD2   /* inc/dec旧値/strcpy/strcmp ワーク */
#define C_TMP_ADDR1     0xFBD4   /* [F4] 追加ワーク（設計書v1.5 D9） */
/* [F4] ランタイムワーク領域 $FBD0〜$FBE6（設計書v1.5 §7.1） */
#define C_MEMCPY_DEST   0xFBD6   /* memcpy dest ptr */
#define C_MEMCPY_SRC    0xFBD8   /* memcpy src ptr  */
#define C_MEMCPY_CNT    0xFBDA   /* memcpy count    */
#define C_MUL_BASE      0xFBDC   /* _cc_mul A */
#define C_MUL_B         0xFBDE   /* _cc_mul B */
#define C_MUL_R         0xFBE0   /* _cc_mul R */
#define C_DIV_BASE      0xFBE2   /* _cc_div A */
#define C_DIV_B         0xFBE4   /* _cc_div B */
#define C_DIV_Q         0xFBE6   /* _cc_div Q */
...
static sir_node_t *parse_unary(void) {
    ...
    if (strcmp(t,"*")==0) {
        sir_node_t *n=sir_unop(SIR_DEREF, parse_unary());
        n->type=T_INT; return n;
    }
    if (strcmp(t,"&")==0) {
...
static sir_node_t *parse_add(void) {
    ...
    for (;;) {
        char op[8]; strncpy(op,next_tok(),7); op[7]='\0';
        if (strcmp(op,"+")==0)      { n=sir_binop(SIR_ADD,n,parse_mul()); }
        else if (strcmp(op,"-")==0) { n=sir_binop(SIR_SUB,n,parse_mul()); }
        else { push_tok(op,cur_ival); break; }
    }
    return n;
}
...
static void emit_expr(sir_node_t *n) {
    ...
    case SIR_ASGN_OP: {
        ...
        case SIR_MUL: emit("    JSR  _cc_mul"); break;
        case SIR_DIV: emit("    JSR  _cc_div"); break;
        case SIR_MOD: emit("    JSR  _cc_mod"); break;
     ...
     * → ワーク変数($3FD0/$3FD2)を使って addr と 旧値 を保持
     *   C_XSAVE_ADDR($FBD0): アドレス退避
     *   C_TMP_ADDR  ($FBD2): 旧値退避
     * ── */
    case SIR_POST_INC: {
        emit_lval(n->left);
...
int main(int argc, char **argv) {
    ...
    for (int i=1;i<argc;i++) {
        if(strcmp(argv[i],"-o")==0&&i+1<argc){ outname=argv[++i]; }
            ...
            fprintf(stderr,"scc22 v%s (%s) for YSD8800 ISA2.2 [IR edition]\n",SCC_VERSION,SCC_DATE);
            return 0;
    ...
    fprintf(stderr,"scc22 v%s (%s) - compiling '%s'\n",SCC_VERSION,SCC_DATE,src_name);

    emit("; scc22 v%s (%s)  source: %s",SCC_VERSION,SCC_DATE,src_name);
    emit("; Target: YSD8800 ISA2.2 [IR edition]");
    emit(";");
    ...
    if(error_count>0)
        fprintf(stderr,"scc22: %d error(s) in '%s'\n",error_count,src_name);
    else

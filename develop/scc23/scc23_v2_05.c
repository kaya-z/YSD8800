/* scc23_v2_05.c - Small C Compiler for YSD8800 ISA2.3
 * Version: 2.05 (2026-08-02)
 *
 ...
 * バージョン
 * ============================================================ */
#define SCC_VERSION   "2.05"
#define SCC_DATE      "2026-08-02"
        ...
        if (strcmp(peek,"(")==0) {
               ...
               未定義(前方宣言なし)の場合のみT_INTフォールバック。 */
            {
                ...
                if (cfs && cfs->sclass==SC_FUNC) {
                    ...
                    /* [v2.05 C-3] 未定義関数呼び出し警告（早期通知）。
                       T_INT フォールバックは維持（int 文脈では従来動作を保つ）。
                       float 文脈で使われた場合は binop_promote で error 停止する。
                       (設計書 scc23_v2_05_C3_forward_decl_design_v0_3.md §2.4) */
                    warning("call to undeclared function '%s': assumed to return int. "
                            "add prototype declaration if returning float/pointer/char.",
                            name);

 ...
 *       (当時の旧メモリマップ「カーネルワーク変数$4200-$4230の直後」)
 ...
 * v1.5: _FMUL_A/B/R を $4232-$4236 から $47B0-$47B4 へ移動 (2026-05-24)
 *       【背景】YUI OS Ph.4 で TCB プール拡張 (v1.3 memmap: 16 タスク化
 *       $4000-$44FF) により TCB[7] フィールドが $4230-$4279 に配置された結果、
 *       _FMUL_A($4232)=TCB[7].PC, _FMUL_B($4234)=TCB[7].SP,
 *       _FMUL_R($4236)=TCB[7].DSP と完全衝突。Forth プログラム中の `*`
 *       実行で tid=7 の TCB が破壊され、READY 状態残留時にスケジューラが
 *       不正 PC/SP/DSP で実行を試みデッドロックする重大バグが発生した。
 *       メモリマップ v1.3 の「カーネルワーク予約 $47C0-$47FF」直前の
 *       「frequently used 領域 $4780-$47BF」の空き末尾 $47B0-$47B5 に移動。
 *       (報告: HANDOVER_CHAT32 真因②、確定: HANDOVER_CHAT33)
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void emit_header(codegen_t *cg) {
    ...
    /* 16bit乗算ルーチン ( a b -- a*b ) */
    /* ワーク変数: ISA2.3 RAM領域 $47B0-$47B4 (カーネルワーク変数$4200-$4230の直後) */
    fprintf(out, "; 16bit multiply runtime (uses $47B0-$47B4 as temp)\n");
    fprintf(out, "_FMUL_A  EQU $47B0\n");
    fprintf(out, "_FMUL_B  EQU $47B2\n");
    fprintf(out, "_FMUL_R  EQU $47B4\n");

...
\ Version: 0.10.18  v0.10.17 + Ph.6 Shell ver コマンド追加 (CHAT59)
\ YSD8800 YUI OS Microkernel
\
\ ★★★ v0.10.18 変更点 (2026-06-19) Ph.6: Shell ver コマンド追加 (CHAT59) ★★★
\   - SH-CMD-VER: OSバージョン "YUIOS V0.10.18" 表示(コード即値EMIT・DATA域消費ゼロ)
\   - SH-DISPATCH に先頭文字分岐方式を追加('v'→ver)。既存run/help/psは従来比較を温存
\   - (CHAT60追記) SH-CMD-MEM('m'): MEM_QUERY→"MEM: N/16" 表示
\   - (CHAT60追記) SH-ATOI + SH-CMD-KILL('k'): kill <tid>。tid7..15のみ許可、
\       保護(0..6)='P'/範囲外・非数字='?'/dead='D'/成功='K'。VARIABLE不使用
\ ★★★ v0.10.17 変更点 (2026-06-19) Ph.6: テスト分離 + Forth常駐Shell ★★★
\   (1) テストワード分離(辞書3093B回収):
\       MEM/UART/STOR-TEST群・DIAG群・PUTC-HEX群・BC-STR を退避ファイル
\       kernel_forth_tests_v0_10_16.fs へ分離(OS-START未起動デッドコード)。
\       辞書実終端 $C16F→$B55A(素)。検証復活はテストファイルから戻す。
\   (2) Ph.6 Forth常駐Shell実装(設計 yuios_ph6_shell_design_v1_1.md 承認済):
\       SHELL-TASK(tid=6)・行入力(SH-READLINE)・パース(SH-PARSE)・
\       ディスパッチ(SH-DISPATCH)・コマンド help/run/ps を実装。
\       SHバッファは DATA域上端($DC60-$DCF3)に CONSTANT固定配置。
\       OS-START に SHELL-START を追加(PROCMGR-START直後・tid=6)。
\       実証: run FIB→F55→復帰 / run 2回連続OK / ps→0 1 2 3 4 5 6。
\       辞書実終端(clean+Shell) $C623・天井$D3FF余裕3548B。
\   ※ 未実装コマンド(ls/cat/kill/mem/ver)と起動メッセージは次チャットで実装。
\   ※ ビルド: Step3 sedラベルリストに WORD_SHELL_TASK 追加が必須(本番恒久)。
\
\ ★★★ v0.10.16 変更点 (2026-06-18) Ph.5 Step4: PROC-DISPATCH バグ修正 ★★★
\   PROC-DISPATCH の op 分岐で client_tid が破壊されるバグを修正。
\   症状: PROC_EXEC 後 client が WAIT_REPLY で永久ブロック(W未達)。
\   原因: ( arg2 arg1 arg0 op tid ) から IMPL へ分岐する 4 箇所の
\         "IF DROP IMPL" の DROP が TOS の tid を捨て op を残していた。
\         IMPL は TOS を client_tid として受けるため op($0301等)が tid 化し、
\         存在しないタスクへ REPLY → client 永久ブロック。
\   修正: 当該 4 箇所の DROP を NIP に変更(op を破棄し tid を保持)。
\   検証: probe=0Ea123MDbcegF557W / exectest=0E123MDF557W / 非回帰=0123MD。
\   ※ 本修正で Ph.5 Step4(PROC_EXEC/PROC_WAIT 経由の実Cバイナリロード)が完成。
\
\ ★★★ v0.10.15 変更点 (2026-06-18) Ph.5 Step4: ProcMgr ロード領域移設 ★★★
\   PROC-LOAD-ADDR $C000 -> $D400（memmap v2.4・案D-ε・承認済）。
\   理由: 旧 $C000 が Forth 辞書実コード（実測終端 $C15F）と物理衝突し、
\   ProcMgr 経由ロード時に走行中カーネルを上書き破壊するため（HANDOVER_CHAT54/55）。
\   案D-ε で C プロセス領域を辞書外 $D400-$DBFF（2KB・子プロセス専用1スロット）へ移設。
\   変更は 560 行 PROC-LOAD-ADDR CONSTANT の 1 点のみ。参照箇所（FILE-READ宛先/TASK-CREATE
\   エントリ）は定数経由で自動追従。crt0=startup_proc v1.1（.org $D400）と整合。
\   MemMgr 5 定数（PAGE-POOL-BASE=$DD00 等）は v0.10.14 で移設済・本版変更なし。
\
\ ★★★ v0.10.13b 変更点 (2026-06-09) (b)当座辞書圧縮 ★★★
\   FILEMGR-TEST-TASK/-ADDR/-START を退避ファイル kernel_forth_tests_filemgr.fs
\   へ切り出し(約8.3KB)。Forthコード部 $E000 超過(W001)の当座解消。
\   本筋は(c) memmap v2.0。本圧縮は(c)完了までの延命措置。
\   ※FT-FID/FT-*-BUF CONSTANT は本体残置(無害)。FileMgr再検証時は退避ファイルをinclude。
\
\ ★★★ v0.10.13 変更点 (2026-06-08) Ph.5 ProcMgr Step3: 本体実装 ★★★
\   設計: yuios_ph5_procmgr_impl_design_v1_0
\   1. PROC_xxx opcode 定数 + PROC-LOAD-ADDR/MAX-SIZE 追加
\   2. PROC-TID-ADDR=$FC6A 追加（OS共有変数域・3ドライバ作法統一・C-1）
\   3. ProcMgr本体: ワークVARIABLE / PROC-REPLY(自前・層分離) /
\      PROC-EXEC/KILL/WAIT/LIST-IMPL / DISPATCH / PROCMGR-TASK / PROCMGR-START
\   4. OS-START を案A-2 に改修（テストタスク除外・PROCMGR-START追加・tid=5自然確保）
\   方針: FileMgr呼び出しは実体TEST同形の素の積み順で直書き（スタックジャグリング排除）
\
\ ★★★ v0.10.12 変更点 (2026-06-07) Ph.5 ProcMgr Step1: MemMgr プール移設（案C） ★★★
\   設計: yuios_design_v2_6.md §8.4 / §10.3 Step1, yuios_memmap_design_v1_7.md §5
\   目的: $C000-$DFFF(8KB) 三重重複(MemMgr×Forth成長予約×ProcMgrロード)を恒久解消。
\         先頭 $C000-$C7FF(2KB) を ProcMgr ロード領域へ転用し、MemMgr プールを
\         $C800-$DFFF(6KB・24page) へ縮小移設する（KY42 衝突解消）。
\   変更4定数:
\     PAGE-POOL-BASE   $C000 → $C800
\     PAGE-TOTAL       32    → 24
\     PAGE-USER-MAX    28    → 20
\     PAGE-BMP-HI-INIT $F000 → $FFF0
\   BMP-HI-INIT=$FFF0 の bit 検証（HI=page16-31）:
\     $FFF0 = bit4-15=1。page20-23(bit4-7)=OS予約4page、
\     page24-31(bit8-15)=無効page を恒久使用中マーク（実在24page中 page0-19 がユーザ用）。
\     §2.2/§5.3 確定仕様と一致。ALLOC は max_start=USER-MAX(20)-n で page0-19 のみ走査（R1）。
\   非回帰: アドレス計算式 base=POOL-BASE+page_no*256 は定数追従のためロジック変更なし。
\         FILE 系・IPC4・TCB に影響なし（MemMgr 定数のみ）。
\
\ ★★★ v0.10.10 追加点 (2026-06-03) Step 5-SEEK: FILE-SEEK-IMPL 実装 ★★★
\   設計: yuios_ph4_filemgr_design_v1_9_3.md §6.11（REV-PH4-SEEK-001 GO・C1反映済）
\   内容: FILE_SEEK ($0205) を実装。fid スロットの OT-POS を offset に設定する
\         絶対シーク（SEEK_SET）。ガード順序 [B]FID-VALID?→[C1]offset<0→
\         [C2]offset>size（==size は EOF 許可）→[D]OT-POS更新・r0=offset で REPLY。
\   変更2点:
\     (1) FILE-SEEK-IMPL 本体を新設（§6.11.3 スケルトン通り）。D スタック中心・
\         R は tid 1個・OT-POS のみ更新（OT-USED 不変・§5.3.2・KY19 非抵触）。
\         FM-WK ループ変数/MEMCPY-B/MEMSET-B 不使用＝5-6b 系再入問題と無縁。
\     (2) FILE-DISPATCH に FILE-SEEK-OP 分岐を追加（WRITE 分岐の後）。
\     (3) FILEMGR-TEST-TASK に FILE-SEEK-TEST 追加（§8.4.7・FS1/FS3/FS4）。
\         OPEN 済み fid に SEEK(5)→READ で pos 反映確認、SEEK(16)→E_INVAL、
\         SEEK(不正fid)→E_BADF。全合格で 'S'($53)、失敗で 's'($73) を出力。
\         期待出力: 0A BC123MPDQLORSCWVX（hello.txt 入り disk・S は R と C の間）。
\   KY35: r0=offset の値域衝突。Phase 1 は 32KB ディスクで offset∈[0,$7FFF] に収まる。
\   非回帰: SEEK はテストタスク未呼出のため正常出力 0A BC123MPDQLORCWVX は不変のはず。
\
\ ★★★ v0.10.9a 修正点 (2026-06-03) コメントのみ・コード無変更 ★★★
\   FILE-STAT-IMPL の見出し/メモにあった「v0.10.2a 定義のみ (FILEMGR-TASK からは呼ばれない)」を
\   実態 (v0.10.3 以降 FILE-DISPATCH に結線済・総合試験 'Q' で検証済) に訂正。γプローブの歴史は保持。
\   ※ 生成コードは v0.10.9 と完全同一 (コメント変更のみ)。再ビルド・Dhrystone 回帰は不要。
\
...
\ ProcMgr 自tid格納 (Ph.5・design v2.6 §3.1/C-1確定) v0.10.13
\   OS共有変数域 $FC6A（3ドライバ作法統一・MEM=$FC44/UART=$FC5C/STOR=$FC64）
$FC6A CONSTANT PROC-TID-ADDR    \ ProcMgr タスクID格納アドレス (=5)
...
\   memmap v1.1 §5.4
\ ============================================================
$ED00 CONSTANT TEST-SRC-BUF     \ 256B 書き込み元 v0.10.14 案A: $EC00→$ED00（memmap v2.1 §14.4(3)）
$EE00 CONSTANT TEST-DST-BUF     \ 256B 読み出し先 v0.10.14 案A: 据置だがSRC分離で重複解消（M-1）
...
\ --- PROC_xxx OP コード (Ph.5 ProcMgr・design v2.6 §8.3) v0.10.13 ---
$0301 CONSTANT PROC-EXEC-OP     \ バイナリをロードして実行
$0302 CONSTANT PROC-KILL-OP     \ タスク強制終了
$0303 CONSTANT PROC-WAIT-OP     \ タスク終了待ち
$0304 CONSTANT PROC-LIST-OP     \ タスク一覧
$D400 CONSTANT PROC-LOAD-ADDR   \ v0.10.15: $C000 -> $D400 (memmap v2.4 D-epsilon / C-proc area $D400-$DBFF)
$0800 CONSTANT PROC-MAX-SIZE    \ Phase1 サイズ上限 2KB
...
\ ★v0.10.14 案A (memmap v2.1 §14.4(3)・M-1解消): 768B枠$ED00-$EFFFへ再配置
\   SRC=$ED00/DST=$EE00/STAT=$EF00 を256B完全分離（同時使用ペアSRC↔DST重複ゼロ=M-1解消・K40）。
\   FT-NAME/FT-NAME2 は $5060/$5070 退避（FileMgr残余$5060-$50FF）。
$5060 CONSTANT FT-NAME-BUF       \ テスト用ファイル名 "hello.txt"  v0.10.14: $EC00→$5060退避
$ED00 CONSTANT FT-SRC-BUF        \ 書き込みデータ(1セクタ未満) v0.10.14: $EC10→$ED00（TEST-SRC兼/時系列共用）
$EE00 CONSTANT FT-DST-BUF        \ 読み出し先(FILE_READ dst)   v0.10.14: $EE00据置（TEST-DST兼/時系列共用）
$EF00 CONSTANT FT-STAT-BUF       \ FILE_STAT 結果              v0.10.14: $EF00据置（実12B/枠256B）
\ v0.10.8 (Step 5-6b・案C): 複数セクタ往復検証バッファ（§8.4.6.6）
\   ★v0.10.14 案A: FT-RW-BUF(1024B)は768B枠に物理的に入らないため、これを使うW1K重テストは
\     v2.1縮小版FileMgr試験から除外（memmap v2.1 §14.4(3)・CHAT51 §2.5方針）。復活はPh.8 MMU後。
\     CONSTANT定義はKY41に従い削除せず残置（縮小版では未使用）。
$EC00 CONSTANT FT-RW-BUF         \ src/dst 共用 1024B（W1K重テスト用・v2.1縮小版では未使用）
$5070 CONSTANT FT-NAME2-BUF      \ 5-6b 用ファイル名 "w1k.txt" v0.10.14: $5060→$5070（FT-NAMEと分離）
...
$DD00 CONSTANT PAGE-POOL-BASE   \ ページプール先頭  v0.10.14 案A: $C800→$DD00（memmap v2.1 §14.4）
16    CONSTANT PAGE-TOTAL        \ 総ページ数        v0.10.14 案A: 24→16（$DD00-$ECFF/256）
12    CONSTANT PAGE-USER-MAX     \ ユーザ用ページ数  v0.10.14 案A: 20→12（16中OS予約4除く）
$FFFF CONSTANT PAGE-BMP-HI-INIT \ HI初期値 v0.10.14 案A: $FFF0→$FFFF
                                \ HI=page16-31。$FFFF=bit0-15=1：page16-31は全無効（恒久使用中）
$F000 CONSTANT PAGE-BMP-LO-INIT \ ★LO初期値 v0.10.14 案A 新設（memmap v2.1 §14.5・K39）
                                \ LO=page0-15。$F000=bit12-15=1：
                                \   page12-15(bit12-15)=OS予約4page（16pg構成でLO側に来る）、
                                \   page0-11(bit0-11)=0=ユーザ空き。検算: LO=$F000/HI=$FFFF→
                                \   32bit $FFFFF000、空き(bit=0)=12個=PAGE-USER-MAX 一致
                                \ ★この値はビットマップ値であって実アドレスでない
...
\ ページビットマップ初期化
\ page 0-15: 全空き（$0000）
\ page 0-11: ユーザ空き（$0000）、page 12-15: OS予約、page16-31: 無効
\   v0.10.14 案A 16pg構成: LO=$F000(page12-15予約)/HI=$FFFF(page16-31無効)
: BMP-INIT  ( -- )
    PAGE-BMP-LO-INIT PAGE-BMP-LO !
...
: MEM-ALLOC-PAGES  ( n -- addr )
    ...
    DUP >R                        \ R=n
    PAGE-USER-MAX SWAP -          \ max_start = 20-n  v0.10.12 案C
    ...
    BEGIN
        ...
        IF
            ...
            256 * PAGE-POOL-BASE + \ addr = $C800 + start*256  v0.10.12 案C
...
: MEM-FREE-PAGES  ( addr n -- )
    ...
    OVER PAGE-POOL-BASE -         \ n (addr-$C800)  v0.10.12 案C
    DUP 0< IF
        DROP DROP EXIT
    THEN
    8 RSHIFT                      \ n page_no
...
\ 設計: yuios_ph4_filemgr_design_v1_9_3.md §6.11（REV-PH4-SEEK-001 GO）
\ in : arg0=fid, arg1=offset(絶対), arg2=0, tid=client
\ out: r0 = offset(0..size) / E-BADF / E-INVAL
\ 動作: fid スロットの OT-POS を offset に設定（SEEK_SET 絶対シーク）。
\       offset==size は EOF として許可、offset>size は E-INVAL。
\ 副作用: REPLY-OK 経由の FM-WK-A/B のみ。OT-POS を 1回 ! で更新（他状態不変・§5.3.2）。
\ ガード順序（符号付き演算の正しさ・KY24派生）:
\   [B] FID-VALID? → [C1] offset<0 → [C2] offset>size → [D] 更新+REPLY
\ KY35: r0=offset の値域衝突。Phase 1 は 32KB ディスクで offset∈[0,$7FFF] に収まる。
\ --------------------------------------------------------------------
...
\ --------------------------------------------------------------------
\ FILE-STAT-IMPL  ( arg2 arg1 arg0 tid -- )                   v0.10.3〜 結線・試験済
...
\
\ メモ(歴史): v0.10.2a では本ワードは定義のみで FILEMGR-TASK からは呼ばれず、
\             仮説γ (コードの存在自体がレイアウト破壊するか) の切り分けプローブとして追加した。
\ メモ(現状): v0.10.3 以降、FILE-DISPATCH ($0206 分岐, FILE-STAT-OP) に結線済みで
\             本番経路から呼ばれる。総合試験で FILE_STAT を送信し 'Q' 出力 (大文字=成功) として検証済み。
\             ※ v0.10.9a コメント訂正: 旧「定義のみ・呼ばれない」は実態と乖離した残骸のため修正。
\ --------------------------------------------------------------------
...
\ --------------------------------------------------------------------
\ FILE-DELETE-IMPL  ( arg2 arg1 arg0 tid -- )                v0.10.11
\ §4.3.7 / §6.5 FILE_DELETE ($0208)。arg0=name_addr / arg1,arg2=0。
\ 論理削除（FLG-USED クリア・データセクタ非回収・append-only §3.5）。
\ open 中は E-BUSY 拒否（§4.6）。file_count -=1。SB 書戻し3段（KY25）。
\ ループ状態は D/R スタックのみ（KY36：FM-WK 不使用・PICK不採用・R@採用）。
\ 戻り: E-OK / E-NOENT / E-BUSY / E-IOERR
\ --------------------------------------------------------------------
: FILE-DELETE-IMPL  ( arg2 arg1 arg0 tid -- )
    >R                                  \ R: tid   DSP:( arg2 arg1 name )
    NIP NIP                             \ arg1,arg2 捨て  ( name )
    \ [B] 手順1: DIR-LOAD（ディレクトリ最新化）
    DIR-LOAD 0<> IF
        DROP  E-IOERR R> REPLY-OK EXIT
    THEN                                \ ( name )
    \ [C] 手順2: DIR-FIND → di（不存在 -1）
    DIR-FIND                            \ ( di )
    DUP -1 = IF
        DROP  E-NOENT R> REPLY-OK EXIT
    THEN                                \ ( di )  di>=0
    \ [D] 手順3: open 中チェック（全4スロット走査・R@で di 覗き・KY36）
    \   di を R へ退避（R: tid di）。ループカウンタ i のみ D。FM-WK 不使用。
    >R                                  \ R: tid di   ( )
    0                                   \ ( i )  i=0
    BEGIN
        DUP FS-MAX-OPEN <               \ ( i flag )  i<4 ?
    WHILE
        DUP SLOT-ADDR                   \ ( i slot )
        DUP OT-USED + @ IF              \ used==1 ?
            OT-DIR-INDEX + @            \ ( i slot_dir_index )
            R@ = IF                     \ slot_dir_index == di(=R@) ?
                DROP  R> DROP  E-BUSY R> REPLY-OK EXIT   \ BUSY: i捨て・di捨て・tidでREPLY
            THEN                        \ ( i )  不一致
        ELSE
            DROP                        \ ( i )  used==0 枝で slot 捨て
        THEN
        1 +                             \ ( i+1 )
    REPEAT
    DROP                                \ ( )  i 捨て（未発見＝削除続行）
    R>                                  \ R: tid   ( di )  di 取り出し
    \ [E] 手順4: ロールバック退避（file_count のみ・next_free 不変）
    FS-FILE-COUNT @  FS-WR-OLD-COUNT !
    \ [F] 手順5: FLG-USED クリア（! は val=NOS / addr=TOS）
    DE-ADDR DE-FLAGS +                  \ ( flags-addr )
    0 SWAP !                            \ ( )  flags=0
    \ [G] 手順6: DIR-STORE（file_count 未変更ゆえ失敗時巻戻し不要）
    DIR-STORE 0<> IF
        E-IOERR R> REPLY-OK EXIT
    THEN
    \ [H] 手順7: メモリ確定更新（file_count -= 1）
    -1 FS-FILE-COUNT +!
    \ [I] 手順8: SB 書戻し3段（SB-LOAD→+26更新→SB-STORE・KY25）
    \   DELETE は next_free 不変ゆえ SECBUF+24 は SB-LOAD 値のまま・+26 のみ更新（C1）
    SB-LOAD 0<> IF
        FS-WR-OLD-COUNT @  FS-FILE-COUNT !
        E-IOERR R> REPLY-OK EXIT
    THEN
    FS-FILE-COUNT @  FS-SECBUF 26 +  !  \ SECBUF+26 = file_count（+24 は触らない）
    SB-STORE 0<> IF
        FS-WR-OLD-COUNT @  FS-FILE-COUNT !
        E-IOERR R> REPLY-OK EXIT
    THEN
    \ [J] 成功 REPLY: r0 = E-OK(0)
    E-OK R> REPLY-OK ;
...
: FILE-DISPATCH  ( arg2 arg1 arg0 op tid -- )
    ...
    THEN
    DUP FILE-SEEK-OP = IF
        DROP R> FILE-SEEK-IMPL EXIT     \ ★v0.10.10: FILE-SEEK-IMPL ( arg2 arg1 arg0 tid -- )
    THEN
    DUP FILE-DELETE-OP = IF
        DROP R> FILE-DELETE-IMPL EXIT   \ ★v0.10.11: FILE-DELETE-IMPL ( arg2 arg1 arg0 tid -- )
    THEN
...
\ --------------------------------------------------------------------
\ [v0.10.13 (b)圧縮] FILEMGR-TEST-TASK / -ADDR / -START を退避ファイル
\   kernel_forth_tests_filemgr.fs へ切り出し (2026-06-09)。
\   理由: Forthコード部 $E000 超過(W001)の当座解消。本番非搭載のため除去。
\   FileMgr再検証時は退避ファイルを include して復活させること。
\   ※FT-FID / FT-*-BUF CONSTANT 定義は本体に残置(数バイト・無害)。
\ --------------------------------------------------------------------

\ ====================================================================
\ Ph.5 ProcMgr 本体実装 (Step 3)  v0.10.13  2026-06-08
\ 設計: yuios_ph5_procmgr_impl_design_v1_0
\ 方針: スタックジャグリング排除のため、引数はワークVARIABLEに退避し
\       FileMgr呼び出しは FILEMGR-TEST-TASK と同一の素の積み順で直書きする
\       (KY39: 動作実績のある実体パターンを踏襲)
\ ====================================================================

\ --- ProcMgr ワーク変数 (design v2.6 §8.7・層分離: Forth VARIABLE) ---
VARIABLE LOAD-SLOT-BUSY     \ ロード領域占有フラグ (0=空き/非0=稼働tid)
VARIABLE PROC-CUR-TID       \ 現稼働プロセスtid
VARIABLE PROC-EXIT-CODE     \ 直近終了プロセスの終了コード (Phase1常時0)
VARIABLE PROC-WK-A          \ PROC-REPLY用 tid退避 (本書§4)
VARIABLE PROC-WK-B          \ PROC-REPLY用 r0退避
\ PROC_EXEC 内部退避用 (スタックジャグリング回避・本書D-2)
VARIABLE PE-NAME            \ ファイル名アドレス
VARIABLE PE-FID             \ FILE_OPEN で得た fid
VARIABLE PE-SIZE            \ FILE_STAT で得た size
VARIABLE PE-STATBUF         \ STAT 結果バッファ (ロード領域上端を流用)

\ --------------------------------------------------------------------
\ PROC-REPLY  ( r0 tid -- )                              本書§4
\ FileMgr REPLY-OK 流用回避(層分離)。IPC4-REPLY(r3 r2 r1 r0 tid--), r0=TOS
\ --------------------------------------------------------------------
: PROC-REPLY  ( r0 tid -- )
    PROC-WK-A !                 \ tid 退避
    PROC-WK-B !                 \ r0 退避
    0 0 0 PROC-WK-B @           \ ( 0 0 0 r0 )
    PROC-WK-A @ IPC4-REPLY ;

\ --------------------------------------------------------------------
\ PROC-EXEC-IMPL  ( arg2 arg1 arg0 tid -- )    op=$0301  本書§5.1
\   arg0 = ファイル名アドレス / tid = client_tid
\   フロー: BUSY検査→OPEN→STAT→size<=2KB→READ($C000)→CLOSE→
\           TASK-CREATE($C000)→登録→tid REPLY
\ --------------------------------------------------------------------
: PROC-EXEC-IMPL  ( arg2 arg1 arg0 tid -- )
    >R                          \ R: client_tid     DSP:( arg2 arg1 arg0 )
    PE-NAME !                   \ arg0=name 退避     DSP:( arg2 arg1 )
    DROP DROP                   \ arg1/arg2 破棄     DSP:( )
    \ STATバッファ: ロード領域は本体用なので、STAT結果は別領域が必要。
    \ FileMgr TEST が使う $EF00 系は衝突回避必要。ここでは FT-STAT-BUF($EF00) を流用。
    ...
    \ (2) FILE_OPEN(name) : 0 0 name OPEN-OP tid IPC4-CALL
    0 0 PE-NAME @ FILE-OPEN-OP FILEMGR-TID-ADDR @ IPC4-CALL
    >R DROP DROP DROP R>        \ r0=fid/err のみ
    DUP 0< IF                   \ err(<0)?
        DROP -1 R> PROC-REPLY EXIT
    THEN
    PE-FID !                    \ fid 退避

    \ (3) FILE_STAT(name -> statbuf) : 0 statbuf name STAT-OP tid IPC4-CALL
    0 PE-STATBUF @ PE-NAME @ FILE-STAT-OP FILEMGR-TID-ADDR @ IPC4-CALL
    >R DROP DROP DROP R>        \ r0=err のみ
    DUP 0< IF
        DROP
        \ OPEN済みなので CLOSE してから失敗REPLY
        0 0 PE-FID @ FILE-CLOSE-OP FILEMGR-TID-ADDR @ IPC4-CALL
        DROP DROP DROP DROP
        -1 R> PROC-REPLY EXIT
    THEN
    DROP                        \ err(=0) 破棄
    \ statbuf から size 取得。FILE_STAT statbuf レイアウトは
    \ FileMgr設計 v1.9.5 §4.3.5 準拠(実装時裏取り・本書RV-6/D-1)。
    \ ここでは statbuf先頭2B = size と仮定(実装時にオフセット確認)。
    PE-STATBUF @ @ PE-SIZE !    \ size = [statbuf+0]

    \ (4) size <= 2KB 検証
    PE-SIZE @ PROC-MAX-SIZE > IF
        0 0 PE-FID @ FILE-CLOSE-OP FILEMGR-TID-ADDR @ IPC4-CALL
        DROP DROP DROP DROP
        -1 R> PROC-REPLY EXIT
    THEN

    \ (5) FILE_READ(fid, $C000, size) : size dst fid READ-OP tid IPC4-CALL
    PE-SIZE @ PROC-LOAD-ADDR PE-FID @ FILE-READ-OP FILEMGR-TID-ADDR @ IPC4-CALL
    >R DROP DROP DROP R>        \ r0=actual/err
    DUP 0< IF
        DROP
        0 0 PE-FID @ FILE-CLOSE-OP FILEMGR-TID-ADDR @ IPC4-CALL
        DROP DROP DROP DROP
    ...
    \ (6) FILE_CLOSE(fid) : 0 0 fid CLOSE-OP tid IPC4-CALL
    0 0 PE-FID @ FILE-CLOSE-OP FILEMGR-TID-ADDR @ IPC4-CALL
    DROP DROP DROP DROP

    \ (7) TASK-CREATE($C000) → tid
    PROC-LOAD-ADDR TASK-CREATE  \ ( -- new_tid )
    DUP 0< IF                   \ TCB枯渇(-1)?
        DROP -1 R> PROC-REPLY EXIT
    THEN
    \ (8) 登録
    DUP LOAD-SLOT-BUSY !        \ BUSY=new_tid
    DUP PROC-CUR-TID !          \ 現プロセス=new_tid
    \ (9) new_tid を REPLY
    R> PROC-REPLY ;

\ --------------------------------------------------------------------
\ PROC-KILL-IMPL  ( arg2 arg1 arg0 tid -- )    op=$0302  本書§7.1
\   arg0=対象tid。TCB-STATE! で DEAD化(機構はカーネル・方針はサーバ)
\   TCB-STATE! ( state tid -- )
\ --------------------------------------------------------------------
: PROC-KILL-IMPL  ( arg2 arg1 arg0 tid -- )
    >R                          \ R: client_tid  DSP:( arg2 arg1 arg0 )
    \ arg0=対象tid。TCB-STATE!( state tid --) へ ( TASK-DEAD target )
    TASK-DEAD SWAP TCB-STATE!   \ ( arg2 arg1 ) 残, 対象tidをDEAD化
    DROP DROP                   \ arg1/arg2 破棄
    0 LOAD-SLOT-BUSY !          \ ロード領域解放
    0 R> PROC-REPLY ;           \ 0 を REPLY

\ --------------------------------------------------------------------
\ PROC-WAIT-IMPL  ( arg2 arg1 arg0 tid -- )    op=$0303  本書§6
\   arg0=対象tid。TCB-STATE@ を TASK-DEAD までビジーポーリング。
\   IRQ0プリエンプションでC側が走りDEAD化→脱出(本書§6.2 往復構造)
\ --------------------------------------------------------------------
: PROC-WAIT-IMPL  ( arg2 arg1 arg0 tid -- )
    >R                          \ R: client_tid  DSP:( arg2 arg1 arg0 )
    \ arg0=対象tid を TOS に残す
    SWAP DROP SWAP DROP         \ ( arg2 arg1 target )→ target のみ残す
    BEGIN
        DUP TCB-STATE@ TASK-DEAD =
    UNTIL                       \ DEAD まで spin
    DROP                        \ 対象tid 破棄
    0 LOAD-SLOT-BUSY !          \ 占有解除
    PROC-EXIT-CODE @            \ 終了コード(Phase1=0)
    R> PROC-REPLY ;

\ --------------------------------------------------------------------
\ PROC-LIST-IMPL  ( arg2 arg1 arg0 tid -- )    op=$0304  本書§7.2/C-2
\   arg0=buf_addr, arg1=buf_size。使用中(state<>DEAD)tidを2Bずつbufへ。
\   上限 buf_size/2 件。件数を REPLY。
\ --------------------------------------------------------------------
VARIABLE PL-BUF             \ 書込先ポインタ
VARIABLE PL-REMAIN          \ 残り書込可能件数
VARIABLE PL-COUNT           \ 書込済件数
VARIABLE PL-IDX             \ 走査tid
: PROC-LIST-IMPL  ( arg2 arg1 arg0 tid -- )
    >R                          \ R: client_tid  DSP:( arg2 arg1 arg0 )
    PL-BUF !                    \ arg0=buf_addr   DSP:( arg2 arg1 )
    1 RSHIFT PL-REMAIN !        \ arg1=buf_size → /2 = 件数上限 DSP:( arg2 )
    DROP                        \ arg2 破棄
    0 PL-COUNT !
    0 PL-IDX !
    BEGIN
        PL-IDX @ 16 <           \ tid 0..15
    WHILE
        PL-IDX @ TCB-STATE@ TASK-DEAD = INVERT IF   \ 使用中?
            PL-REMAIN @ 0> IF                       \ 枠あり?
                PL-IDX @ PL-BUF @ !                  \ tid を buf へ(2B)
                PL-BUF @ 2 + PL-BUF !
                PL-COUNT @ 1 + PL-COUNT !
                PL-REMAIN @ 1 - PL-REMAIN !
            THEN
        THEN
        PL-IDX @ 1 + PL-IDX !
    REPEAT
    PL-COUNT @ R> PROC-REPLY ;

\ --------------------------------------------------------------------
\ PROC-REORDER-MSG  ( msg3 msg2 msg1 msg0 -- arg2 arg1 arg0 op )  恒等
\ --------------------------------------------------------------------
: PROC-REORDER-MSG  ( msg3 msg2 msg1 msg0 -- arg2 arg1 arg0 op )
    ;

\ --------------------------------------------------------------------
\ PROC-DISPATCH  ( arg2 arg1 arg0 op tid -- )
\ --------------------------------------------------------------------
: PROC-DISPATCH  ( arg2 arg1 arg0 op tid -- )
    OVER PROC-EXEC-OP = IF NIP PROC-EXEC-IMPL EXIT THEN
    OVER PROC-KILL-OP = IF NIP PROC-KILL-IMPL EXIT THEN
    OVER PROC-WAIT-OP = IF NIP PROC-WAIT-IMPL EXIT THEN
    OVER PROC-LIST-OP = IF NIP PROC-LIST-IMPL EXIT THEN
    DROP DROP DROP DROP         \ arg0/arg1/arg2/op 破棄
    -1 SWAP PROC-REPLY ;        \ 未知op は -1

\ --------------------------------------------------------------------
\ PROCMGR-TASK  ( -- )   tid=5。FILEMGR-TASK と同形サーバループ
\ --------------------------------------------------------------------
: PROCMGR-TASK  ( -- )
    BEGIN
        IPC4-RECV               \ ( -- msg3 msg2 msg1 msg0 )
        ...
        IPC4-SENDER-DIRECT >R   \ R: client_tid
        EI-OP
        PROC-REORDER-MSG        \ ( -- arg2 arg1 arg0 op )
        R>                      \ ( -- arg2 arg1 arg0 op tid )
        PROC-DISPATCH
    AGAIN ;

CODE PROCMGR-TASK-ADDR  ( -- addr )
    SUBI X, #2
...
\ --------------------------------------------------------------------
\ PROCMGR-START  ( -- )   OS-START 5番目で呼ぶ → tid=5 自然確保
\ --------------------------------------------------------------------
: PROCMGR-START  ( -- )
    0 LOAD-SLOT-BUSY !
    0 PROC-CUR-TID !
...
\ OS-START  v0.10.13 (Ph.5・案A-2: テストタスク除外・本番起動列へ是正)
\   design v2.6 §4.4.1 本番起動列に一致。テストタスクは本番非搭載。
\   PROCMGR-START を5番目に置き tid=5 を自然確保(yuios_ph5_procmgr_impl_design_v1_0 §2.2)
\   ※旧テストタスク起動行(UART-TEST/STOR-TEST/FILEMGR-TEST)は検証時のみ復活させる
...
: SH-CMD-HELP  ( -- )
    ...
    $70 SH-EMIT $73 SH-EMIT SH-CR                 \ "ps"
...
\ --- [SHELL phase3] ver コマンド追加 (v0.10.18) ---
\ OSバージョン表示。文字列バッファ不使用・コード即値EMIT(help流儀)。
\ "YUIOS V0.10.18"
: SH-CMD-VER  ( -- )
...
\ 先頭文字分岐ヘルパ: SH-CMD-BUF の先頭1文字を取得 ( -- ch )
: SH-CMD0  ( -- ch )  SH-CMD-BUF C@ ;

\ --- [SHELL phase3] mem コマンド追加 (v0.10.18) ---
\ MEM_QUERY($0103)→r0=空きページ数。"MEM: N/16" 表示。
\ 数値出力は SH-EMIT-TID(0-15用)を流用(16も "16" で正しく出る)。
: SH-CMD-MEM  ( -- )
    0 0 0  MEM-QUERY-OP  MEM-TID-ADDR @  IPC4-CALL
    >R DROP DROP DROP R>                 \ r0=空きページ数
    $4D SH-EMIT $45 SH-EMIT $4D SH-EMIT  \ "MEM"
    $3A SH-EMIT $20 SH-EMIT              \ ": "
    SH-EMIT-TID                          \ 空きページ数(0-16)
    $2F SH-EMIT                          \ '/'
    $31 SH-EMIT $36 SH-EMIT              \ "16" (PAGE-TOTAL)
    SH-CR ;

\ --- [SHELL phase3] kill コマンド追加 (v0.10.18) ---
\ SH-ATOI: SH-ARG-BUF のヌル終端10進文字列を符号なし数値化。
\   ( -- n )  正常=値(>=0) / 非数字含む or 空 = -1
\   VARIABLE不使用(VAR域0B逼迫・KY)。Rに累積値、TOSに走査ポインタ。
: SH-ATOI  ( -- n )
    SH-ARG-BUF C@ 0= IF -1 EXIT THEN     \ 空文字列→-1
    0 >R                                 \ R=acc=0
    SH-ARG-BUF                           \ TOS=ptr
    BEGIN
        DUP C@ DUP 0= IF                 \ 終端?
            DROP DROP R> EXIT            \ acc を返す
        THEN
        DUP $30 < OVER $39 > OR IF       \ '0'..'9' 以外?
            DROP DROP R> DROP -1 EXIT    \ 非数字→-1
        THEN
        $30 -                            \ 数字値 d
        R> 10 * + >R                     \ acc = acc*10 + d
        1 +                              \ ptr++
...
\ kill <tid>: tid7..15 のみ許可。保護(0..6)='P'、範囲外/非数字='?'
: SH-CMD-KILL  ( -- )
    SH-ATOI                              \ ( -- n )
    DUP 0< IF DROP $3F SH-EMIT SH-CR EXIT THEN        \ 非数字/空→'?'
    DUP 7 < IF DROP $50 SH-EMIT SH-CR EXIT THEN       \ 0..6 保護→'P'
    DUP 15 > IF DROP $3F SH-EMIT SH-CR EXIT THEN      \ 16..→'?' (有効tid 0..15)
    DUP TCB-STATE@ TASK-DEAD = IF                     \ 既にdead/不在?
        DROP $44 SH-EMIT SH-CR EXIT THEN              \ →'D'
    0 0 ROT  PROC-KILL-OP  PROC-TID-ADDR @  IPC4-CALL \ arg2 arg1 arg0=tid
    DROP DROP DROP DROP
    $4B SH-EMIT SH-CR ;                               \ 完了→'K'

\ --- [SHELL phase3] ls コマンド追加 (v0.10.18・A案: 先頭4件) ---
\ 0..99 を10進出力(除算不使用・十の位を減算ループで算出)。
\ SH-EMIT-TID は0..15専用でNが16+で壊れるため、総数Nには本ワードを使う。
: SH-EMIT-DEC2  ( n -- )
    DUP 10 < IF
        $30 + SH-EMIT EXIT               \ 1桁(0-9)
    THEN
    0 SWAP                               \ ( tens=0 n )
    BEGIN DUP 10 >= WHILE                \ n>=10 の間
        10 - SWAP 1 + SWAP               \ n-=10, tens++
    REPEAT                               \ ( tens ones )
    SWAP $30 + SH-EMIT                   \ 十の位
    $30 + SH-EMIT ;                      \ 一の位
\ FILE_LIST($0207)→SH-LINE-BUF(64B=4件)へ。各16Bエントリ名をTYPE。
\ 末尾に "表示件数/総数(FS-FILE-COUNT)"。全件化はD案(PAGE-POOL借用)で後日。
\ SH-LINE-BUF流用: ls実行はREADLINE完了後ゆえ入力行と時系列競合なし。
\ VARIABLE不使用(VAR域0B逼迫・KY)。ループ状態はR/データスタックで完結。
: SH-CMD-LS  ( -- )
    0 64 SH-LINE-BUF  FILE-LIST-OP  FILEMGR-TID-ADDR @  IPC4-CALL
    >R DROP DROP DROP R>                 \ r0=件数 or 負(エラー)
    DUP 0< IF DROP $21 SH-EMIT SH-CR EXIT THEN        \ エラー→'!'
    \ r0=表示件数。各16Bエントリ名を出力
    DUP >R                               \ R=件数(後で件数表示に再利用)
    0                                    \ i=0  (TOS)
    BEGIN  DUP R@ <  WHILE               \ i < 件数
        DUP 4 LSHIFT SH-LINE-BUF +       \ entry = SH-LINE-BUF + i*16
        SH-TYPE  SH-CR                   \ ファイル名出力
        1 +                              \ i++
    REPEAT
    DROP                                 \ i 捨てる
    \ 末尾 "表示件数/総数"
    R> SH-EMIT-TID                       \ 表示件数(0-4)
    $2F SH-EMIT                          \ '/'
    FS-FILE-COUNT @ SH-EMIT-DEC2         \ 総数N(0-32・2桁対応)
    SH-CR ;

\ --- [SHELL phase3] cat コマンド追加 (v0.10.18) ---
\ FILE_OPEN→FILE_READ(64Bずつ逐次・OT-POS自動前進)→FILE_CLOSE。
\ 読み先=SH-LINE-BUF(65B)流用。actualバイトをそのままEMIT(ヌル終端非依存)。
\ FILE-READは pos>=fsz で actual=0 を返す→ループ終了。VARIABLE不使用。
\ fid は R スタックで保持(IPC4-CALLを跨ぐためデータスタックでなくRに退避)。
: SH-CAT-DUMP  ( n addr -- )         \ addr から n バイトを EMIT
    SWAP                             \ ( addr n )
    BEGIN  DUP 0>  WHILE             \ n>0
        OVER C@ SH-EMIT              \ [addr] を出力
        SWAP 1 + SWAP                \ addr++
        1 -                          \ n--
    REPEAT
    DROP DROP ;
...
: SH-CMD-CAT  ( -- )
    SH-ARG-BUF C@ 0= IF $3F SH-EMIT SH-CR EXIT THEN   \ 引数なし→'?'
    0 0 SH-ARG-BUF FILE-OPEN-OP FILEMGR-TID-ADDR @ IPC4-CALL
    >R DROP DROP DROP R>             \ r0=fid or err
    DUP 0< IF DROP $21 SH-EMIT SH-CR EXIT THEN        \ open失敗→'!'
    >R                               \ R=fid
    BEGIN
        64 SH-LINE-BUF R@ FILE-READ-OP FILEMGR-TID-ADDR @ IPC4-CALL
        >R DROP DROP DROP R>         \ r0=actual or err
        DUP 1 < IF                   \ EOF(0) or err(<0) : actual<1 ⇔ actual<=0
            DROP  0                  \ ループ脱出フラグ
        ELSE
            DUP SH-LINE-BUF SH-CAT-DUMP   \ actualバイト出力
            64 <                     \ actual<64 なら最終ブロック→脱出
        THEN
    UNTIL
    R>                               \ fid 回収
    0 0 ROT FILE-CLOSE-OP FILEMGR-TID-ADDR @ IPC4-CALL
    DROP DROP DROP DROP
    SH-CR ;
...
: SH-DISPATCH  ( -- )
    ...
    \ --- phase3: 先頭文字分岐(DATA域消費ゼロ・全頭文字一意) ---
    SH-CMD0 $76 = IF SH-CMD-VER EXIT THEN                        \ 'v' ver
    SH-CMD0 $6D = IF SH-CMD-MEM EXIT THEN                        \ 'm' mem
    SH-CMD0 $6B = IF SH-CMD-KILL EXIT THEN                       \ 'k' kill
    SH-CMD0 $6C = IF SH-CMD-LS EXIT THEN                         \ 'l' ls
    SH-CMD0 $63 = IF SH-CMD-CAT EXIT THEN                        \ 'c' cat
    $3F SH-EMIT SH-CR ;
...
\ --- [SHELL phase3] 起動メッセージ (v0.10.18・一括表示案・1行版) ---
\ 全コアサービス起動後・SHELL-START前に1回出力。emit-char直(ドライバ非依存)。
\ "YUIOS Booted!" のみ。2行目"YUKARI Semiconductor Devices."は辞書天井($D3FF)
\ 制約で削除→Level2 C実装Shell+端末サービスのブートログで復活予定。
\ 0123MD マーカーは温存(各サービス内蔵・非改変)。
: BOOT-MSG  ( -- )
    $59 emit-char $55 emit-char $49 emit-char $4F emit-char $53 emit-char
    $20 emit-char $42 emit-char $6F emit-char $6F emit-char $74 emit-char
    $65 emit-char $64 emit-char $21 emit-char emit-nl ;

: OS-START  ( -- )
    MEMMGR-START                    \ tid=1: MemMgr
    UART-START                      \ tid=2: UARTドライバ
    STOR-START                      \ tid=3: ストレージドライバ
    FILEMGR-START                   \ tid=4: FileMgr
    PROCMGR-START                   \ tid=5: ProcMgr (★Ph.5 v0.10.13 新規)
    BOOT-MSG                         \ 起動メッセージ (★Ph.6 v0.10.18 一括表示)
    SHELL-START                     \ tid=6: Forth常駐Shell (★Ph.6)

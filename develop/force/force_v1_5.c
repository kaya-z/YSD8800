 ...
 *
 * v1.5 (2026-05-24): _FMUL_A/B/R を $4232-$4236 から $47B0-$47B4 へ移動。
 *                    TCB プール拡張 (memmap v1.3: 16 タスク化) と _FMUL ハードコード
 *                    が衝突し、tid=7 の TCB を破壊する重大バグ修正。
 *                    詳細は backend/codegen.c v1.5 改版コメント参照。
 * v1.4 (2026-05-23): VARIABLE/VALUE/DEFER のデータ部とコード部を分離出力。
 *                    hasm23 .org 後退による getter コード上書きバグを回避。
 *                    詳細は backend/codegen.c v1.4 改版コメント参照。
 * v1.2: ベースライン
 *
 * 使用法:
